...
duplicate_versions="$(
    find crates/dcode-db/migrations -maxdepth 1 -type f -name '*.sql' -print \
        | awk -F/ '
            {
                name = $NF
                if (name ~ /^[0-9]+_/) {
...
if [[ "${DCODE_ALLOW_MAIN_MIGRATION_EDIT:-}" == "1" ]]; then
    echo "DCODE_ALLOW_MAIN_MIGRATION_EDIT=1; skipping migration immutability check"
    exit 0
fi

base_ref="${DCODE_MIGRATION_BASE_REF:-}"
if [[ -z "$base_ref" ]]; then
...
changed="$(
    git diff --name-status --diff-filter=DMR "$base_commit" -- 'crates/dcode-db/migrations/*.sql'
)"
...
If this is an intentional high-risk exception, rerun with DCODE_ALLOW_MAIN_MIGRATION_EDIT=1.
