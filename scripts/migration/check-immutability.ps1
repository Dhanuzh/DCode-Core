...
$migrationDir = Join-Path $repoRoot "crates/dcode-db/migrations"
...
if ($env:DCODE_ALLOW_MAIN_MIGRATION_EDIT -eq "1") {
    Write-Output "DCODE_ALLOW_MAIN_MIGRATION_EDIT=1; skipping migration immutability check"
    exit 0
}

$baseRef = $env:DCODE_MIGRATION_BASE_REF
...
$changed = git diff --name-status --diff-filter=DMR $baseCommit -- "crates/dcode-db/migrations/*.sql"
if (-not [string]::IsNullOrWhiteSpace(($changed -join "`n"))) {
    ...
    [Console]::Error.WriteLine("If this is an intentional high-risk exception, rerun with DCODE_ALLOW_MAIN_MIGRATION_EDIT=1.")
    [Console]::Error.WriteLine("")
