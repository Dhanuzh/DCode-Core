...
`image_input_models.json` is a hand-maintained DCode Core asset. It is embedded at compile time and is never downloaded or refreshed at runtime. Its API roots mirror the fixed `base_url` presets in AionUi's `modelPlatforms.ts`; model IDs are maintained independently from `models.dev`.

The catalog is intentionally a positive allowlist:

- Match both the provider API root and the exact model ID.
- Add a model only when the provider's own documentation confirms image input on the API protocol used by DCode Core.
...
- Aggregator entries may be refreshed manually from that aggregator's own catalog. The reviewed result must be committed as a static snapshot; DCode Core never fetches it at runtime.
