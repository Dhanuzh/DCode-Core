...
use crate::types::{DCODE_RUNTIME_TOKEN_ENV, BuildTaskOptions, RuntimeCapabilities};
...
impl WorkerTaskManagerImpl {
    ...
    fn refresh_runtime_token_for_new_task(&self, options: &mut BuildTaskOptions) {
        ...
        options
            .context
            .runtime_env
            .retain(|(key, _)| key != DCODE_RUNTIME_TOKEN_ENV);
        options
            .context
            .runtime_env
            .push((DCODE_RUNTIME_TOKEN_ENV.to_owned(), issue.token));
    }
}
...
/// Wired up by `dcode-app` so deleting a conversation tears down its
...
mod tests {
    ...
    use dcode_common::{AgentKillReason, AgentType, ConversationStatus, ProviderWithModel};
    ...
    fn make_team_options_with_runtime_token(conversation_id: &str, runtime_token: &str) -> BuildTaskOptions {
        ...
        let team = dcode_api_types::TeamSessionBinding {
            team_id: "team-1".into(),
            ...
            move |opts: BuildTaskOptions| {
                ...
                async move {
                    if let Some((_, token)) = opts
                        ...
                        .runtime_env
                        .iter()
                        .find(|(key, _)| key == DCODE_RUNTIME_TOKEN_ENV)
                    {
