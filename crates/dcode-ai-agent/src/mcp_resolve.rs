...
//! module emits the NEUTRAL `dcode_api_types::SessionMcpServer` so the app
//! boundary (`dcode-app`) can convert it once into the crate-local
//! `dcode_session::McpServerSpec`, and each backend serializes that into its own
...
use dcode_runtime::ensure_runtime_command;
