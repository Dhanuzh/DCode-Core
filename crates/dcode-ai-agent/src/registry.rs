...
use dcode_common::{AgentType, now_ms};
use dcode_db::{AgentMetadataRow, IAgentMetadataRepository, UpdateAgentHandshakeParams};
...
impl AgentRegistry {
    ...
    pub async fn run_slow_probe_recheck(&self) {
        ...
        for id in pending {
            ...
            let params = dcode_db::UpdateAgentAvailabilitySnapshotParams {
...
fn decode_row(
    ...
    let runtime_backend = dcode_db::runtime_backend_for_agent(&row);
    ...
    let descriptor_team_capable = dcode_session::backend_capability_descriptor(&runtime_backend)
        .map(dcode_session::BackendCapabilityDescriptor::resolved)
    ...
    let team_capable = behavior_policy.supports_team
        ...
        || dcode_common::constants::is_team_capable(persisted_backend, handshake.agent_capabilities.as_ref());
    ...
    if is_internal_aion_cli && command_override.is_some() {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring command override for internal DCode CLI agent"
        );
    }
    ...
    if is_internal_aion_cli && env_override.as_ref().is_some_and(|entries| !entries.is_empty()) {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring environment overrides for internal DCode CLI agent"
        );
    }
...
/// unavailable rows go to `info!` so the default dcode-core.log surfaces
...
/// agent's runtime environment or DCode-internal wiring. Case-insensitive.
pub(crate) fn is_blocked_override_env_key(key: &str) -> bool {
    let upper = key.to_ascii_uppercase();
    if upper.starts_with("DCODE_") {
        return true;
    }
    matches!(
        upper.as_str(),
...
mod tests {
    use super::*;
    use dcode_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database_memory};
    ...
    async fn codex_yolo_id_maps_to_agent_full_access() {
        ...
        // Legacy DCode yolo aliases resolve to Codex's native
    ...
    async fn pi_builtin_uses_stable_acp_adapter_and_requires_pi_cli() {
        ...
        // gives it the `$DCODE_HELPER_BIN team ...` command surface instead of
    ...
    /// (DCode CLI is `agent_source = internal` with no `command`).
    ...
    async fn list_all_filters_out_unavailable_rows() {
        ...
        // DCode CLI (internal, no spawn command) is always available.
        assert!(
    ...
    #[test]
    fn decode_row_ignores_internal_dcode_cli_overrides() {
        use dcode_db::AgentMetadataRow;
        let row = AgentMetadataRow {
            id: "632f31d2".to_string(),
            user_id: None,
            icon: None,
            name: "DCode CLI".to_string(),
            name_i18n: None,
            description: None,
            description_i18n: None,
            backend: None,
