...
use dcode_api_types::{AcpBuildExtra, AgentMetadata};
...
fn append_runtime_env(command_spec: &mut CommandSpec, runtime_env: &[(String, String)]) {
    for (name, value) in runtime_env {
        command_spec.env.push(dcode_common::EnvVar {
            name: name.clone(),
            value: value.clone(),
        });
    }
...
fn apply_codex_runtime_config_args(
    ...
    command_spec
        .args
        .extend(dcode_session::codex_shell_environment_policy_args().map(str::to_owned));
    ...
    fn apply_acp_launch_policy_adds_runtime_env_and_codex_full_access_config() {
        ...
        apply_acp_launch_policy(
            ...
            AcpLaunchPolicyInput {
                ...
                runtime_env: &[("DCODE_CONVERSATION_ID".into(), "conv-1".into())],
            },
        );
        ...
        assert!(
            command_spec
                .env
                .iter()
                .any(|entry| entry.name == "DCODE_CONVERSATION_ID" && entry.value == "conv-1")
        );
    }
