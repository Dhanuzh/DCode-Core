...
use dcode_common::ProviderWithModel;
use dcode_db::IMcpServerRepository;
use dcode_db::models::McpServerRow;
use dcode_realtime::EventBroadcaster;
use dcode_runtime::ensure_runtime_command_with_reporter;
...
pub(super) async fn build(
    ...
    let api_key = dcode_common::decrypt_string(&row.api_key_encrypted, &deps.encryption_key)
    ...
    let vertex_config = if row.platform == "vertex-ai" {
        resolve_vertex_config(row.vertex_config.as_deref())
    } else {
        None
    };

    // Gemini on Vertex uses the OpenAI-compatible endpoint, which authenticates
    // with a short-lived OAuth bearer token rather than the static api_key other
    // providers carry, and expects the model id qualified as `<publisher>/<model>`
    // — so URL, credential, and model id are all replaced here.
    let mut api_key = api_key;
    let mut base_url = base_url;
    let mut model_id = model_id;
    if row.platform == "vertex-ai" && !is_anthropic_model(&model_id) {
        let cfg = row
            .vertex_config
            .as_deref()
            .and_then(|json| serde_json::from_str::<dcode_api_types::VertexConfig>(json).ok())
            .ok_or_else(|| {
                AgentError::bad_request(String::from("Vertex provider is missing its project configuration"))
            })?;

        base_url = Some(vertex_openai_base_url(&cfg.project_id, &cfg.location));
        api_key = vertex_bearer_token(&cfg).await?;
        if !model_id.contains('/') {
            model_id = format!("google/{model_id}");
        }
    }
...
/// Map DCode DB platform/protocol settings to the aionrs provider identifier.
pub(crate) fn map_aionrs_provider(
    ...
    match platform {
        ...
        // One Vertex project serves both publishers, but aionrs' Vertex client
        // speaks the Anthropic wire format (publishers/anthropic/…:streamRawPredict),
        // so only Claude models can route through it. Gemini models on the same
        // provider need the OpenAI-compatible Vertex endpoint instead.
        // Claude goes through aionrs' Vertex client (Anthropic wire format);
        // Gemini goes through Vertex's OpenAI-compatible endpoint, whose URL and
        // bearer token are resolved in resolve_vertex_gemini_transport().
        "vertex-ai" => {
            return if is_anthropic_model(model_id) {
                Ok("vertex".to_owned())
            } else {
                Ok("openai".to_owned())
            };
...
/// Claude models are the only ones aionrs' Vertex client can serve.
pub(crate) fn is_anthropic_model(model_id: &str) -> bool {
    model_id.to_ascii_lowercase().starts_with("claude")
}

/// Base URL for Gemini on Vertex via the OpenAI-compatible surface.
///
/// aionrs' OpenAI client appends `/chat/completions`, so this stops at
/// `endpoints/openapi`.
/// Mint a bearer token for Gemini's OpenAI-compatible Vertex endpoint.
async fn vertex_bearer_token(cfg: &dcode_api_types::VertexConfig) -> Result<String, AgentError> {
    let http = reqwest::Client::new();
    dcode_system::model_fetcher::vertex::mint_access_token(&http, cfg)
        .await
        .map_err(|e| AgentError::bad_request(format!("Google Cloud authentication failed: {e}")))
}

pub(crate) fn vertex_openai_base_url(project_id: &str, location: &str) -> String {
    // The `global` location is served by the unprefixed host.
    let host = if location == "global" {
        "aiplatform.googleapis.com".to_string()
    } else {
        format!("{location}-aiplatform.googleapis.com")
    };
    format!("https://{host}/v1/projects/{project_id}/locations/{location}/endpoints/openapi")
}
...
pub(crate) fn resolve_vertex_config(json: Option<&str>) -> Option<aion_config::config::VertexConfig> {
    ...
    Some(aion_config::config::VertexConfig {
        ...
        // ADC needs no file: aionrs falls back to the gcloud application
        // default credentials when neither credential field is set.
        credentials_file: None,
        service_account_json: match vc.auth_method {
            dcode_api_types::VertexAuthMethod::ServiceAccount => vc.service_account_json,
            dcode_api_types::VertexAuthMethod::Adc => None,
        },
    })
}

pub(crate) fn resolve_bedrock_config(json: Option<&str>) -> Option<aion_config::config::BedrockConfig> {
    let bc: dcode_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
...
mod tests {
    use super::*;
    use dcode_realtime::BroadcastEventBus;
    use dcode_runtime::{ManagedResourcesMode, init as init_runtime, set_managed_resources_mode};
    ...
    impl BundledRuntimeModeGuard {
        fn install(root: &Path) -> Self {
            unsafe { std::env::set_var("DCODE_BUNDLED_MANAGED_RESOURCES", root) };
    ...
    impl Drop for BundledRuntimeModeGuard {
        fn drop(&mut self) {
            unsafe { std::env::remove_var("DCODE_BUNDLED_MANAGED_RESOURCES") };
    ...
    impl IMcpServerRepository for MockMcpRepo {
        async fn list(&self, user_id: &str) -> Result<Vec<McpServerRow>, dcode_db::DbError> {
            Ok(self.rows.iter().filter(|row| row.user_id == user_id).cloned().collect())
        }

        async fn find_by_id(&self, user_id: &str, id: &str) -> Result<Option<McpServerRow>, dcode_db::DbError> {
            Ok(self
                .rows
                .iter()
        ...
        async fn find_by_name(&self, user_id: &str, name: &str) -> Result<Option<McpServerRow>, dcode_db::DbError> {
            Ok(self
                .rows
                .iter()
        ...
        async fn create(
            &self,
            _params: dcode_db::CreateMcpServerParams<'_>,
        ) -> Result<McpServerRow, dcode_db::DbError> {
        ...
        async fn update(
            ...
            _user_id: &str,
            _id: &str,
            _params: dcode_db::UpdateMcpServerParams<'_>,
        ) -> Result<McpServerRow, dcode_db::DbError> {
            unimplemented!("not needed for factory tests")
        }
        ...
        async fn batch_upsert(
            &self,
            _user_id: &str,
            _servers: &[dcode_db::CreateMcpServerParams<'_>],
        ) -> Result<Vec<McpServerRow>, dcode_db::DbError> {
            unimplemented!("not needed for factory tests")
        }
        ...
        async fn update_status(
            ...
            _id: &str,
            _status: &str,
            _last_connected: Option<dcode_common::TimestampMs>,
