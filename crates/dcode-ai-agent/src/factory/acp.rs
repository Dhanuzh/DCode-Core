...
use dcode_api_types::{AgentMetadata, SessionMcpServer, SessionMcpTransport};
use dcode_common::CommandSpec;
...
use dcode_runtime::{ensure_runtime_command, ensure_runtime_command_with_reporter};
        ...
        && matches!(route_for_backend(Some(backend_label)), BackendRoute::DirectCli)
    {
        let instance = crate::session_agent::build_session_instance(
            ...
            crate::session_agent::SessionBuildInputs {
                ...
                // The DCODE_* conversation runtime context the legacy path
...
async fn resolve_agent_command_spec(
    meta: &dcode_api_types::AgentMetadata,
    user_id: &str,
    workspace: &str,
    conversation_id: &str,
    ...
    let mut args: Vec<String> = resolved
        ...
        .collect();
    let launch_args = if meta.agent_source == dcode_api_types::AgentSource::Builtin
        && meta.agent_source_info.bridge_binary.as_deref() == Some("npx")
        && let Some(backend) = meta.backend.as_deref()
    {
        dcode_runtime::pin_registry_npx_args(backend, &meta.args)
    ...
    let mut env: Vec<dcode_common::EnvVar> = meta
        .env
        .iter()
        ...
        .collect();
    env.extend(resolved.env.iter().map(|(name, value)| dcode_common::EnvVar {
...
mod tests {
    ...
    async fn resolve_agent_command_spec_flattens_bare_npx_command() {
        ...
        let mut meta = dcode_api_types::AgentMetadata {
            id: "agent-1".into(),
            icon: None,
            name: "Test ACP".into(),
            name_i18n: None,
            ...
            agent_source: dcode_api_types::AgentSource::Custom,
            ...
            env: vec![dcode_api_types::AgentEnvEntry {
                name: "K".into(),
                value: "V".into(),
                description: None,
            }],
            native_skills_dirs: None,
            behavior_policy: dcode_api_types::BehaviorPolicy::default(),
            yolo_id: None,
            sort_order: 0,
            team_capable: false,
            ...
            handshake: dcode_api_types::AgentHandshake::default(),
        ...
        meta.agent_source = dcode_api_types::AgentSource::Builtin;
    ...
    impl IMcpServerRepository for MockRepo {
        async fn list(&self, user_id: &str) -> Result<Vec<McpServerRow>, dcode_db::DbError> {
            if self.fail {
                Err(dcode_db::DbError::Init("simulated".into()))
            } else {
                Ok(self.rows.iter().filter(|row| row.user_id == user_id).cloned().collect())
            }
        }
        async fn find_by_id(&self, _user_id: &str, _id: &str) -> Result<Option<McpServerRow>, dcode_db::DbError> {
            unimplemented!()
        }
        async fn find_by_name(&self, _user_id: &str, _name: &str) -> Result<Option<McpServerRow>, dcode_db::DbError> {
            unimplemented!()
        }
        async fn list_by_ids_any(
            ...
            if self.fail {
                return Err(dcode_db::DbError::Init("simulated".into()));
            }
            Ok(ids
                .iter()
                .filter_map(|id| {
    ...
    /// installed. Verified end-to-end from the DCode UI.
    #[test]
    ...
    fn every_registered_direct_descriptor_has_a_non_acp_factory_route() {
        for descriptor in dcode_session::backend_capability_descriptors()
            .iter()
            .filter(|descriptor| descriptor.origin == dcode_common::CapabilityOrigin::DirectDescriptor)
        {
            assert_ne!(
    ...
    fn every_non_acp_factory_route_has_a_direct_descriptor() {
        for (backend, _) in configured_non_acp_routes() {
            let descriptor = dcode_session::backend_capability_descriptor(backend).unwrap_or_else(|| {
                panic!(
            ...
            assert_eq!(
                descriptor.origin,
                dcode_common::CapabilityOrigin::DirectDescriptor,
