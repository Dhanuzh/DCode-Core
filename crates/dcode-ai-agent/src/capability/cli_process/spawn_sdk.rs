use dcode_common::{CommandSpec, ErrorChain};
use dcode_runtime::Builder as CmdBuilder;
#[cfg(test)]
use std::path::Path;
use std::sync::Arc;
...
impl CliAgentProcess {
    ...
    pub async fn spawn_for_sdk(config: CommandSpec) -> Result<Self, AgentError> {
        ...
        let agent_env = dcode_runtime::agent_process_env().await;
...
mod tests {
    ...
    async fn spawn_for_sdk_uses_clean_agent_env_and_explicit_overrides() {
        const CHILD_ENV: &str = "DCODE_TEST_SDK_AGENT_ENV_CHILD";
        ...
        if std::env::var_os(CHILD_ENV).is_none() {
            ...
            write_fake_shell(
                ...
                r#"#!/bin/sh
printf '%s\n' \
  'DCODE_SHELL_ONLY=from-shell' \
  'DCODE_OVERLAY=from-shell' \
...
"#,
        ...
        let mut config = simple_script_config(
            "printf 'shell=%s\nconfig=%s\noverlay=%s\nnpm=%s\nnode=%s\n' \
             \"${DCODE_SHELL_ONLY:-unset}\" \
             \"${DCODE_CONFIG_ONLY:-unset}\" \
             \"${DCODE_OVERLAY:-unset}\" \
             \"${npm_lifecycle_event:-unset}\" \
             \"${NODE_OPTIONS:-unset}\"",
        );
        config.env.push(EnvVar {
            name: "DCODE_CONFIG_ONLY".into(),
            value: "from-config".into(),
        });
        config.env.push(EnvVar {
            name: "DCODE_OVERLAY".into(),
            value: "from-config".into(),
        });
