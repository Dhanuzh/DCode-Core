...
use dcode_common::{AgentType, ErrorChain};
...
fn write_registry_file(path: &Path, registry: &ProcessRegistry) -> io::Result<()> {
    ...
    // dcode-core backends sharing one data-dir) can never clobber each other's
...
/// sibling dcode-core instance sharing the same data-dir cannot interleave its
