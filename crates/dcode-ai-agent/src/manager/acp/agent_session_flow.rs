...
use dcode_api_types::SlashCommandItem;
...
impl AcpAgentManager {
    ...
    pub(super) async fn open_session_fork(&self, fork: &dcode_api_types::ForkSpec) -> Result<String, AgentError> {
    ...
    async fn emit_snapshot_events(&self) {
        use dcode_api_types::{ModelInfoEntry, ModelInfoPayload};
...
/// the text block's `[[DCODE_FILES]]` path list, byte-identical to the
...
mod tests {
    ...
    use dcode_api_types::{AgentErrorCode, SlashCommandCompletionBehavior, SlashCommandItem};
    ...
    async fn prompt_blocks_carry_native_image_when_capable() {
        ...
        let content = format!(
            "look\n\n{}\n{img}\n{pdf}",
            dcode_common::constants::DCODE_FILES_MARKER
        );
        let data = SendMessageData {
            content,
            msg_id: "m2".into(),
        ...
        match &blocks[0] {
            ContentBlock::Text(text) => {
                ...
                assert_eq!(
                    text.text,
                    format!("look\n\n{}\n{pdf}", dcode_common::constants::DCODE_FILES_MARKER)
                );
            }
            other => panic!("expected text block, got {other:?}"),
        }
    ...
    async fn prompt_blocks_fall_back_to_paths_when_read_fails() {
        ...
        let content = format!("gone\n\n{}\n{img_path}", dcode_common::constants::DCODE_FILES_MARKER);
        let data = SendMessageData {
            content: content.clone(),
            msg_id: "m3".into(),
            turn_id: None,
