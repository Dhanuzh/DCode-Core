...
impl AcpAgentManager {
    ...
    pub(crate) async fn mode(&self) -> Result<dcode_api_types::AgentModeResponse, AgentError> {
        let desired = self
            .session
            .read()
            .await
...
mod tests {
    ...
    fn session_command_loading_preserves_empty_turn_meta() {
        ...
        assert_eq!(
            ...
            Some(dcode_api_types::SlashCommandCompletionBehavior::NeutralTipOnEmpty)
        );
        assert_eq!(
    ...
    async fn register_failure_kills_spawned_process() {
        ...
        let process = Arc::new(
            CliAgentProcess::spawn_for_sdk(dcode_common::CommandSpec {
