...
#[test]
fn resolve_dcode_config_discards_standalone_max_token_settings() {
...
"#,
    ...
    let embedded = resolve_dcode_config(&cli_args).unwrap();
    assert_eq!(embedded.max_tokens, None);
    assert_eq!(embedded.compat.default_max_tokens_for_model("gpt-test"), None);
}

#[test]
fn resolve_dcode_config_keeps_builtin_provider_max_token_policy() {
    let project = tempfile::tempdir().unwrap();
    fs::write(
        project.path().join(".aionrs.toml"),
        r#"
...
fn aionrs_final_input_dump_value_contains_raw_split_input_and_context() {
    ...
    let context = AionrsFinalInputDumpContext {
        ...
        skills: vec!["dcode-config".to_owned()],
        mcp_servers,
        runtime_env: vec![("DCODE_RAW".to_owned(), "raw-env-value".to_owned())],
    };
    ...
    assert_eq!(value["resolved_context"]["skills"][0], "dcode-config");
    assert_eq!(
