...
fn resolve_dcode_config(cli_args: &CliArgs) -> Result<Config, AgentError> {
    let mut config =
        Config::resolve(cli_args).map_err(|e| AgentError::internal(format!("Config resolve failed: {e}")))?;

    // DCode owns the embedded runtime policy. Standalone aionrs max-token
...
impl AionrsAgentManager {
    pub async fn new(
        ...
        let mut config = resolve_dcode_config(&cli_args)?;

        // Backend-specific overrides
        config.bedrock = config_extra.bedrock_config;
        config.vertex = config_extra.vertex_config;
