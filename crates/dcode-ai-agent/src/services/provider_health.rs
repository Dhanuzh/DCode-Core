...
use dcode_db::{IProviderRepository, models::Provider};
use regex::Regex;
use tracing::{info, warn};

use crate::factory::aionrs::{
    map_aionrs_provider, resolve_aionrs_url_and_compat_with_mode, resolve_bedrock_config, resolve_vertex_config,
...
impl ProviderHealthCheckService {
    ...
    fn resolve_probe_config(&self, row: &Provider, model_id: &str) -> Result<AionrsResolvedConfig, AgentError> {
        let api_key = dcode_common::decrypt_string(&row.api_key_encrypted, &self.encryption_key)
...
mod tests {
    ...
    fn test_service() -> ProviderHealthCheckService {
        ProviderHealthCheckService {
            ...
            data_dir: PathBuf::from("/tmp/dcode-core-provider-health-test"),
        }
    }
