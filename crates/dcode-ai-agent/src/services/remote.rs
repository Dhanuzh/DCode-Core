...
use dcode_common::{RemoteAgentAuthType, RemoteAgentProtocol, RemoteAgentStatus, decrypt_string, encrypt_string};
use dcode_db::models::RemoteAgentRow;
use dcode_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
use base64::Engine;
...
impl RemoteAgentService {
    ...
    pub async fn create(
        ...
        let row = self
            .repo
            .create(dcode_db::CreateRemoteAgentParams {
                user_id,
                name: &req.name,
    ...
    pub async fn update(
        ...
        let row = self.repo.update(user_id, id, params).await.map_err(|e| match e {
            dcode_db::DbError::NotFound(msg) => AgentError::not_found(msg),
...
fn generate_device_keypair(encryption_key: &[u8; 32]) -> Result<(String, String, String), AgentError> {
    ...
    let device_id = dcode_common::generate_prefixed_id("dev");
...
fn db_err(e: dcode_db::DbError) -> AgentError {
