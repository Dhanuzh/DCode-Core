...
//! full `[[DCODE_FILES]]` form (UI chips and history are untouched); only
...
use dcode_common::constants::DCODE_FILES_MARKER;
use tracing::warn;
...
pub struct MediaPartition {
    /// Agent-bound content: the user text with the `[[DCODE_FILES]]` block
...
/// Strip the trailing `[[DCODE_FILES]]` block iff its path list matches
...
fn strip_files_marker<'a>(content: &'a str, files: &[String]) -> &'a str {
    let Some((user_text, metadata)) = content.rsplit_once(DCODE_FILES_MARKER) else {
        return content;
    };
...
mod tests {
    ...
    fn temp_file(name: &str, bytes: &[u8]) -> String {
        let dir = std::env::temp_dir().join("dcode-media-tests");
    ...
    fn svg_and_missing_and_oversized_stay_paths() {
        ...
        let missing = "/nonexistent/dcode-media-test.png".to_owned();
