    ...
    pub fn apply_conversation_runtime_context(
        ...
        self.context.runtime_env.retain(|(key, _)| {
            !matches!(
                key.as_str(),
                DCODE_USER_ID_ENV
                    | DCODE_CONVERSATION_ID_ENV
                    | DCODE_HELPER_BIN_ENV
                    | DCODE_BASE_URL_ENV
                    | DCODE_RUNTIME_TOKEN_ENV
            )
        });
        self.context
            .runtime_env
            .push((DCODE_USER_ID_ENV.to_owned(), user_id.to_owned()));
        self.context
            .runtime_env
            .push((DCODE_CONVERSATION_ID_ENV.to_owned(), conversation_id.to_owned()));
        if let Some(helper_bin) = helper_bin {
            self.context
                .runtime_env
                .push((DCODE_HELPER_BIN_ENV.to_owned(), helper_bin.to_owned()));
        }
        if let Some(base_url) = base_url {
            self.context
                .runtime_env
                .push((DCODE_BASE_URL_ENV.to_owned(), base_url.to_owned()));
        }
        if let Some(runtime_token) = runtime_token {
            self.context
                .runtime_env
                .push((DCODE_RUNTIME_TOKEN_ENV.to_owned(), runtime_token.to_owned()));
        }
        self.runtime_capabilities.conversation_runtime_context_version = Some(CONVERSATION_RUNTIME_CONTEXT_VERSION);
    }
}

pub const DCODE_USER_ID_ENV: &str = "DCODE_USER_ID";
pub const DCODE_CONVERSATION_ID_ENV: &str = "DCODE_CONVERSATION_ID";
pub const DCODE_HELPER_BIN_ENV: &str = "DCODE_HELPER_BIN";
pub const DCODE_BASE_URL_ENV: &str = "DCODE_BASE_URL";
pub const DCODE_RUNTIME_TOKEN_ENV: &str = "DCODE_RUNTIME_TOKEN";
...
pub struct AionrsResolvedConfig {
    ...
    /// Normal DCode conversations leave this unset.
    ...
    /// Google Cloud Vertex AI settings (project + location, service account or ADC).
    pub vertex_config: Option<aion_config::config::VertexConfig>,
...
mod tests {
    use super::*;
    use dcode_api_types::{AcpBuildExtra, AcpModelInfo, AionrsBuildExtra, SlashCommandItem};
    use serde_json::json;
    ...
    fn build_task_options_applies_conversation_runtime_context_once() {
        ...
        use dcode_common::{AgentType, ProviderWithModel};
        ...
        let context = AgentSessionContext {
            ...
            skills: vec![],
            runtime_env: vec![
                (DCODE_USER_ID_ENV.into(), "old-user".into()),
                (DCODE_CONVERSATION_ID_ENV.into(), "old-conv".into()),
                (DCODE_RUNTIME_TOKEN_ENV.into(), "old-token".into()),
        ...
        options.apply_conversation_runtime_context(
            "user-1",
            "conv-1",
            Some("/Applications/DCode/dcode-core"),
        ...
        assert_eq!(
            options
                ...
                .runtime_env
                .iter()
                .filter(|(key, _)| key == DCODE_USER_ID_ENV)
                .count(),
            1
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(DCODE_USER_ID_ENV.to_owned(), "user-1".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(DCODE_CONVERSATION_ID_ENV.to_owned(), "conv-1".to_owned()))
        );
        assert!(options.context.runtime_env.contains(&(
            DCODE_HELPER_BIN_ENV.to_owned(),
            "/Applications/DCode/dcode-core".to_owned()
        )));
        assert!(
            options
                .context
                .runtime_env
                .contains(&(DCODE_BASE_URL_ENV.to_owned(), "http://127.0.0.1:25808".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(DCODE_RUNTIME_TOKEN_ENV.to_owned(), "runtime-token-1".to_owned()))
        );
        assert_eq!(
            options
                .context
                .runtime_env
                .iter()
                .filter(|(key, _)| key == DCODE_RUNTIME_TOKEN_ENV)
                .count(),
            1
        );
