...
use dcode_api_types::{AgentHandshake, TryConnectCustomAgentResponse};
use dcode_common::{CommandSpec, EnvVar};

use crate::protocol::acp_init_budget::{INIT_TIMEOUT_SECS, InitBudget};
use dcode_runtime::{NodeRuntimeProgressReporter, ResolvedCommand, ensure_runtime_command_with_reporter};
...
mod tests {
    ...
    async fn probe_returns_fail_cli_when_command_missing() {
        let resp = try_connect_custom_agent("dcode-definitely-does-not-exist-xyz", &[], &HashMap::new(), None).await;
        match resp {
