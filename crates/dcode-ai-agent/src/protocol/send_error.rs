        ...
        match err {
            AgentError::WorkspacePathRuntimeUnavailable(path) => Self {
                stream_error: AgentStreamErrorData {
                    ...
                    ownership: Some(AgentErrorOwnership::Dcode),
            ...
            AgentError::Internal(_) => Self::new(
                "DCode failed while sending the message",
                AgentErrorCode::DcodeInternalError,
                AgentErrorOwnership::Dcode,
                Some(detail),
                true,
                true,
                resolution(
            ...
            AgentError::Forbidden(_) => Self::new(
                "DCode blocked the request before it reached the Agent",
                AgentErrorCode::DcodePermissionError,
                AgentErrorOwnership::Dcode,
                Some(detail),
                false,
                true,
    ...
    fn from_acp_non_internal(err: &AcpError, detail: String) -> Self {
        match err {
            ...
            AcpError::NotConnected => Self::new(
                "DCode lost its Agent protocol connection",
                AgentErrorCode::UserAgentDisconnected,
                AgentErrorOwnership::UserAgent,
                Some(detail),
                true,
            ...
            let classified = acp_agent_internal_texts(message, data.as_ref())
                ...
                .find_map(|text| {
                    ...
                    classify_agent_lifecycle(&lower)
                        ...
                        .or_else(|| classify_dcode_state(&lower))
...
fn classify_dcode_state(lower: &str) -> Option<ClassifiedError> {
    if lower.contains("conversation is already processing") {
        return Some(ClassifiedError {
            message: "The current response is still running",
            code: AgentErrorCode::DcodeConversationBusy,
            ownership: AgentErrorOwnership::Dcode,
            retryable: true,
            feedback_recommended: false,
...
mod tests {
    use super::*;
    use dcode_api_types::{AgentErrorResolutionKind, AgentErrorResolutionTarget};
    use serde_json::json;
    ...
    #[test]
    fn preserves_runtime_workspace_validation_as_structured_dcode_error() {
        let err =
        ...
        assert_eq!(err.ownership(), Some(AgentErrorOwnership::Dcode));
        assert_eq!(
    ...
    fn classifies_agent_setup_failures() {
        ...
        for managed_binary_missing in [
            "expected managed Claude ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\DCode\\dcode\\runtime\\acp\\claude-agent-acp\\0.58.1\\win32-x64\\node_modules\\@anthropic-ai\\claude-agent-sdk-win32-x64\\claude.exe",
            "expected managed Codex ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\DCode\\dcode\\runtime\\acp\\codex-acp\\1.1.2\\win32-x64\\node_modules\\@openai\\codex-win32-x64\\vendor\\x86_64-pc-windows-msvc\\bin\\codex.exe",
        ] {
            assert_classification(
    ...
    #[test]
    fn classifies_dcode_conversation_busy_after_agent_and_provider_checks() {
        assert_classification(
            "Conflict: Conversation is already processing a message",
            AgentErrorCode::DcodeConversationBusy,
            AgentErrorOwnership::Dcode,
