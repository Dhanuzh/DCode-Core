//! Writes the `hooks.json` that registers DCode's own binary as agy's
...
use dcode_api_types::AntigravityHookConfig;
...
///
/// The `matcher: "*"` is deliberate — DCode decides per call, so agy must ask
/// about all of them.
///
...
/// (it knows the workspace, not where DCode's binary lives). Handing it the
...
mod tests {
    ...
    fn registers_the_bridge_without_embedding_any_policy() {
        ...
        write_hooks_json(dir.path(), Path::new("/opt/dcode/backend")).unwrap();
        ...
        assert_eq!(
            ...
            "\"/opt/dcode/backend\" antigravity-hook"
        );

        // The decision must never be baked into the file: DCode owns it at
    ...
    fn an_install_path_with_spaces_stays_one_argument() {
        ...
        // unquoted path like /Applications/DCode App.app/... would be split into
        // two arguments and the hook would never run.
        let dir = tempfile::tempdir().unwrap();
        write_hooks_json(dir.path(), Path::new("/Apps/DCode App/backend")).unwrap();
        let raw = std::fs::read_to_string(dir.path().join(".agents/hooks.json")).unwrap();
        let v: serde_json::Value = serde_json::from_str(&raw).unwrap();
        assert_eq!(
            v[AntigravityHookConfig::HOOK_NAME]["PreToolUse"][0]["hooks"][0]["command"],
            "\"/Apps/DCode App/backend\" antigravity-hook"
        );
    }
    ...
    fn removing_the_hook_clears_a_previous_install() {
        ...
        let dir = tempfile::tempdir().unwrap();
        write_hooks_json(dir.path(), std::path::Path::new("/opt/dcode/dcode-core")).unwrap();
