...
pub trait IAgentTask: Send + Sync {
    ...
    /// `dcode_session::Capabilities::supports_midturn_delivery` — defaults
...
pub trait IMockAgent: IAgentTask {
    ...
    fn answer_ask(
        &self,
        _request_id: &str,
        _answers: Option<Vec<dcode_api_types::AskQuestionAnswer>>,
    ) -> Result<(), AgentError> {
    ...
    async fn mode(&self) -> Result<dcode_api_types::AgentModeResponse, AgentError> {
...
pub enum AgentInstance {
    ...
    /// Wraps an `dcode_session::SessionBackend` via [`SessionAgentTask`],
...
impl AgentInstance {
    ...
    pub async fn get_mode(&self) -> Result<dcode_api_types::AgentModeResponse, AgentError> {
        match self {
    ...
    /// [`dcode_common::normalize_keys_to_snake_case`] so keys land as
    /// `used` / `size` / `cost` to match the DCode wire convention —
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AgentError> {
        match self {
            Self::Acp(m) => {
                ...
                dcode_common::normalize_keys_to_snake_case(&mut value);
                Ok(Some(value))
            }
...
mod aionrs_config_option_tests {
    ...
    use dcode_api_types::ConfigOptionConfirmation;
