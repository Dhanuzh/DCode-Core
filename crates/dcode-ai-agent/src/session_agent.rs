//! `SessionAgentTask` — adapts the clean-slate `dcode_session::SessionBackend`
...
use dcode_common::{AgentKillReason, ConversationStatus, TimestampMs, now_ms};
use dcode_session::{
...
use dcode_api_types::AcpBuildExtra;
use dcode_common::AgentType;
use dcode_db::{IAcpSessionRepository, IMcpServerRepository, SaveRuntimeStateParams};
use dcode_realtime::EventBroadcaster;
...
fn resolve_current_model_efforts(models: &[dcode_session::ModelInfo], current_model: Option<&str>) -> Vec<String> {
...
/// handshake, in the SAME `dcode_session` shape the getters read off live
...
struct CatalogPreload {
    available_models: Vec<dcode_session::ModelInfo>,
    current_model: Option<String>,
    available_modes: Vec<dcode_session::ModeInfo>,
...
impl CatalogPreload {
    ...
    fn from_handshake(handshake: &dcode_api_types::AgentHandshake) -> Self {
...
impl SessionAgentTask {
    ...
    pub fn new_with_preload(
        ...
        backend: Arc<dyn SessionBackend>,
        session_repo: Option<Arc<dyn IAcpSessionRepository>>,
        handshake: &dcode_api_types::AgentHandshake,
        prompt_dump: Option<SessionPromptDump>,
    ...
    /// pre-multimodal form (path in the [[DCODE_FILES]] text + resource link).
    ...
    /// `PreToolUse` hook calls DCode over HTTP instead, and that request lands
    ...
    ) -> dcode_session::PermissionDecision {
        self.backend.request_external_permission(tool_name, input).await
    }

    pub fn get_confirmations(&self) -> Vec<dcode_common::Confirmation> {
        self.backend
            ...
            .map(|p| {
                ...
                dcode_common::Confirmation {
                    ...
                    options: options
                        .into_iter()
                        .map(|o| dcode_common::ConfirmationOption {
                            label: o.name,
    ...
    pub fn answer_ask(
        &self,
        request_id: &str,
        answers: Option<Vec<dcode_api_types::AskQuestionAnswer>>,
    ) -> Result<(), AgentError> {
        ...
        let answers = answers.map(|list| {
            list.into_iter()
                .map(|a| dcode_session::QuestionAnswer {
                    question: a.question,
                    labels: a.labels,
                })
                .collect::<Vec<_>>()
        ...
        tokio::spawn(async move {
            let command = dcode_session::Command::AnswerAsk {
    ...
    pub fn confirm(
        ...
        use dcode_session::PermissionDecision;
    ...
    fn effective_catalog(
        ...
        Vec<dcode_session::ModelInfo>,
        Option<String>,
        Vec<dcode_session::ModeInfo>,
        Option<String>,
    ) {
    ...
    pub async fn mode(&self) -> Result<dcode_api_types::AgentModeResponse, AgentError> {
        let caps = self.backend.capabilities();
        ...
        let current_mode = caps.current_mode.or_else(|| self.catalog_preload.current_mode.clone());
        Ok(dcode_api_types::AgentModeResponse {
    ...
    pub async fn get_model(&self) -> Result<dcode_api_types::GetModelInfoResponse, AgentError> {
        ...
        if models.is_empty() && current_model.is_none() && override_model.is_none() {
            return Ok(dcode_api_types::GetModelInfoResponse { model_info: None });
        }
        let available_models: Vec<dcode_api_types::ModelInfoEntry> = models
            .iter()
            .map(|m| dcode_api_types::ModelInfoEntry {
                id: m.id.clone(),
                label: m.name.clone(),
            })
            .collect();
        ...
        Ok(dcode_api_types::GetModelInfoResponse {
            model_info: Some(dcode_api_types::ModelInfoPayload {
    ...
    pub async fn get_config_options(&self) -> Result<dcode_api_types::GetConfigOptionsResponse, AgentError> {
        // Live catalog wins; cold-start resume falls back to the persisted-handshake
        ...
        if !modes.is_empty() {
            config_options.push(dcode_api_types::AcpConfigOptionDto {
                id: "mode".into(),
                name: Some("Mode".into()),
                label: None,
                description: None,
                ...
                options: modes
                    .iter()
                    .map(|m| dcode_api_types::AcpConfigSelectOptionDto {
                        value: m.id.clone(),
        ...
        // path's `thought_level` config option (category-keyed so DCode's
        ...
        if !efforts.is_empty() {
            config_options.push(dcode_api_types::AcpConfigOptionDto {
                ...
                options: efforts
                    .iter()
                    .map(|e| dcode_api_types::AcpConfigSelectOptionDto {
                        value: e.clone(),
                        name: Some(e.clone()),
                        label: None,
                        description: None,
        ...
        Ok(dcode_api_types::GetConfigOptionsResponse { config_options })
    }
    ...
    pub async fn set_config_option(
        ...
        option_id: &str,
        value: &str,
    ) -> Result<dcode_api_types::SetConfigOptionResponse, AgentError> {
        ...
        let deferred = option_id == "mode"
            && self.backend.capabilities().mode_switch_effect == dcode_session::ModeSwitchEffect::NextTurn;
        ...
        match option_id {
            ...
            _ => {
                return Ok(dcode_api_types::SetConfigOptionResponse {
                    confirmation: dcode_api_types::ConfigOptionConfirmation::CommandAck,
                    config_options: None,
                });
            }
        }
        ...
        let observed = snapshot
            ...
            == Some(value);
        Ok(dcode_api_types::SetConfigOptionResponse {
            ...
            confirmation: if deferred {
                dcode_api_types::ConfigOptionConfirmation::PendingNextTurn
            } else if observed {
                dcode_api_types::ConfigOptionConfirmation::Observed
            } else {
                dcode_api_types::ConfigOptionConfirmation::CommandAck
            },
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<dcode_api_types::SlashCommandItem>, AgentError> {
        let caps = self.backend.capabilities();
        Ok(caps
            .slash_commands
            .iter()
            .map(|c| dcode_api_types::SlashCommandItem {
    ...
    async fn cancel(&self) -> Result<(), AgentError> {
        ...
        self.backend
            .dispatch(Command::Cancel {
                target: dcode_session::CancelTarget::Turn,
            })
            .await
            .map(|_| ())
...
pub struct SessionBuildInputs<'a> {
    ...
    pub metadata: &'a dcode_api_types::AgentMetadata,
    ...
    /// The conversation runtime context env (`DCODE_USER_ID` /
    /// `DCODE_CONVERSATION_ID` / `DCODE_HELPER_BIN` / `DCODE_BASE_URL` /
    /// `DCODE_RUNTIME_TOKEN`, filled by `apply_conversation_runtime_context`).
...
pub(crate) fn resolved_effort(
    config: &dcode_api_types::AcpBuildExtra,
...
pub(crate) fn resolved_session_mode(
    ...
    session_snapshot: Option<&PersistedSessionState>,
    metadata: &dcode_api_types::AgentMetadata,
) -> Option<String> {
    session_snapshot
...
fn spec_mode_model(
    ...
    session_snapshot: Option<&PersistedSessionState>,
    metadata: &dcode_api_types::AgentMetadata,
) -> (dcode_session::SessionSpec, Option<String>, Option<String>) {
    use dcode_session::SessionSpec;
    ...
    // transform the ACP path applies in `initial_mode_from_params`. DCode persists
...
/// assembly (`crates/dcode-app/src/session_runtime/mod.rs`): it resolves the
...
pub async fn build_antigravity_instance(
    ...
    spawner: Arc<dyn dcode_process::Spawner>,
) -> Result<crate::agent_task::AgentInstance, AgentError> {
    use dcode_session::{AntigravityConnection, BackendConnection, McpServerSpec, SessionConfig, SessionInit};
    ...
    let init = SessionInit {
        ...
        resume: matches!(spec, dcode_session::SessionSpec::Resume { .. }),
    };
    ...
    let backend = AntigravityConnection::new(spawner)
        ...
        .await
        .map_err(|e| match e {
            dcode_session::BackendError::WorkspaceUnavailable(path) => {
    ...
    if backend_label == "claude" {
        ...
        if !provider_env.is_empty() {
            ...
            session_config.spawn_env.extend(
                provider_env
                    .into_iter()
                    .map(|(name, value)| dcode_common::EnvVar { name, value }),
            );
...
fn resolve_session_cli_program(
    ...
    // prompt behaved differently in DCode and in the user's terminal, with
    ...
    dcode_runtime::resolve_command_path(backend_label)
}
...
///     path injected these via `resolve_agent_command_spec`; `DCODE_*`/`PATH`/…
///     keys are already filtered at the registry (`is_blocked_override_env_key`),
///     so they cannot shadow the runtime context below.
///  2. the `DCODE_*` conversation runtime context (`DCODE_USER_ID` /
///     `DCODE_CONVERSATION_ID` / `DCODE_HELPER_BIN` / `DCODE_BASE_URL` /
///     `DCODE_RUNTIME_TOKEN`) — the legacy path appended these via
///     `apply_acp_launch_policy` for every agent spawn.
fn assemble_spawn_env(
    agent_env: &[dcode_api_types::AgentEnvEntry],
    runtime_env: &[(String, String)],
) -> Vec<dcode_common::EnvVar> {
    let mut env: Vec<dcode_common::EnvVar> = agent_env
        .iter()
        .map(|entry| dcode_common::EnvVar {
            name: entry.name.clone(),
            value: entry.value.clone(),
        })
        .collect();
    env.extend(runtime_env.iter().map(|(name, value)| dcode_common::EnvVar {
        name: name.clone(),
        value: value.clone(),
    }));
    env
...
fn build_session_cli_config_dump_value(backend: &str, cfg: &dcode_session::SessionConfig) -> serde_json::Value {
    use dcode_session::McpTransport;
...
fn session_server_to_spec(server: &dcode_api_types::SessionMcpServer) -> dcode_session::McpServerSpec {
    use dcode_api_types::SessionMcpTransport as T;
    use dcode_session::{McpServerSpec, McpTransport};
...
fn team_mcp_server_spec(cfg: &dcode_api_types::TeamMcpStdioConfig) -> dcode_session::McpServerSpec {
    use dcode_api_types::TeamMcpStdioConfig as C;
    dcode_session::McpServerSpec {
        name: dcode_api_types::TEAM_MCP_SERVER_NAME.to_owned(),
        transport: dcode_session::McpTransport::Stdio {
...
pub fn spawn_catalog_writeback(
    agent_id: String,
    user_id: String,
    backend: Arc<dyn dcode_session::SessionBackend>,
...
/// going empty).
fn catalog_partial_from_caps(caps: &dcode_session::Capabilities) -> Option<dcode_api_types::AgentHandshake> {
    let mut config_options = Vec::new();
    ...
    Some(dcode_api_types::AgentHandshake {
        config_options,
        available_modes,
        available_models,
        available_commands,
...
fn emit_config_options_snapshot(modes: &[ModeInfo], models: &[ModelInfo], runtime: &SessionRuntime) {
    ...
    let mut config_options: Vec<dcode_api_types::AcpConfigOptionDto> = Vec::new();
    if !modes.is_empty() {
        config_options.push(dcode_api_types::AcpConfigOptionDto {
            id: "mode".into(),
            name: Some("Mode".into()),
...
fn spawn_event_pump(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            {
                use dcode_session::{SubagentStatus, SubagentTaskKind};
                match status {
            ...
            let suppress_intermediate_finish = matches!(&env.event, SessionEvent::TurnResult { is_error, outcome, .. }
                if !workflow_inflight.is_empty()
                    && !*is_error
                    && !matches!(outcome, dcode_session::TurnOutcome::Cancelled { .. }));
                ...
                SessionEvent::TurnResult { .. } | SessionEvent::Detached { .. } if !suppress_intermediate_finish => {
                    ...
                    let keep_background = matches!(
                        ...
                        SessionEvent::TurnResult { is_error: false, outcome, .. }
                            if !matches!(outcome, dcode_session::TurnOutcome::Cancelled { .. })
                    );
                    for data in settle_workflow_cards(
                        &mut workflow_cards,
                        crate::workflow_progress::CardStatus::Cancelled,
...
/// `Orchestrator` uses (`dcode_session::is_unrecoverable_resume_error`), so a
...
fn is_dead_resume_anchor(event: &SessionEvent) -> bool {
    use dcode_session::{TurnOutcome, is_unrecoverable_resume_error};
    ...
    let reason = dcode_session::ErrorReason::Backend {
...
fn broadcast_usage_frame(bus: &dyn EventBroadcaster, conversation_id: &str, user_id: &str, event: &SessionEvent) {
    ...
    dcode_common::normalize_keys_to_snake_case(&mut event_data);
    ...
    bus.broadcast(dcode_api_types::WebSocketMessage::new("message.stream", payload));
}
...
fn empty_turn_tip(outcome: &dcode_session::TurnOutcome) -> Option<TipsEventData> {
    use dcode_session::{StopReason, TruncationKind, TurnOutcome};
...
fn translate_event(event: SessionEvent, conversation_id: &str, terminal_result_seen: bool) -> Vec<AgentStreamEvent> {
    match event {
        ...
        SessionEvent::TurnResult {
            ...
            let is_cancel = matches!(outcome, dcode_session::TurnOutcome::Cancelled { .. });
        ...
        SessionEvent::Detached { exit, redacted_summary } => {
            ...
            match dcode_session::crash_outcome(terminal_result_seen, exit) {
                dcode_session::Outcome::Crashed => {
                ...
                dcode_session::Outcome::CleanNoResult | dcode_session::Outcome::FollowResult => {
                    vec![AgentStreamEvent::Finish(FinishEventData::default())]
                }
            }
        }
        ...
        // what they are approving (DCode issue #3779); the generic
        ...
        SessionEvent::UsageDelta {
            ...
            // The renderer reads these five keys out of `_meta` (DCode
            ...
            let entries: Vec<serde_json::Value> = entries
                ...
                .map(|e| {
                    let status = match e.status {
                        dcode_session::PlanStatus::Pending => "pending",
                        dcode_session::PlanStatus::InProgress => "in_progress",
                        dcode_session::PlanStatus::Completed => "completed",
                    };
        ...
        SessionEvent::Notice {
            ...
            let tip_type = match level {
                dcode_session::NoticeLevel::Info => TipType::Info,
                dcode_session::NoticeLevel::Warning => TipType::Warning,
            };
...
mod build_mapping_tests {
    ...
    fn extra_with_thought_level(level: Option<&str>) -> dcode_api_types::AcpBuildExtra {
        dcode_api_types::AcpBuildExtra {
    ...
    fn assemble_spawn_env_orders_agent_overrides_before_runtime_context() {
        ...
        let runtime_env = vec![
            ("DCODE_USER_ID".to_owned(), "user-1".to_owned()),
            ("DCODE_RUNTIME_TOKEN".to_owned(), "tok".to_owned()),
        ];
        ...
        assert_eq!(
            names,
            ["AWS_PROFILE", "HTTPS_PROXY", "DCODE_USER_ID", "DCODE_RUNTIME_TOKEN"]
        );
    ...
    fn session_cli_config_dump_value_captures_resolved_surface() {
        use dcode_session::{McpServerSpec, McpTransport, SessionConfig, SessionInit};
        let cfg = SessionConfig {
            ...
            spawn_env: vec![dcode_common::EnvVar {
    ...
    fn test_metadata(backend: Option<&str>, yolo_id: Option<&str>) -> dcode_api_types::AgentMetadata {
        use dcode_api_types::{AgentHandshake, AgentMetadata, AgentSource, AgentSourceInfo, BehaviorPolicy};
        use dcode_common::AgentType;
        AgentMetadata {
            id: "test".into(),
            icon: None,
    ...
    fn spec_fork_quadrants() {
        let fork_cfg = AcpBuildExtra {
            fork: Some(dcode_api_types::ForkSpec {
    ...
    fn catalog_partial_projects_discovered_modes_models_commands() {
        use dcode_session::{ModeInfo, ModelInfo, SlashCommandInfo};
        let caps = dcode_session::Capabilities {
        ...
        let empty = dcode_session::Capabilities::default();
        assert!(
    ...
    fn session_server_to_spec_collapses_4_transports_to_3_and_sorts_kv() {
        use dcode_api_types::{SessionMcpServer, SessionMcpTransport};
        use dcode_session::McpTransport;
...
mod translate_tests {
    use super::*;
    use crate::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    use dcode_session::PermissionKind;
    ...
    /// (DCode `BREAKDOWN_KEYS`). A backend that reports nothing must emit NO
    ...
    fn breakdown_rides_meta_under_the_renderer_key_names() {
        let with = translate_event(
            SessionEvent::UsageDelta {
                ...
                breakdown: dcode_session::UsageBreakdown {
    ...
    fn plan_surfaces_with_snake_case_status() {
        use dcode_session::{PlanEntry, PlanStatus};
        let events = translate_event(
            SessionEvent::Plan {
                entries: vec![
                    PlanEntry {
    ...
    fn cancelled_turn_emits_finish_without_error() {
        use dcode_session::{CancelReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
                api_error_status: None,
    ...
    fn errored_turn_emits_rich_error_terminal() {
        use dcode_session::{StopReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
    ...
    fn detached_crash_surfaces_as_rich_disconnect_error() {
        use dcode_session::ExitStatusLite;
        let events = translate_event(
    ...
    fn tip_code(outcome: dcode_session::TurnOutcome) -> Option<(TipType, String)> {
...
mod persist_tests {
    ...
    use super::*;
    use dcode_db::{CreateAcpSessionParams, IAcpSessionRepository, SqliteAcpSessionRepository, init_database_memory};
    ...
    async fn seeded_repo() -> (Arc<dyn IAcpSessionRepository>, dcode_db::Database) {
    ...
    async fn usage_arriving_after_the_turn_ended_is_still_persisted() {
        ...
        persist_side_effects(
            ...
            &SessionEvent::TurnResult {
                ...
                epoch: 0,
                outcome: dcode_session::TurnOutcome::EndTurn,
            },
        )
        .await;
    ...
    fn broadcast_frame_matches_the_renderer_contract() {
        #[derive(Default)]
        struct Recorder(std::sync::Mutex<Vec<dcode_api_types::WebSocketMessage<serde_json::Value>>>);
        impl EventBroadcaster for Recorder {
            fn broadcast(&self, event: dcode_api_types::WebSocketMessage<serde_json::Value>) {
    ...
    async fn send_dispatch_session_not_found_clears_anchor_and_classifies() {
        ...
        assert_eq!(
            err.code(),
            Some(dcode_api_types::AgentErrorCode::UserAgentSessionNotFound),
    ...
    async fn set_config_option_effort_returns_observed_via_override() {
        ...
        assert!(
            matches!(resp.confirmation, dcode_api_types::ConfigOptionConfirmation::Observed),
    ...
    fn is_dead_resume_anchor_matrix() {
        use dcode_session::{CancelReason, StopReason, TurnOutcome};
...
mod pump_tests {
    ...
    // Image-capable backend: an image attachment leaves the [[DCODE_FILES]]
    ...
    async fn send_message_partitions_image_into_native_block() {
        let dir = std::env::temp_dir().join("dcode-session-media-tests");
        ...
        let marker = dcode_common::constants::DCODE_FILES_MARKER;
    ...
    async fn clean_turn_end_keeps_detached_exec_card_but_cancels_others() {
        ...
        let clean_end = SessionEvent::TurnResult {
            ...
            epoch: 0,
            outcome: TurnOutcome::Completed {
                stop_reason: dcode_session::StopReason::EndTurn,
            },
        };
    ...
    async fn cancelled_turn_closes_open_tool_calls_as_canceled_before_finish() {
        use dcode_session::{CancelReason, SubagentKind, TurnOutcome};
        let script = vec![
    ...
    async fn completed_tool_call_is_not_recanceled_at_turn_end() {
        use dcode_session::SubagentKind;
        let script = vec![
            env(SessionEvent::TurnStarted { epoch: 1 }),
            env(SessionEvent::ToolCall {
                tool_use_id: "call-1".into(),
    ...
    async fn catalog_updated_projects_config_option_with_model_and_mode() {
        use dcode_session::{ModeInfo, ModelInfo};
    ...
    async fn catalog_updated_projects_available_commands_frame() {
        use dcode_session::SlashCommandInfo;
        let script = vec![env(SessionEvent::CatalogUpdated {
            models: Vec::new(),
            modes: Vec::new(),
            slash_commands: vec![
    ...
    async fn workflow_launch_result_finish_is_suppressed_until_workflow_completes() {
        use dcode_session::SubagentStatus;
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_wf".into(),
                name: "Task".into(),
                subagent: dcode_session::SubagentKind::Workflow,
                input: serde_json::Value::Null,
                parent_tool_use_id: None,
            }),
            ...
            env(SessionEvent::SubagentUpdate {
                ...
                kind: Some(dcode_session::SubagentTaskKind::WorkflowContainer),
            }),
            env(SessionEvent::MessageDelta {
                item_id: "m".into(),
    ...
    fn wf_detail(r#ref: &str, label: &str, state: dcode_session::WorkflowLoopState) -> dcode_session::SessionEvent {
        SessionEvent::SubagentDetail {
            r#ref: r#ref.into(),
    ...
    fn wf_container(
        status: dcode_session::SubagentStatus,
        kind: Option<dcode_session::SubagentTaskKind>,
    ) -> SessionEvent {
        SessionEvent::SubagentUpdate {
            r#ref: "task-wf".into(),
            label: Some("wf".into()),
    ...
    async fn workflow_progress_streams_while_the_workflow_runs() {
        use dcode_session::{SubagentStatus, SubagentTaskKind, WorkflowLoopState};
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_wf".into(),
                name: "Workflow".into(),
    ...
    async fn background_bash_gets_a_live_card_and_settles_on_notification() {
        ...
        let script = vec![
            ...
            env(SessionEvent::ToolResult {
                tool_use_id: "toolu_bg".into(),
                is_error: false,
                content: vec![dcode_session::ToolResultContent::Text(
    ...
    async fn cancelled_turn_settles_background_cards() {
        use dcode_session::{CancelReason, SubagentStatus, SubagentTaskKind};
        let script = vec![
            env(SessionEvent::ToolCall {
                tool_use_id: "toolu_bg".into(),
                name: "Bash".into(),
            ...
            env(SessionEvent::TurnResult {
                ...
                result_text: String::new(),
                epoch: 0,
                outcome: dcode_session::TurnOutcome::Cancelled {
                    reason: CancelReason::UserCancel,
                },
            }),
        ];
    ...
    struct PendingPermBackend(dcode_session::PendingPermissionView);
    ...
    impl SessionBackend for PendingPermBackend {
        ...
        fn pending_permission_requests(&self) -> Vec<dcode_session::PendingPermissionView> {
            vec![self.0.clone()]
        }
    }
    ...
    async fn get_confirmations_recovers_pending_ask_user_question() {
        let backend: Arc<dyn SessionBackend> = Arc::new(PendingPermBackend(dcode_session::PendingPermissionView {
    ...
    impl SessionBackend for NextTurnEffectBackend {
        ...
        fn capabilities(&self) -> Capabilities {
            Capabilities {
                mode_switch_effect: dcode_session::ModeSwitchEffect::NextTurn,
                ..StaticCapsBackend.capabilities()
            }
        }
    }
    ...
    fn handshake_with_catalog() -> dcode_api_types::AgentHandshake {
        dcode_api_types::AgentHandshake {
    ...
    async fn live_capabilities_overwrite_stale_preload() {
        ...
        let stale = dcode_api_types::AgentHandshake {
            available_models: Some(serde_json::json!({
...
mod force_kill_tests {
    ...
    use dcode_common::{AgentKillReason, ConversationStatus};
    use dcode_session::{
        Admission, BackendError, Capabilities, Command, CommandReceipt, SessionBackend, SessionEnvelope,
    };
    use futures_util::stream::BoxStream;
...
mod cold_start_effort_tests {
    ...
    fn handshake_with(models: serde_json::Value) -> dcode_api_types::AgentHandshake {
        dcode_api_types::AgentHandshake {
...
mod catalog_writeback_tests {
    ...
    use super::*;
    use dcode_session::{Admission, Capabilities, CommandReceipt, ModeInfo, ModelInfo};
    use std::sync::atomic::{AtomicUsize, Ordering};
