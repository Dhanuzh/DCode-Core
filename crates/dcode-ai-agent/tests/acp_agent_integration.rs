...
use dcode_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use dcode_ai_agent::manager::acp::AcpAgentManager;
use dcode_ai_agent::registry::AgentRegistry;
use dcode_ai_agent::{AgentInstance, AgentStreamEvent, IAgentTask};
use dcode_common::ConversationStatus;
use dcode_db::{SqliteAgentMetadataRepository, init_database_memory};
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = dcode_ai_agent::AcpBuildExtra {
        agent_id: None,
    ...
    let skill_paths = std::sync::Arc::new(dcode_extension::resolve_skill_paths(
        tmp_skills.path(),
        tmp_skills.path(),
    ));
    let skill_manager = dcode_ai_agent::AcpSkillManager::new(skill_paths);
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: dcode_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), dcode_common::AgentType::Acp);
...
async fn acp_agent_kill_terminates_process() {
    ...
    agent.kill(Some(dcode_common::AgentKillReason::IdleTimeout)).unwrap();
