...
use dcode_ai_agent::{AgentRegistry, AgentService};
use dcode_api_types::{AgentManagementStatus, AgentSnapshotCheckKind, AgentSnapshotCheckStatus};
use dcode_db::{
...
async fn management_rows_derive_missing_available_and_unavailable_statuses() {
    ...
    repo.upsert(&custom_params(
        "agent-missing",
        "Missing Agent",
        "dcode-missing-agent-binary",
        r#"{"binary_name":"dcode-missing-agent-binary"}"#,
    ))
    .await
    .unwrap();
    repo.upsert(&custom_params(
...
async fn manual_health_check_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        ...
        "dcode-definitely-missing-health-check-target",
        r#"{"binary_name":"dcode-definitely-missing-health-check-target"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_enabled_toggle_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-toggle",
        "Target Toggle Agent",
        "dcode-target-toggle-command",
        r#"{"binary_name":"dcode-target-toggle-command"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_delete_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-delete",
        "Target Delete Agent",
        "dcode-target-delete-command",
        r#"{"binary_name":"dcode-target-delete-command"}"#,
    ))
    .await
    .unwrap();
