...
use dcode_ai_agent::AcpSessionSyncService;
use dcode_ai_agent::AcpSkillManager;
use dcode_ai_agent::factory::{AgentFactoryDeps, build_agent_factory};
use dcode_ai_agent::registry::AgentRegistry;
...
use dcode_api_types::AionrsBuildExtra;
use dcode_common::{AgentType, ProviderWithModel, encrypt_string};
use dcode_db::{
...
fn make_factory(
...
) -> dcode_ai_agent::task_manager::AgentFactory {
    let tmp = tempfile::TempDir::new().unwrap();
    let skill_paths = Arc::new(dcode_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    ...
    let process_registry = Arc::new(dcode_process::FileRegistryStore::new(tmp.path()));
    let session_spawner: Arc<dyn dcode_process::Spawner> = Arc::new(dcode_process::RealSpawner::new(
        process_registry,
        uuid::Uuid::now_v7(),
        dcode_process::local_machine_id(tmp.path()),
    ));
    build_agent_factory(AgentFactoryDeps {
        ...
        antigravity_hook_tokens: Arc::new(dcode_ai_agent::antigravity_hook::HookTokenRegistry::new()),
    })
}
