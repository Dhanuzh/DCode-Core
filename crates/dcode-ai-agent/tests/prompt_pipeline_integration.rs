...
use dcode_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use dcode_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use dcode_ai_agent::manager::acp::{AcpSession, SessionNewPreludeHook};
use dcode_ai_agent::registry::AgentRegistry;
use dcode_ai_agent::shared_kernel::ModelId;
use dcode_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntime};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(dcode_extension::resolve_skill_paths(tmp.path(), tmp.path()));
