...
use dcode_process::{BoxedStdin, ProcessError, Spawner};
    ...
    fn parse_control_request(&mut self, v: &Value) -> Vec<SessionEvent> {
        ...
        match (subtype, request_id.is_empty()) {
            ("can_use_tool", false) => {
                ...
                // approves blind (DCode issue #3779). This is display-to-the-approver,
...
impl BackendAdapter for ClaudeAdapter {
    async fn start_turn(
        ...
        let mut args = vec![
            ...
            // (dcode-ai-agent `claude_flags`): the CLI rejects an unknown option at
            // parse time, so passing it to an older PATH-resolved claude would kill
            // the spawn outright. This crate is self-contained (dcode-process +
            // dcode-common only) and cannot probe `--version` itself.
        ...
        let proc = spawner.spawn(spec, &[], "dcode-session").await?;
...
async fn write_ndjson_line(stdin: &mut BoxedStdin, value: &Value) -> Result<(), ProcessError> {
    ...
    bytes.push(b'\n');
    // DIAGNOSTIC (env-gated, default OFF via DCODE_CLAUDE_WIRE_DUMP): log the RAW
    ...
    if std::env::var("DCODE_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty()) {
...
mod tests {
    ...
    async fn captured_frame_with_id(content: &[crate::ContentBlock], client_msg_id: Option<&str>) -> String {
        ...
        let mut stdin: dcode_process::BoxedStdin = Box::new(CaptureStdin(buf.clone()));
        ClaudeAdapter::new()
    ...
    /// (DCode issue #3779: with input dropped the card read "命令: Bash" and the
