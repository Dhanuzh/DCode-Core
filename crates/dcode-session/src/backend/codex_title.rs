...
async fn run_exchange(io: &dyn AgentIo, req: &TitleRequest) -> Option<String> {
    ...
    write_frame(
        ...
        &json!({
            "id": 1,
            "method": "initialize",
            "params": { "clientInfo": { "name": "dcode-title", "version": "1" } },
        }),
    )
    .await?;
...
async fn write_frame(stdin: &mut dcode_process::BoxedStdin, frame: &Value) -> Option<()> {
...
impl SpawnedTitleIo {
    pub(crate) fn new(spawner: Arc<dyn dcode_process::Spawner>, command: dcode_common::CommandSpec) -> Self {
...
impl TitleIoSource for SpawnedTitleIo {
    async fn open(&self) -> Option<Box<dyn AgentIo>> {
        match self
            .spawner
            .spawn(self.command.clone(), &[], "dcode-session-title")
            .await
        {
...
mod tests {
    ...
    /// `~/dcode/protocols/samples/codex-cli/0.144.6/ephemeral_title_thread_roundtrip.jsonl`
