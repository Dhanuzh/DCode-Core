...
//!
//! None of claude / codex / agy ship with DCode: each is whatever the user
...
//! install could differ, so the same prompt behaved differently in DCode and
...
pub fn drift_notice(cli: &str, reported: &str, verified: &str) -> Option<(NoticeLevel, String, LocalizedText)> {
    ...
    match classify(reported, verified) {
        ...
        VersionVerdict::Older => Some((
            NoticeLevel::Info,
            format!(
                "The installed {cli} is older than the version DCode verified; \
                 some features may be missing. Consider upgrading {cli}. \
                 (installed {reported} / verified by DCode: {verified})"
            ),
        ...
        VersionVerdict::Newer => Some((
            NoticeLevel::Info,
            format!(
                "The installed {cli} is newer than the version DCode verified. \
...
async fn read_first_line(stdout: dcode_process::BoxedStdout) -> Option<String> {
