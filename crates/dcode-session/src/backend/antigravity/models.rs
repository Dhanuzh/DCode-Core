...
async fn read_to_end(stdout: dcode_process::BoxedStdout) -> String {
...
";
    ...
    async fn probing_closes_stdin_so_agy_can_exit() {
        ...
        let spawner: Arc<dyn Spawner> = Arc::new(dcode_process::RealSpawner::new(
            Arc::new(dcode_process::FileRegistryStore::new(tmp.path())),
            uuid::Uuid::now_v7(),
            "test-machine",
        ));
