...
/// (dcode-agent-rest/src/protocol/acp.rs:55, an `oneshot` + `tokio::time::timeout`)
...
/// Restore the legacy 30s budget (env-overridable via `DCODE_HANDSHAKE_TIMEOUT_SECS`
...
pub(crate) fn handshake_budget() -> std::time::Duration {
    let secs = std::env::var("DCODE_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
pub trait SessionBackend: Send + Sync {
    ...
    /// headless mode, so DCode registers itself as agy's `PreToolUse` hook;
...
/// its `McpServerConfig` map). Deliberately crate-local — `dcode-session` stays
/// self-contained (no `dcode-api-types`/ACP-SDK dep, §Cargo.toml). The app
...
pub struct SessionInit {
    ...
    /// blob). Kept as `serde_json::Value` so `dcode-session` need not depend on
...
pub struct SessionConfig {
    ...
    pub spawn_env: Vec<dcode_common::EnvVar>,
    /// Antigravity only: the `.agents/hooks.json` body that registers DCode as
    /// agy's approval gate.
    ///
    ...
    /// itself (it knows the workspace, not where DCode's binary lives). Holding
