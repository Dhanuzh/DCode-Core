...
use dcode_session::testing::CountingTerminateIo;
use dcode_session::{AcpSessionBackend, ClaudeSessionBackend, CodexSessionBackend, SessionBackend};
...
/// covered in `dcode-process`; it cannot be unit-constructed here without a
/// real OS process).
#[tokio::test]
async fn fake_agent_io_terminate_is_default_noop() {
    use dcode_session::AgentIo;
    let io = dcode_session::testing::FakeAgentIo::never_exits(Vec::new());
