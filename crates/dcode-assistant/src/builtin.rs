...
//! still can, via the `DCODE_BUILTIN_ASSISTANTS_PATH` env var → disk
//! fallback path.
...
/// `crates/dcode-app/assets/builtin-assistants/`.
static BUILTIN_ASSETS: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../dcode-app/assets/builtin-assistants");
...
/// `DCODE_BUILTIN_ASSISTANTS_PATH` at a fixture directory.
enum Source {
    Embedded,
    Disk(PathBuf),
}
...
impl BuiltinAssistantRegistry {
    ...
    ///
    /// If `DCODE_BUILTIN_ASSISTANTS_PATH` is set and points to a readable
    /// directory, read from disk (test-only override). Otherwise use the
    /// assets embedded at compile time.
    pub fn load() -> Self {
        if let Ok(env) = std::env::var("DCODE_BUILTIN_ASSISTANTS_PATH") {
            let p = PathBuf::from(env);
            if p.exists() {
                return Self::load_from_dir(p);
            }
            warn!(
                "DCODE_BUILTIN_ASSISTANTS_PATH points to missing directory; \
    ...
    /// `DCODE_BUILTIN_ASSISTANTS_PATH` E2E fixtures — the three
...
mod tests {
    ...
    // DCODE_BUILTIN_ASSISTANTS_PATH). Graceful-degradation semantics must
    // stay intact.
    // -----------------------------------------------------------------------
    ...
    fn load_respects_env_var_disk_override() {
        ...
        let key = "DCODE_BUILTIN_ASSISTANTS_PATH";
