...
use dcode_common::constants::{CSRF_COOKIE_NAME, CSRF_HEADER_NAME};
...
///   matches the `dcode-csrf-token` cookie.
...
pub async fn csrf_middleware(
    ...
    // agy's PreToolUse callback. A local `dcode-core antigravity-hook` process
    ...
    // per-conversation token instead (`x-dcode-hook-token`) — but the CSRF
