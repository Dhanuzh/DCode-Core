...
use dcode_common::constants::COOKIE_MAX_AGE_DAYS;
use dcode_db::{DbError, IUserRepository, UserStatus, UserType, models::User};
...
pub struct AuthRouterState {
    ...
    /// Optional on-disk adoption side-effect (DCode → AionPro upgrade).
...
async fn refresh_handler(
    ...
    if state.aionpro_mode && user.user_type != dcode_db::UserType::Aionpro {
        return Err(user_context_required());
    }
