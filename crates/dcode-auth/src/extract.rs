...
use dcode_common::constants::COOKIE_NAME;
...
///
/// Priority: `Authorization: Bearer <token>` > `dcode-session` cookie.
...
mod tests {
    ...
    fn token_from_cookie() {
        let headers = headers_with(&[("cookie", "dcode-session=cookie_token; other=val")]);
    ...
    fn token_header_takes_priority_over_cookie() {
        let headers = headers_with(&[
            ...
            ("cookie", "dcode-session=cookie_token"),
        ]);
    ...
    fn ws_token_from_cookie() {
        let headers = headers_with(&[("cookie", "dcode-session=ws_cookie")]);
    ...
    fn cookie_value_malformed_before_target() {
        ...
        let headers = headers_with(&[("cookie", "bad_entry; dcode-session=tok123")]);
        assert_eq!(extract_cookie_value(&headers, "dcode-session"), Some("tok123".into()));
    }
    ...
    fn token_from_cookie_with_malformed_entries() {
        ...
        let headers = headers_with(&[("cookie", "garbage; dcode-session=abc; nope")]);
