...
use dcode_db::{IUserRepository, UserStatus, UserType};
...
/// and injected into agent subprocess environments as `DCODE_RUNTIME_TOKEN`.
pub const RUNTIME_TOKEN_HEADER: &str = "x-dcode-runtime-token";
/// Header carrying the acting user id asserted by the helper CLI.
pub const RUNTIME_USER_ID_HEADER: &str = "x-dcode-user-id";
/// Header carrying the conversation id the helper CLI runs inside.
pub const RUNTIME_CONVERSATION_ID_HEADER: &str = "x-dcode-conversation-id";
...
/// service; `dcode-auth` must not depend on `dcode-ai-agent` directly.
...
pub struct AuthState {
    ...
    /// (`dcode-core config` / `diagnose`), which cannot carry a JWT or cookies.
...
///
/// Flow:
/// 1. Extract bearer token from `Authorization` header or `dcode-session` cookie
