...
use dcode_db::{ExternalUserProjection, IUserRepository, UserStatus, UserType, models::User};
...
pub enum ProvisionError {
    ...
    Db(#[from] dcode_db::DbError),
    #[error("token signing failed: {0}")]
    Token(#[from] crate::AuthError),
}

/// Port for the on-disk side of DCode → AionPro adoption. When DB adoption
...
/// in the composition layer over the skill filesystem so `dcode-auth` stays
...
impl AuthProvisionService {
    ...
    pub async fn ensure_external_user(
        ...
        // Upgrade path (DCode -> AionPro over the same database): while this
