...
use dcode_extension::hub::{HubIndexManager, HubInstaller};
use dcode_extension::registry::ExtensionRegistry;
use dcode_extension::state::ExtensionStateStore;
use dcode_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![dcode_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: dcode_extension::types::ExtensionSource::Env,
    }];
