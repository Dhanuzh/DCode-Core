...
use dcode_api_types::{ApiResponse, AssistantSource, SkillImportLimitsResponse};
use dcode_auth::CurrentUser;
use dcode_db::{UserStatus, UserType};
use dcode_extension::classifier::{AssistantClassifier, AssistantRuleDispatcher};
use dcode_extension::error::ExtensionError;
use dcode_extension::external_paths::ExternalPathsManager;
use dcode_extension::skill_routes::{SkillRouterState, skill_routes};
use dcode_extension::skill_service::SkillPaths;
use axum::Extension;
use axum::body::Body;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let skill_repo = Arc::new(dcode_db::SqliteSkillRepository::new(db.pool().clone()));
    std::mem::forget(tmp);
