//! Filesystem side of DCode → AionPro adoption.
//!
...
pub async fn adopt_user_filesystem(
    ...
    let Ok(adopter_dir) = dcode_common::user_dir_name(adopter_user_id) else {
...
async fn rewrite_adopted_skill_paths(
    ...
    for row in rows {
        ...
        let params = dcode_db::UpsertSkillParams {
            name: &row.name,
...
mod tests {
    use super::*;
    use dcode_db::{SqliteSkillRepository, UpsertSkillParams, init_database_memory};
    ...
    async fn seed_user(db: &dcode_db::Database) -> String {
        use dcode_db::IUserRepository;
        dcode_db::SqliteUserRepository::new(db.pool().clone())
