...
//!
//! These traits live in `dcode-extension` (not `dcode-assistant`) so
//! `skill_routes` can depend on them without pulling `dcode-assistant` into
//! the dependency graph; the concrete implementation ships from
//! `dcode-assistant::AssistantService`.

use dcode_api_types::AssistantSource;
...
///
/// Implemented by `dcode_assistant::AssistantService`; depended on by
