...
use dcode_api_types::{
    BedrockConfig, FetchModelsAnonymousRequest, FetchModelsRequest, FetchModelsResponse, VertexConfig,
};
use dcode_common::decrypt_string;
use dcode_db::IProviderRepository;
...
/// Platforms whose credentials live in a dedicated config struct rather than
/// `api_key`, so an empty `api_key` is legitimate.
fn uses_external_credentials(platform: &str) -> bool {
    matches!(platform, "bedrock" | "vertex-ai")
}
...
impl ModelFetchService {
    ...
    pub async fn fetch_models_anonymous(
        ...
        let config = FetchConfig {
            ...
            vertex_config: req.vertex_config.clone(),
        };
        self.fetch_with_config(&config, req.try_fix).await
    }
    ...
    async fn load_provider_config(&self, user_id: &str, provider_id: &str) -> Result<FetchConfig, SystemError> {
        ...
        if api_key.trim().is_empty() && !uses_external_credentials(&row.platform) {
        ...
        let vertex_config: Option<VertexConfig> = deserialize_opt(&row.vertex_config, "vertex_config")?;
...
fn validate_anonymous_request(req: &FetchModelsAnonymousRequest) -> Result<(), SystemError> {
    ...
    // Bedrock and Vertex are addressed by account/project identity rather than a
    // user-supplied endpoint, so their forms omit baseUrl entirely.
    if !uses_external_credentials(&req.platform) && req.base_url.trim().is_empty() {
        return Err(SystemError::BadRequest("baseUrl is required".into()));
    }
    // Bedrock and Vertex carry credentials in their own config structs, so an
    // empty api_key is legitimate for those platforms.
    if !uses_external_credentials(&req.platform) && req.api_key.trim().is_empty() {
...
mod tests {
    use super::*;
    use dcode_common::encrypt_string;
    use dcode_db::{CreateProviderParams, SqliteProviderRepository, init_database_memory};

    const TEST_KEY: [u8; 32] = [0x42; 32];
    const TEST_USER_ID: &str = "user-1";

    async fn setup() -> (ModelFetchService, dcode_db::Database) {
    ...
    async fn create_provider(db: &dcode_db::Database, platform: &str, base_url: &str, api_key: &str) -> String {
    ...
    #[tokio::test]
    async fn fetch_models_vertex_ai_without_config_is_rejected() {
        // Vertex no longer returns a hardcoded list: model availability is
        // per-project, so discovery requires an explicit vertex_config.
        let (svc, db) = setup().await;
        let id = create_provider(&db, "vertex-ai", "https://unused", "fake-key").await;
        let req = FetchModelsRequest { try_fix: false };
        let err = svc.fetch_models(TEST_USER_ID, &id, &req).await.unwrap_err();
        assert!(matches!(err, SystemError::BadRequest(_)));
    }
