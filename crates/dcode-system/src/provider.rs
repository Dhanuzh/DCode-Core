...
use dcode_api_types::{CreateProviderRequest, ProviderResponse, UpdateProviderRequest};
use dcode_common::{decrypt_string, encrypt_string};
use dcode_db::{CreateProviderParams, IProviderRepository, UpdateProviderParams, models::Provider};
...
impl ProviderService {
    ...
    pub async fn create(&self, user_id: &str, req: CreateProviderRequest) -> Result<ProviderResponse, SystemError> {
        ...
        let vertex_json = serialize_opt(&req.vertex_config, "vertex_config")?;
        ...
        let params = CreateProviderParams {
            ...
            vertex_config: vertex_json.as_deref(),
    ...
    pub async fn update(
        ...
        let params = UpdateProviderParams {
            ...
            vertex_config: vertex_json.as_ref().map(|s| Some(s.as_str())),
            is_full_url: req.is_full_url,
        };
    ...
    fn row_to_response(&self, row: Provider) -> Result<ProviderResponse, SystemError> {
        ...
        let vertex_config = deserialize_opt(&row.vertex_config, "vertex_config")?;
...
fn validate_create_request(req: &CreateProviderRequest) -> Result<(), SystemError> {
    ...
    } else if req.platform == "vertex-ai" {
        // Vertex is identified by project + location and authenticates via a
        // service account or ADC, so baseUrl and apiKey may be empty.
        let cfg = req
            .vertex_config
            .as_ref()
            .ok_or_else(|| SystemError::BadRequest("vertexConfig is required for vertex-ai platform".into()))?;
        if cfg.project_id.trim().is_empty() {
            return Err(SystemError::BadRequest("vertexConfig.projectId is required".into()));
        }
        if cfg.location.trim().is_empty() {
            return Err(SystemError::BadRequest("vertexConfig.location is required".into()));
        }
        if !req.base_url.trim().is_empty() {
            validate_base_url(&req.base_url)?;
        }
...
mod tests {
    ...
    use dcode_db::{SqliteProviderRepository, init_database_memory};
    ...
    fn validate_create_bedrock_allows_empty_base_url_and_api_key() {
        let req = CreateProviderRequest {
            ...
            base_url: "".into(),
            api_key: "".into(),
            bedrock_config: Some(dcode_api_types::BedrockConfig {
                auth_method: dcode_api_types::BedrockAuthMethod::Profile,
