use dcode_api_types::SystemInfoResponse;
...
/// Resolve the cache directory for DCode.
///
/// Priority: `DCODE_CACHE_DIR` env → `dirs::cache_dir()/dcode`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("DCODE_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("dcode").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for DCode.
///
/// Priority: `DCODE_WORK_DIR` env → `dirs::data_dir()/dcode`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("DCODE_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
/// Resolve the log directory for DCode.
///
/// Priority: `DCODE_LOG_DIR` env →
///   macOS: `~/Library/Logs/dcode`
///   Linux: `dirs::state_dir()/dcode/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/dcode/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("DCODE_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/dcode").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("dcode/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/dcode/logs
    dirs::data_dir()
        .map(|p| p.join("dcode/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "dcode".
        let dir = resolve_cache_dir();
        assert!(dir.contains("dcode"), "cache_dir should contain 'dcode': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("dcode"), "work_dir should contain 'dcode': {dir}");
    }
    ...
    fn test_env_override_log_dir() {
        ...
        assert!(
            dir.to_ascii_lowercase().contains("dcode"),
            "log_dir should contain 'dcode' (case-insensitive): {dir}"
        );
    }
}
