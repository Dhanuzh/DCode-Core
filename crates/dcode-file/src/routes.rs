...
use dcode_common::ApiError;
use dcode_common::constants::UPLOAD_MAX_SIZE;
...
pub struct FileRouterState {
    ...
    pub project: Arc<dcode_project::ProjectService>,
...
pub fn file_routes(state: FileRouterState) -> Router {
    ...
    // We first disable the global `DefaultBodyLimit` that `dcode-app`
...
/// project service (`resolve_reference` is covered in `dcode-project`).
...
/// the project service (`resolve_reference` is covered in `dcode-project`).
...
/// fixed.
fn chat_file_resolve_error(err: dcode_project::ProjectError) -> ApiError {
    let code = err.code();
    tracing::warn!(target: "chat_file", error = %err, code, "could not resolve chat file reference");
    match err {
        dcode_project::ProjectError::Database(_) => ApiError::Internal("failed to resolve target".to_owned()),
        _ => ApiError::coded(
            axum::http::StatusCode::NOT_FOUND,
            "FILE_NOT_FOUND",
            "The requested file no longer exists.",
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let mode = match info.mode {
        SnapshotMode::GitRepo => dcode_api_types::SnapshotMode::GitRepo,
        SnapshotMode::Snapshot => dcode_api_types::SnapshotMode::Snapshot,
    };
...
mod tests {
    ...
    fn chat_file_resolve_error_is_path_free_for_path_bearing_variants() {
        ...
        let variants = [
            dcode_project::ProjectError::ChatFileMissing {
                path: secret.to_owned(),
            },
            dcode_project::ProjectError::LocalPathNotReadable {
                path: secret.to_owned(),
            },
            dcode_project::ProjectError::UploadPathOutsideRoot {
                path: secret.to_owned(),
            },
        ];
    ...
    /// not covered here: constructing a `DbError` would mean adding `dcode-db` as a
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, dcode_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, dcode_api_types::SnapshotMode::Snapshot);
        assert!(r.branch.is_none());
    }

    #[test]
    fn compare_response_conversion() {
        use dcode_common::FileChangeOperation;
