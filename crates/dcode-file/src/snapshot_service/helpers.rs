...
pub(super) const SNAPSHOT_DIR_PREFIX: &str = "dcode-snapshot-";
...
const SNAPSHOT_SIG_NAME: &str = "dcode";
/// Signature email used for snapshot commits.
const SNAPSHOT_SIG_EMAIL: &str = "snapshot@dcode.local";
...
pub(super) struct WorkspaceState {
    ...
    /// - snapshot mode: `/tmp/dcode-snapshot-{hash}` (bare-style git dir).
