...
**Option B: Start DCode WebUI Remote Mode**

- **Experience**: Access through browser with DCode's full interface features
...
- **Configuration requirements**: Start DCode WebUI service, access through browser
...
5. **Post-installation configuration guidance** (IMPORTANT):
   ...
   - **Usage guidance**:
     - **Local usage**: "After OpenClaw installation is complete, **please restart DCode**, then you can see OpenClaw in the available Agent list on the DCode homepage and start chatting directly."
...
1. **Ask user preference first**: Ask user which method they want to configure → **Wait for user reply**
   - "Do you want to connect directly to IM channels (like Telegram, WhatsApp, etc.), or use DCode WebUI remote mode?"
...
4. **Option B: Start DCode WebUI Remote Mode**
   ...
   - **Workflow**:
     ...
     2. After user replies, **guide user to DCode settings interface**:
        - **Open settings interface**: Clearly tell user how to open it
          - "Please click the **Settings icon** (gear icon) at the bottom left of DCode"
   ...
   - **Key principles**:
     ...
     - **Don't attempt to install `@aionui/webui` or similar npm packages**: WebUI is a built-in feature of DCode, not a separate package
...
5. **Post-installation configuration guidance**:
   ...
   - **Usage guidance**:
     - Introduce local usage (return to DCode homepage)
