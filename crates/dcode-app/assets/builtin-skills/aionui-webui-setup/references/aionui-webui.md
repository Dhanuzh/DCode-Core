...
DCode 支持 WebUI 模式，允许通过浏览器访问应用。这对于远程使用 OpenClaw 非常有用。DCode 提供三种远程连接方式，满足不同场景的需求。

**重要**：WebUI 配置应通过 DCode 设置界面完成，无需使用命令行。本指南将引导你如何在设置界面中完成配置。
...
1. 在 DCode 主界面，点击左下角的**设置图标**（齿轮图标）
2. 在设置菜单中，点击 **"WebUI"** 选项
3. 进入 WebUI 配置界面

**方式 2：通过快捷键**

- 在 DCode 主界面，使用快捷键打开设置（具体快捷键请查看 DCode 帮助文档）
...
#### Step 1: 在 DCode 电脑上配置 WebUI

1. **打开 WebUI 设置界面**：
   - 在 DCode 主界面，点击左下角的**设置图标**
   - 点击 **"WebUI"** 选项
...
#### Step 2: 在 DCode 电脑上安装并登录 Tailscale
...
#### Step 1: 在服务器上安装 DCode

按照 DCode 安装指南在服务器上安装 DCode 应用。

#### Step 2: 通过设置界面配置 WebUI

1. **打开 WebUI 设置界面**：
   - 如果服务器有图形界面，直接打开 DCode 应用
...
如果需要 DCode 开机自启，可以配置 systemd 服务。但建议通过 DCode 设置界面管理 WebUI，而不是通过命令行。
...
按照 DCode 安装指南在 macOS 服务器上安装 DCode 应用。
...
1. **打开设置**：点击 DCode 左下角的**设置图标** → 点击 **"WebUI"**
