...
description: >-
  Diagnose a running DCode installation: inspect stuck or errored conversations, read provider health, scheduled task state, MCP server health, team member state, backend health, and dcode-core logs. Use when the user reports DCode is misbehaving, a conversation is stuck, an LLM/provider call is failing, a scheduled task did not run, an MCP server has no tools, a team member is hung, or they ask to troubleshoot DCode.
---

# DCode Troubleshooting

Use the bundled `dcode-core diagnose` CLI for read-only troubleshooting. It uses
...
4. Treat every command as read-only. To change DCode configuration, use the
...
For a vague "DCode is broken" report, run:
