...
name: aionui-config
description: >-
  Configure DCode itself through the bundled dcode-core config CLI: create and edit assistants, update assistant rules, inspect and import skills, manage MCP servers, configure model providers, update settings, manage agents, configure scheduled tasks, and manage app configuration from an agent conversation. Use when the user wants you to set up or modify a DCode assistant, attach skills, change an assistant's system prompt, add MCP or model provider configuration, schedule recurring work, or otherwise configure their DCode installation, including when the user needs to know whether assistant changes affect the current conversation or only new conversations.
---

# DCode Config

Configure DCode with the bundled agent-facing CLI. Do not discover ports, do
...
Ask dcode-core what this version supports:
...
DCode persists assistant configuration immediately, but running conversations
