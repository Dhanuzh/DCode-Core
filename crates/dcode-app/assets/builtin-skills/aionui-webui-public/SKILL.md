...
description: >-
  Expose the user's local DCode WebUI to the public internet with a near-zero-effort flow. Detects whether the WebUI is running, guides the user to switch it on if needed (the only manual step), self-installs cloudflared cross-platform, opens a Cloudflare quick tunnel, verifies the public URL actually works, then explains the limitations honestly (temporary/random URL, must stay running, password is the only protection, traffic transits Cloudflare). Use whenever the user wants to reach their DCode from outside the LAN, over the internet, or share a public link. Distinct from aionui-webui-setup (which covers manual LAN / Tailscale / server config through the settings UI): this skill produces a one-click public link via an automatic tunnel.
---
...
# DCode WebUI Public Access Guide

You help a user turn their local DCode WebUI (LAN-only at best) into a public internet URL, with the user doing almost nothing. You have a shell (Bash) and run on the same machine as DCode, so you do the work yourself. The user only does what you architecturally cannot: flip the WebUI toggle in the desktop UI.

## Core facts (verified, do not re-derive)

- DCode WebUI is a local HTTP server on port 25808 (the dcode-core default; override with the `--port` CLI flag — so don't hardcode it, confirm with the curl probe below). It has built-in user+password / JWT auth, which is the *only* thing protecting it once it's on the public internet — so the password is load-bearing, not a formality (see Step 6 risks).
...
```bash
mkdir -p ~/.dcode/tools && cd ~/.dcode/tools
...
(Use the path to the binary you installed, e.g. `~/.dcode/tools/cloudflared`.)
...
between the open internet and the user's DCode. **Before** you give them the
...
> These `/api/webui/*` credential endpoints only work when dcode-core runs in
> **local mode** (the normal desktop/Electron case, which is exactly the scenario
> this skill targets). If dcode-core is deployed as a standalone server, they
