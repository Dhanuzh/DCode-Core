...
use dcode_ai_agent::protocol::events::TextEventData;
use dcode_ai_agent::types::{BuildTaskOptions, SendMessageData};
use dcode_ai_agent::{AgentError, AgentStreamEvent, IWorkerTaskManager};
use dcode_api_types::AgentSource;
use dcode_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs, now_ms};
use dcode_db::UpsertAgentMetadataParams;
...
async fn build_app_with_mock_tasks() -> (axum::Router, dcode_app::AppServices, Arc<MockTaskManager>) {
    let db = dcode_db::init_database_memory().await.unwrap();
    let services = dcode_app::AppServices::from_config(db, &dcode_app::AppConfig::default())
        .await
        .unwrap();
    ...
    let router = dcode_app::create_router(&services).await.expect("build router");
...
async fn upsert_visible_agent_metadata(services: &dcode_app::AppServices, id: &str, agent_type: &str) {
    services
        .agent_registry
        .repo_handle()
...
async fn agent_logos_endpoint_returns_backend_to_logo_catalog() {
    ...
    // DCode CLI has no vendor `backend` (NULL); it must still be keyed by its
