...
//! mirroring `dcode-shell/tests/stt_stream_deepgram_integration.rs`).
//!
...
async fn seed_stt_prefs(app: &TestApp, config: Value) {
    let repo = Arc::new(dcode_db::SqliteClientPreferenceRepository::new(
        app.services.database.pool().clone(),
    ));
    let service = dcode_system::ClientPrefService::new(repo);
    let mut req = dcode_api_types::UpdateClientPreferencesRequest::new();
...
// ---------------------------------------------------------------------------
// Mock Deepgram live server (minimal copy of the dcode-shell integration
