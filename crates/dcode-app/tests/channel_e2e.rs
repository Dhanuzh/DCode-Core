...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn dcode_db::IChannelRepository> =
        std::sync::Arc::new(dcode_db::SqliteChannelRepository::new(pool));
    let pairing_svc = dcode_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
