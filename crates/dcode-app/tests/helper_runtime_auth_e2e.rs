...
use dcode_ai_agent::{RuntimeTokenScope, TEAM_RUNTIME_TOKEN_SESSION_GENERATION};

const CONVERSATION_ID: &str = "conv-helper-auth";

async fn build_aionpro_app() -> (axum::Router, dcode_app::AppServices, String) {
    ...
    let services = dcode_app::AppServices::from_config(db, &config).await.unwrap();
    ...
    let resp = app
        .oneshot(
            Request::builder()
                ...
                .header("x-dcode-user-id", &user_id)
                .header("x-dcode-conversation-id", CONVERSATION_ID)
                .header("x-dcode-runtime-token", &issue.token)
                .body(Body::from("{}"))
                .unwrap(),
        )
        .await
