...
//! against a real `dcode_app::create_router_with_states` instance backed by
...
use dcode_app::{AppConfig, AppServices, ModuleStates, build_module_states, create_router_with_states};
use dcode_assistant::{AssistantAgentCatalogPort, AssistantRouterState, AssistantService, BuiltinAssistantRegistry};
...
impl AssistantAgentCatalogPort for TestAgentCatalog {
    async fn list_management_agents(
        &self,
        _user_id: &str,
    ) -> Result<Vec<AgentManagementRow>, dcode_assistant::AssistantError> {
        Ok(self.rows.clone())
    }
}
...
async fn fixture() -> Fixture {
    ...
    // `build_module_states` point at `~/.dcode/` for the state store and
    ...
    // `~/.dcode/` for user data — neither is appropriate for tests.
    ...
    provider_repo
        .create(dcode_db::CreateProviderParams {
        ...
        dcode_assistant::service::AssistantServiceDeps {
            ...
            agent_catalog: Some(Arc::new(TestAgentCatalog {
                rows: vec![
                    ...
                    test_agent_row("632f31d2", None, AgentType::Aionrs, "DCode CLI"),
                ],
            })),
        },
        user_data_dir.clone(),
...
async fn rule_and_skill_files_are_isolated_by_current_user() {
    ...
    assert_eq!(
        std::fs::read_to_string(
            rule_root
                .join("users")
                .join(dcode_common::user_dir_name(&user_b.id).unwrap())
                .join("shared-assistant.md")
        )
        .unwrap(),
        "rule-b"
...
async fn avatars_of_two_users_are_physically_isolated() {
    ...
    let dir_b = dcode_common::user_dir_name(&user_b.id).unwrap();
