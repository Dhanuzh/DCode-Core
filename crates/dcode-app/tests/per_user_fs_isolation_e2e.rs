...
//! DCode → AionPro adoption moves the default user's files to the adopter.
//!
...
use dcode_extension::skill_service::{
...
async fn aionpro_services(data_dir: &Path) -> dcode_app::AppServices {
    let db = dcode_db::init_database_memory().await.unwrap();
    let config = dcode_app::AppConfig {
        identity_mode: dcode_app::IdentityMode::AionPro,
        bootstrap_secret: Some("bootstrap-secret".to_string()),
        data_dir: data_dir.to_path_buf(),
        work_dir: data_dir.to_path_buf(),
        ..Default::default()
    };
    dcode_app::AppServices::from_config(db, &config).await.unwrap()
}

async fn seed_user(services: &dcode_app::AppServices, username: &str) -> String {
    use dcode_db::IUserRepository;
    dcode_db::SqliteUserRepository::new(services.database.pool().clone())
...
async fn same_named_skill_is_physically_isolated_per_user() {
    ...
    let dir_a = dcode_common::user_dir_name(&user_a).unwrap();
    let dir_b = dcode_common::user_dir_name(&user_b).unwrap();
...
async fn provisioning_first_aionpro_user_adopts_default_user_files_on_disk() {
    let data = tempfile::tempdir().unwrap();
    let services = aionpro_services(data.path()).await;
    let (app, _runtime) = dcode_app::create_router_with_runtime(&services).await.unwrap();
    ...
    let adopter_dir = dcode_common::user_dir_name(adopter).unwrap();
