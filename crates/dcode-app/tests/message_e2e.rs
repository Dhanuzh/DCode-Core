...
async fn insert_message(
    services: &dcode_app::AppServices,
    conv_id: &str,
    msg_id: &str,
    content: &str,
    created_at: i64,
) {
    let repo = dcode_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(conv_id).await.unwrap().unwrap();
    let msg = dcode_db::models::MessageRow {
        id: msg_id.into(),
    ...
    dcode_db::IConversationRepository::insert_message(&repo, &user_id, &msg)
        .await
        .unwrap();
}

async fn update_conversation_workspace(services: &dcode_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = dcode_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(conv_id).await.unwrap().unwrap();
...
async fn upsert_artifact(services: &dcode_app::AppServices, artifact: dcode_db::ConversationArtifactRow) {
    let repo = dcode_db::SqliteConversationRepository::new(services.database.pool().clone());
    let user_id = repo.owner_user_id(&artifact.conversation_id).await.unwrap().unwrap();
    dcode_db::IConversationRepository::upsert_artifact(&repo, &user_id, &artifact)
        .await
        .unwrap();
}
