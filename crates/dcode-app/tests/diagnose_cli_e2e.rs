...
async fn diagnose_health_reads_backend_health_from_runtime_base_url() {
    ...
    let output = diagnose_command()
        ...
        .env("DCODE_CONVERSATION_ID", "conv-health")
        .env("DCODE_USER_ID", "user-health")
        .output()
        .await
        .unwrap();
...
async fn diagnose_http_get_is_get_only_redacted_escape_hatch_for_api_paths() {
    ...
    let mut child = diagnose_command()
        .args(["http", "get"])
        .env("DCODE_BASE_URL", &base_url)
        .env("DCODE_CONVERSATION_ID", "conv-http")
        .env("DCODE_USER_ID", "user-http")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn diagnose_overview_aggregates_common_failure_signals() {
    ...
    let output = diagnose_command()
        .arg("overview")
        .env("DCODE_BASE_URL", &base_url)
        .env("DCODE_CONVERSATION_ID", "conv-overview")
        .env("DCODE_USER_ID", "user-overview")
        .output()
        .await
        .unwrap();
...
fn builtin_troubleshooting_skill_uses_diagnose_cli_not_python_helper() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("assets/builtin-skills/dcode-troubleshooting");
    let skill = std::fs::read_to_string(root.join("SKILL.md")).unwrap();

    for forbidden in ["python3", "aion_diag.py", "lsof", "ps -", "curl"] {
        assert!(
            !skill.contains(forbidden),
            "dcode-troubleshooting skill must not mention {forbidden}"
        );
    }
    assert!(skill.contains("\"$DCODE_HELPER_BIN\" diagnose capabilities"));
    assert!(skill.contains("\"$DCODE_HELPER_BIN\" diagnose overview"));
    assert!(skill.contains("\"$DCODE_HELPER_BIN\" diagnose conversations get"));
    assert!(skill.contains("\"$DCODE_HELPER_BIN\" diagnose http get"));
}
