...
async fn user_id_for_username(services: &dcode_app::AppServices, username: &str) -> String {
    services
        .user_repo
...
async fn management_list_includes_missing_custom_agents() {
    ...
    repo.upsert_for_user(
        ...
        &UpsertAgentMetadataParams {
            ...
            agent_type: "acp",
            agent_source: "custom",
            agent_source_info: Some(r#"{"binary_name":"dcode-missing-agent-binary"}"#),
            enabled: true,
            command: Some("dcode-missing-agent-binary"),
            args: Some("[]"),
            env: Some("[]"),
