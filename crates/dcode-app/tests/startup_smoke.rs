...
async fn second_start_with_same_version_is_noop() {
    ...
    let first =
        dcode_extension::materialize_if_needed(data_dir, dcode_extension::builtin_skills_corpus(), "test-1.0.0")
            .await
            .unwrap();
...
async fn version_bump_triggers_rewrite() {
    ...
    let second =
        dcode_extension::materialize_if_needed(data_dir, dcode_extension::builtin_skills_corpus(), "test-2.0.0")
            .await
            .unwrap();
