...
//! `dcode_app::create_router_with_states` against an in-memory DB.

mod common;

use std::sync::Arc;

use dcode_app::{ModuleStates, build_module_states, create_router_with_states};
use dcode_db::init_database_memory;
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("DCODE_BUILTIN_SKILLS_PATH");
    }
    ...
    dcode_extension::materialize_if_needed(&data_dir, dcode_extension::builtin_skills_corpus(), "test-fixture")
        .await
    ...
    // `build_module_states` builds a state pointing at `~/.dcode/`,
    ...
    let skill_repo = Arc::new(dcode_db::SqliteSkillRepository::new(services.database.pool().clone()));
    ...
    dcode_extension::sync_skill_catalog_into_repo(&states.skill.skill_paths, skill_repo.as_ref())
        .await
...
async fn unified_skill_list_includes_auto_inject_entries_from_embedded_corpus() {
    ...
    assert!(
        names.contains(&"dcode-config"),
        "dcode-config should be shipped as an auto-inject builtin skill: {names:?}",
    );
    assert!(
        !names.contains(&"dcode-skills"),
        "dcode-skills should not be shipped as an auto-inject builtin skill: {names:?}",
    );
