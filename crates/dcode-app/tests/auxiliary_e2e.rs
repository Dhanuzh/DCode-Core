...
async fn build_app() -> (axum::Router, dcode_app::AppServices) {
...
async fn workspace_browse_treats_symlinked_skill_dir_as_directory() {
    ...
    let builtin = tmp.path().join("builtin-skills/auto-inject/dcode-skills");
    std::fs::create_dir_all(workspace.join(".claude/skills")).unwrap();
    std::fs::create_dir_all(&builtin).unwrap();
    std::fs::write(builtin.join("SKILL.md"), b"---\ndescription: test\n---\nbody").unwrap();
    std::os::unix::fs::symlink(&builtin, workspace.join(".claude/skills/dcode-skills")).unwrap();
    ...
    assert!(
        entries
            .iter()
            .any(|entry| entry["name"] == "dcode-skills" && entry["type"] == "directory"),
        "symlinked skill dir should stay visible as directory: {entries:?}"
    );

    let req = get_with_token(
        &format!("/api/conversations/{conv_id}/workspace?path=/.claude/skills/dcode-skills"),
        &token,
    );
    let resp = app.oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
