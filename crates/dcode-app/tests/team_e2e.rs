...
use dcode_db::{IConversationRepository, MessagePageDirection, MessagePageParams};
use axum::http::StatusCode;
use serde_json::{Value, json};
use tokio::net::TcpStream;
use tower::ServiceExt;

use dcode_api_types::TeamMcpStdioConfig;
...
async fn ensure_default_team_agent_installed(services: &dcode_app::AppServices) {
...
async fn mark_claude_backend_team_mcp_stdio_capable(services: &dcode_app::AppServices) {
...
async fn tc6c_create_team_rejects_missing_workspace_path() {
    ...
    let missing_workspace =
        std::env::temp_dir().join(format!("dcode-team-missing-{}", dcode_common::generate_short_id()));
