...
//! integration level in `dcode-office/tests/`.
...
use dcode_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
...
fn build_test_office_state(
    allowed_roots: Vec<std::path::PathBuf>,
    project: std::sync::Arc<dcode_project::ProjectService>,
) -> OfficeRouterState {
    use dcode_office::error::OfficeError;
    use dcode_office::types::DocType;
    use dcode_office::{ProcessHandle, ProcessSpawner};
