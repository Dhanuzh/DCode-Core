...
use dcode_db::SqliteProviderRepository;
...
async fn upgrade_from_pre_user_scope_keeps_encryption_key() {
    ...
    // ── Phase 1: a genuine pre-user-scope (≤028) database ────────────────
    {
        ...
        let mut migrator: Migrator = sqlx::migrate!("../dcode-db/migrations");
        ...
        let key = dcode_app::derive_encryption_key("legacy-secret-0123456789");
        let enc = dcode_common::encrypt_string("sk-upgrade-SECRET", &key).unwrap();
        sqlx::query(
    ...
    let db = dcode_db::init_database(&db_path).await.unwrap();
    ...
    let key = dcode_app::derive_encryption_key(&services.jwt_secret_raw);
    let repo = Arc::new(SqliteProviderRepository::new(services.database.pool().clone()));
    let svc = dcode_system::ProviderService::new(repo, key);
    let list = svc
