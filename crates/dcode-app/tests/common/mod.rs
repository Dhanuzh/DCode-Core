//! Shared test helpers for dcode-app E2E tests.
#![allow(dead_code)]
...
use dcode_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use dcode_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use dcode_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use dcode_file::FileService;
use dcode_system::VersionCheckService;
...
pub async fn build_app_with_skill_paths(root: &std::path::Path) -> (axum::Router, AppServices, SkillPaths) {
    ...
    states.skill = SkillRouterState {
        ...
        skill_repo: std::sync::Arc::new(dcode_db::SqliteSkillRepository::new(services.database.pool().clone())),
...
pub async fn sync_skill_catalog_for_test(services: &AppServices, paths: &SkillPaths, username: &str) {
    let repo = dcode_db::SqliteSkillRepository::new(services.database.pool().clone());
    let user = services
        .user_repo
    ...
    dcode_extension::sync_skill_catalog_into_repo_for_user(paths, &repo, &user.id)
        .await
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await.expect("build module states");
    states.shell.shell_service = std::sync::Arc::new(dcode_shell::ShellService::new(std::sync::Arc::new(
        dcode_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                dcode_ai_agent::types::BuildTaskOptions,
            )
                -> futures_util::future::BoxFuture<'static, Result<AgentInstance, dcode_ai_agent::AgentError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn dcode_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> dcode_common::AgentType {
        dcode_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<dcode_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> dcode_common::TimestampMs {
        dcode_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<dcode_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: dcode_ai_agent::types::SendMessageData,
    ) -> Result<(), dcode_ai_agent::AgentSendError> {
        Ok(())
    }
    async fn cancel(&self) -> Result<(), dcode_ai_agent::AgentError> {
        Ok(())
    }
    fn kill(&self, _reason: Option<dcode_common::AgentKillReason>) -> Result<(), dcode_ai_agent::AgentError> {
        Ok(())
    }
}
...
impl IMockAgent for NoopMockAgent {
    async fn get_config_options(
        &self,
    ) -> Result<dcode_api_types::GetConfigOptionsResponse, dcode_ai_agent::AgentError> {
        Ok(dcode_api_types::GetConfigOptionsResponse {
            config_options: vec![dcode_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                ...
                options: vec![dcode_api_types::AcpConfigSelectOptionDto {
    ...
    async fn set_config_option(
        ...
        _option_id: &str,
        _value: &str,
    ) -> Result<dcode_api_types::SetConfigOptionResponse, dcode_ai_agent::AgentError> {
        Ok(dcode_api_types::SetConfigOptionResponse {
            confirmation: dcode_api_types::ConfigOptionConfirmation::Observed,
            config_options: Some(vec![dcode_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                name: Some("Model".to_owned()),
                label: None,
                description: None,
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("dcode-csrf-token="))
        .map(|s| {
            s.strip_prefix("dcode-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub fn json_with_token(method_str: &str, uri: &str, body: serde_json::Value, token: &str, csrf: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("cookie", format!("dcode-csrf-token={csrf}"))
...
pub async fn setup_and_login(
    ...
    let hash = dcode_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
