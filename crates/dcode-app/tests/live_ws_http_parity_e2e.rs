...
//!
//! ```sh
//! cargo test -p dcode-app --test live_ws_http_parity_e2e -- --ignored --nocapture
//! ```
...
use dcode_app::{AppConfig, AppServices, create_router};
...
async fn start_live_app() -> LiveApp {
    start_live_app_on(dcode_db::init_database_memory().await.unwrap(), true).await
}
...
/// it again.
async fn start_live_app_on(db: dcode_db::Database, create_user: bool) -> LiveApp {
    ...
    let services = AppServices::from_config_with_backend_binary_path(
        db,
        &config,
        std::path::PathBuf::from(env!("CARGO_BIN_EXE_dcode_core")),
    )
    .await
    .unwrap();
...
async fn run_backend_parity(backend: &str, prompt: &str) {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-parity-{backend}-{}", dcode_common::now_ms()));
...
async fn run_backend_cancel(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-cancel-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_usage(backend: &str, expects_window_size: bool) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-usage-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_set_model(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-model-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_session_title(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-title-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_resume(backend: &str) {
    ...
    let secret = format!("AION-{}", dcode_common::now_ms() % 100_000);

    let ws_dir = std::env::temp_dir().join(format!("live-resume-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn run_backend_mcp_provisioning(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-mcp-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    ...
    let created_server = http_json(
        ...
        "/api/mcp/servers",
        json!({
            "name": format!("live-probe-{}", dcode_common::now_ms()),
            "description": "unlaunchable on purpose — proves a failure is reported, not swallowed",
            "transport": {"type": "stdio", "command": "dcode-no-such-mcp-server", "args": []}
        }),
    )
    .await;
...
async fn run_direct_backend_team_mcp_and_runtime_env(backend: &str, agent_id: &str) {
    const SENTINEL: &str = "DCODE_DIRECT_CLI_E2E_42";
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "dcode_app=info,dcode_ai_agent=info,dcode_session=info,dcode_team=info",
        ))
        .with_test_writer()
        .try_init();
    assert_eq!(
        std::env::var("DCODE_E2E_SENTINEL").as_deref(),
        Ok(SENTINEL),
        "run this ignored release gate with DCODE_E2E_SENTINEL={SENTINEL}"
    );

    let app = start_live_app().await;
    let suffix = dcode_common::now_ms();
    ...
    let shell_frames = drive_and_collect_from_uri(
        ...
        "Now use your real shell/command-execution tool to run exactly: \
         printf '{}\\n' | \"$DCODE_HELPER_BIN\" team members >/dev/null && \
         test \"$DCODE_E2E_SENTINEL\" = \"DCODE_DIRECT_CLI_E2E_42\" && echo DCODE_ENV_OK. \
         Only after the Team CLI fallback helper and sentinel check both succeed, reply with exactly: DCODE_ENV_OK",
        300,
    )
    .await;
    ...
    let mcp_prompt = "Call the team_members MCP tool from the dcode-team MCP server now. \
         Do not use DCODE_HELPER_BIN or any CLI fallback for that call. \
    ...
    assert!(
        shell_tool_evidence.contains("DCODE_HELPER_BIN")
            && (shell_tool_evidence.contains("DCODE_E2E_SENTINEL") || shell_tool_evidence.contains("DCODE_ENV_OK")),
    ...
    let shell_reply = shell_frames
        ...
        .collect::<String>();
    assert!(
        shell_reply.contains("DCODE_ENV_OK"),
...
async fn run_backend_permission_prompt(backend: &str) {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-perm-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    ...
    let outside = std::env::temp_dir().join(format!("aion-approval-{}.txt", dcode_common::now_ms()));
    let frames = connect_ws_recorder(app.addr, &app.token).await;
    http_json(
        &app,
        "POST",
...
async fn conversation_for(app: &LiveApp, backend: &str, label: &str) -> String {
    let ws_dir = std::env::temp_dir().join(format!("live-{label}-{backend}-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
    let created = http_json(
        app,
        "POST",
...
async fn live_claude_ask_user_question_round_trip() {
    let app = start_live_app().await;
    let ws_dir = std::env::temp_dir().join(format!("live-ask-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_codex_full_access_writes_without_approval() {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-fullaccess-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_natural_completion_still_finishes() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "dcode_ai_agent=debug,dcode_session=info",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-wf-natural-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_cancel_recovers_conversation() {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "dcode_ai_agent=debug,dcode_session=debug",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-wf-cancel-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_claude_workflow_progress_streams() {
    ...
    let ws_dir = std::env::temp_dir().join(format!("live-wf-progress-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
...
async fn live_codex_background_work_probe() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "dcode_ai_agent=debug,dcode_session=debug,dcode_conversation=info",
        ))
        .try_init();
    let app = start_live_app().await;

    let ws_dir = std::env::temp_dir().join(format!("live-codex-bg-{}", dcode_common::now_ms()));
    std::fs::create_dir_all(&ws_dir).unwrap();
