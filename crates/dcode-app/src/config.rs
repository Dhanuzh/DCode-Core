...
impl AppConfig {
    ...
    pub fn database_path(&self) -> PathBuf {
        self.data_dir.join("dcode-backend.db")
    }
}
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            host: dcode_common::constants::DEFAULT_HOST.to_string(),
            port: dcode_common::constants::DEFAULT_PORT,
...
pub fn derive_encryption_key(jwt_secret: &str) -> [u8; 32] {
    ...
    hasher.update(b"dcode-encryption-key:");
...
mod tests {
    ...
    fn test_app_config_database_path() {
        let config = AppConfig {
            data_dir: PathBuf::from("/tmp/dcode"),
            ..Default::default()
        };
        assert_eq!(config.database_path(), PathBuf::from("/tmp/dcode/dcode-backend.db"));
    }
}
