//! `dcode-core` (no subcommand): the main HTTP server.
...
use dcode_api_types::{RuntimeStatusScope, RuntimeStatusScopeKind};
use dcode_app::{AppConfig, AppServices, RouterBuildError, create_router_with_runtime};
use dcode_system::RuntimePrepareService;
use dcode_team::TeamIdleCleanupCoordinator;
...
// so this marker only signals the "now serving" edge. DCode treats it as the
...
pub(crate) async fn run_server(
    ...
    // 10 min / 30 min / 60 s and are overridable via DCODE_IDLE_TIMEOUT_SECS,
    // DCODE_TEAM_IDLE_TIMEOUT_SECS, and DCODE_IDLE_SCAN_INTERVAL_SECS. The
    ...
    let idle_cleanup_coordinator: Arc<dyn dcode_ai_agent::IdleCleanupCoordinator> =
        Arc::new(TeamIdleCleanupCoordinator::new(
            router_runtime.team_service.clone(),
            services.active_lease_registry.clone(),
        ));
    let (solo_timeout_secs, team_timeout_secs, scan_interval_secs) = dcode_ai_agent::resolve_idle_config_from_env();
    let idle_scanner_handle = dcode_ai_agent::start_idle_scanner_with_coordinator(
    ...
    // authoritative readiness marker now (never at bind time) so DCode can treat
