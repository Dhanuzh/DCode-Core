//! Top-level agent-readable capability index for the `dcode-core` binary.
...
fn data() -> Value {
    json!({
        "schema_version": 1,
        "contract": "agent-facing-dcode-core-cli",
        "stability": "stable",
        "entrypoint": "dcode-core capabilities",
        "purpose": "Top-level index for agent-facing DCode Core CLI domains.",
        "output": {
        ...
        "runtime_context": {
            "primary": "DCODE_CONVERSATION_ID",
            "environment": RUNTIME_ENV,
            "selectors": {
                "conversation_id": {
                    "current": "resolve from DCODE_CONVERSATION_ID"
                },
                "assistant_id": {
                    "current": "resolve via current conversation"
                },
                "user_id": {
                    "current": "resolve from DCODE_USER_ID"
                }
            }
        },
        "input": {
        ...
        "domains": [
            {
                "name": "config",
                "mode": "read-write",
                "description": "Manage DCode configuration: assistants, assistant rules, skills, MCP servers, providers, settings, agents, and scheduled tasks.",
                "contract": "agent-facing-config-cli",
                "contract_command": "config capabilities",
                "invocation": "dcode-core config capabilities",
                "runtime_required": ["DCODE_BASE_URL", "DCODE_CONVERSATION_ID", "DCODE_USER_ID"],
                "safety": {
                    "can_write": true,
            ...
            },
            {
                "name": "diagnose",
                "mode": "read-only",
                "description": "Diagnose a running DCode installation: backend health, conversations, provider health, MCP, cron, teams, logs, and controlled GET reads.",
                "contract": "agent-facing-diagnose-cli",
                "contract_command": "diagnose capabilities",
                "invocation": "dcode-core diagnose capabilities",
                "runtime_required": ["DCODE_BASE_URL", "DCODE_CONVERSATION_ID", "DCODE_USER_ID"],
                "optional_runtime": ["DCODE_LOG_DIR"],
                "safety": {
                    "can_write": false,
                    "read_only": true,
                    "redacted_by_default": true,
            ...
            },
            {
                ...
                "invocation": "dcode-core team capabilities",
                "runtime_required": ["DCODE_BASE_URL", "DCODE_CONVERSATION_ID", "DCODE_USER_ID", "DCODE_RUNTIME_TOKEN"],
