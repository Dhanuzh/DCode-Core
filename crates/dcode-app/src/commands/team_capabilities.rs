use dcode_api_types::{TEAM_TOOLS_SCHEMA_VERSION, team_tool_descriptors};
...
pub(crate) fn data() -> Value {
    ...
    json!({
        ...
        "commands": {
            ...
            "context": { "runtime_env_required": ["DCODE_BASE_URL", "DCODE_USER_ID", "DCODE_CONVERSATION_ID", "DCODE_RUNTIME_TOKEN"] },
            "tool_call": { "runtime_env_required": ["DCODE_BASE_URL", "DCODE_USER_ID", "DCODE_CONVERSATION_ID", "DCODE_RUNTIME_TOKEN"] }
        },
        "output_envelope": {
            "success": "boolean",
...
pub(crate) fn help_markdown() -> String {
    let mut text = String::from("# DCode Core Team CLI\n\nUse `dcode-core team capabilities` for exact schemas.\n\n");
    for tool in team_tool_descriptors() {
        ...
        text.push_str(&format!(
            "- `dcode-core team {command}` -> `{}` ({permission}): {}\n",
