//! `dcode-core doctor` subcommand: agent CLI detection self-check.
//!
...
//! `dcode_runtime::init` + `enhance_process_path` for `Doctor` as it
//! does for the server, so managed runtimes and CLI commands resolve
//! through the same paths the server uses.
//!
//! Writes to stdout (not the rolling dcode-core.log) — the user
...
use dcode_ai_agent::{AgentRegistry, UnavailableReason};
use dcode_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database, maybe_copy_legacy_database};
use dcode_runtime::doctor_snapshot;
...
pub async fn run_doctor(cli: &Cli, merged_path: &str) -> Result<ExitCode, CliBoundaryError> {
    ...
    let db_path = cli.data_dir.join("dcode-backend.db");
...
fn print_environment(merged_path: &str, data_dir: &Path) {
    ...
    println!("DCode backend doctor — agent CLI detection self-check");
...
fn print_snapshot(snapshot: &[(dcode_api_types::AgentMetadata, Option<UnavailableReason>)]) {
...
mod tests {
    ...
    fn doctor_database_error_uses_stable_code_without_raw_path() {
        ...
        assert!(!err.stderr_line().contains("/Users/secret/dcode-backend.db"));
    }
