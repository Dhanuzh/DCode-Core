...
impl RuntimeEnv {
    fn headers(&self) -> reqwest::header::HeaderMap {
        ...
        headers.insert("x-dcode-user-id", self.user_id.parse().unwrap());
        headers.insert("x-dcode-conversation-id", self.conversation_id.parse().unwrap());
        headers.insert("x-dcode-runtime-token", self.runtime_token.parse().unwrap());
        headers
    }
}
...
fn validate_against_descriptor(command: &str, tool: TeamToolName, value: Value) -> Result<Value, ExitCode> {
    ...
    let descriptor = dcode_api_types::team_tool_descriptor(tool.as_str()).expect("descriptor for canonical tool");
