//! `dcode-core mcp-bridge` subcommand: stdio ↔ TCP bridge for the team MCP server.
//!
//! Spawned by the ACP agent CLI as an MCP server with command `dcode-core mcp-bridge`.
...
//! `127.0.0.1:<TEAM_MCP_PORT>` (reusing `dcode_team::mcp::protocol`).
//!
...
/// Entry point for `dcode-core mcp-bridge`. Returns an [`ExitCode`] so the
