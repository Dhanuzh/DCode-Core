...
use dcode_app::{AppConfig, IdentityMode};
use dcode_db::Database;
...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment, BootstrapError> {
    ...
    unsafe {
        std::env::set_var("DCODE_WORK_DIR", &work_dir);
    }
...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database, BootstrapError> {
    ...
    dcode_db::maybe_copy_legacy_database(&db_path).map_err(|e| {
        BootstrapError::new(
            BootstrapErrorCode::DataInitFailed,
            "data.legacy_db",
            "failed to initialize application data",
    ...
    let database = dcode_db::init_database_staged_with_options(
        &db_path,
        dcode_db::DatabaseInitOptions {
...
fn database_init_bootstrap_error(error: dcode_db::DatabaseInitError, db_path: &std::path::Path) -> BootstrapError {
    ...
    if let Some(db_version) = source.missing_migration_version() {
        ...
        if let Some(app_version) = dcode_db::latest_known_migration_version() {
...
mod tests {
    ...
    fn database_stage_comes_from_db_boundary_error() {
        let err = dcode_db::DatabaseInitError::new(
            "database.migration",
            dcode_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMismatch(42)),
        );
    ...
    fn database_schema_repair_stage_comes_from_db_boundary_error() {
        let err = dcode_db::DatabaseInitError::new(
            ...
            dcode_db::DbError::Init("repair failed".into()),
        );
    ...
    fn database_recoverable_corruption_stage_comes_from_db_boundary_error() {
        let err = dcode_db::DatabaseInitError::new(
            ...
            dcode_db::DbError::Migration(sqlx::migrate::MigrateError::ExecuteMigration(
    ...
    fn newer_than_app_stage_carries_migration_versions_on_stderr() {
        let err = dcode_db::DatabaseInitError::new(
            dcode_db::DATABASE_NEWER_THAN_APP_STAGE,
            dcode_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMissing(39)),
        );

        let bootstrap = database_init_bootstrap_error(err, std::path::Path::new("/db/path/dcode-backend.db"));
        ...
        let app_version = dcode_db::latest_known_migration_version().unwrap();
    ...
    fn generic_database_failures_do_not_carry_migration_versions() {
        let err = dcode_db::DatabaseInitError::new(
            "database.migration",
            dcode_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMismatch(7)),
        );
