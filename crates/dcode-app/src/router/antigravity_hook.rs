...
//!
//! agy cannot prompt for tool permission in headless mode, so DCode registers
...
use dcode_ai_agent::antigravity_hook::HookTokenRegistry;
...
async fn handle_hook(
    ...
    if !state.tokens.verify(&conversation_id, presented) {
        ...
        return Json(AntigravityHookOutput::deny(
            "dcode rejected an unauthenticated hook callback",
        ));
    }
    ...
    let Some(AgentInstance::Session(task)) = state.task_manager.get_task(&conversation_id) else {
        ...
        return Json(AntigravityHookOutput::deny(
            "dcode has no live session for this conversation",
        ));
    };
    ...
    tracing::info!(
        ...
        approved = matches!(
            decision,
            dcode_session::PermissionDecision::Approved | dcode_session::PermissionDecision::AllowAlways
        ),
    ...
    Json(match decision {
        dcode_session::PermissionDecision::Approved | dcode_session::PermissionDecision::AllowAlways => {
            AntigravityHookOutput::allow("approved in DCode")
        }
        dcode_session::PermissionDecision::Denied => AntigravityHookOutput::deny("rejected in DCode"),
    })
}
...
mod tests {
    use super::*;
    use dcode_ai_agent::WorkerTaskManagerImpl;
    use dcode_ai_agent::task_manager::AgentFactory;
    use dcode_api_types::{AntigravityHookDecision, AntigravityHookToolCall};
