...
use dcode_api_types::{AssistantConversationRequest, CreateConversationRequest, GetConfigOptionsResponse};
...
use dcode_db::models::{AgentMetadataRow, MessageRow};
use dcode_db::{IAgentMetadataRepository, IConversationRepository};
use dcode_team::{
...
impl NativeSlashCommandPort for TeamConversationAdapters {
    ...
    /// the SHARED grammar owner (`dcode_session::slash_command_name`, same
    ...
    async fn recognize(&self, conversation_id: &str, content: &str) -> SlashCommandRecognition {
        let Some(name) = dcode_session::slash_command_name(content) else {
...
impl TeamConversationAdapters {
    ...
    async fn resolve_slash_catalog(&self, conversation_id: &str) -> Option<(Vec<String>, SlashCatalogSource)> {
        ...
        {
            let names = dcode_session::codex_capabilities()
                .slash_commands
                .into_iter()
