...
use dcode_common::{CapabilityOrigin, McpTransportCapabilities, ResolvedBackendCapabilities};
use dcode_db::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
use dcode_team::{TeamError, TeamToolCapabilityPort};
...
impl TeamToolCapabilityPort for TeamCapabilityResolver {
    async fn resolve(
        ...
        if let Some(descriptor) = dcode_session::backend_capability_descriptor(backend) {
