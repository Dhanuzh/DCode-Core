...
use dcode_api_types::ErrorResponse;
use dcode_assets::{AssetRouterState, asset_routes};
use dcode_assistant::assistant_routes;
use dcode_auth::{
...
use dcode_channel::channel_routes;
#[cfg(feature = "weixin")]
use dcode_channel::weixin_login_route;
use dcode_common::ApiErrorLogContext;
use dcode_conversation::{conversation_ops_routes, conversation_routes};
use dcode_cron::cron_routes;
use dcode_extension::{extension_routes, hub_routes, skill_routes};
use dcode_file::file_routes;
use dcode_mcp::mcp_routes;
use dcode_office::{office_proxy_routes, office_routes};
use dcode_project::project_routes;
use dcode_realtime::{NoopMessageRouter, WsHandlerState, ws_upgrade_handler};
use dcode_shell::shell_routes;
use dcode_system::{ClientPrefService, connection_test_routes, system_routes};
use dcode_team::{TeamSessionService, team_routes};
...
pub async fn create_router_with_runtime(services: &AppServices) -> Result<(Router, RouterRuntime), RouterBuildError> {
    ...
    let inbound_router: Arc<dyn dcode_realtime::MessageRouter> = match scm_router {
...
pub fn create_router_with_all_state(services: &AppServices, states: ModuleStates, ws_state: WsHandlerState) -> Router {
    ...
    let router = router.layer(DefaultBodyLimit::max(dcode_common::constants::BODY_LIMIT));
...
/// Adapter running the on-disk side of DCode → AionPro adoption over the
...
/// as the conversation-helper credential channel (dcode-auth cannot depend on
/// dcode-ai-agent, so the binding happens here in the composition layer).
