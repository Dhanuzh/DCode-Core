...
use dcode_auth::{CookieConfig, JwtService, QrTokenStore, resolve_jwt_secret};
use dcode_common::OnConversationDelete;
use dcode_conversation::{ConversationService, runtime_state::ConversationRuntimeStateService};
use dcode_db::{
...
use dcode_project::ProjectService;
use dcode_realtime::{BroadcastEventBus, WebSocketManager};
...
pub struct AppServices {
    ...
    pub skill_paths: Arc<dcode_extension::SkillPaths>,
    ...
    pub(crate) antigravity_hook_tokens: Arc<dcode_ai_agent::antigravity_hook::HookTokenRegistry>,
}
...
impl AppServices {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        let backend_binary_path = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("dcode-core"));
    ...
    /// integration tests run the real `dcode-core` MCP/helper subcommands instead
    ...
    pub async fn from_config_with_backend_binary_path(
        ...
        let skill_paths = Arc::new(dcode_extension::resolve_skill_paths(&app_resource_dir, &data_dir));
        if identity_mode.is_local() {
            dcode_extension::sync_skill_catalog_into_repo(skill_paths.as_ref(), skill_repo.as_ref())
                .await
            ...
            dcode_extension::sync_builtin_skill_catalog_into_repo(skill_paths.as_ref(), skill_repo.as_ref())
                .await
                .map_err(|e| anyhow::anyhow!("Failed to synchronize skill catalog: {e}"))?;
        }
        ...
        let antigravity_hook_tokens = Arc::new(dcode_ai_agent::antigravity_hook::HookTokenRegistry::new());
        ...
        let process_registry = Arc::new(dcode_process::FileRegistryStore::new(&data_dir));
        let machine_id = dcode_process::local_machine_id(&data_dir);
        let session_spawner: Arc<dyn dcode_process::Spawner> = Arc::new(dcode_process::RealSpawner::new(
            process_registry,
            uuid::Uuid::now_v7(),
            machine_id,
        ));
        ...
        let factory = build_agent_factory(AgentFactoryDeps {
            ...
            session_spawner,
            // agy cannot prompt for tool permission in headless mode, so DCode
...
struct ConversationServiceDeps<'a> {
    ...
    skill_paths: Arc<dcode_extension::SkillPaths>,
