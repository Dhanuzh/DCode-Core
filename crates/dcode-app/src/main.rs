...
fn run_main() -> Result<ExitCode, MainError> {
    ...
    // parse a uniform CLI, but bypass `dcode_runtime::init` (which would
    ...
    // detection path exactly. It must hit the same `dcode_runtime::init`
    // before performing managed-runtime and PATH probing.
    let needs_runtime = cli.command.as_ref().is_none_or(Command::need_runtime);
    if needs_runtime {
        dcode_runtime::set_managed_resources_mode(cli.managed_resources_mode.into());
        dcode_runtime::init(&cli.data_dir);
    }
    ...
    let merged_path = unsafe { dcode_runtime::enhance_process_path() };
...
async fn async_main(merged_path: String, cli: Cli) -> Result<ExitCode, MainError> {
    ...
    match cli.command {
        ...
        None => {
            ...
            // touching the DB, so a second dcode-core yields structurally rather
            ...
            let _instance_guard = match dcode_db::DataDirInstanceGuard::try_acquire(&db_path) {
                Ok(Some(guard)) => {
                    ...
                    match bootstrap::wait_for_instance_guard(&db_path) {
                        ...
                        Ok(None) => {
                            tracing::info!(
                                ...
                                "another dcode-core owns the data directory; yielding"
                            );
                            return Err(MainError::Bootstrap(bootstrap::BootstrapError::new(
                                ...
                                "another dcode-core already owns this data directory",
                            )));
                        }
                        Err(error) => {
                            tracing::warn!(
