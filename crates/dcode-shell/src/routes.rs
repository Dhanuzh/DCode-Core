...
use dcode_system::ClientPrefService;
...
///
/// Key mismatch fix: the DCode frontend stores the config under
...
pub(crate) async fn load_stt_config(
    ...
    user_id: &str,
) -> Result<SpeechToTextConfig, dcode_system::SystemError> {
    ...
    Ok(prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: dcode_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        }))
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(dcode_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = dcode_system::ClientPrefService::new(repo);
