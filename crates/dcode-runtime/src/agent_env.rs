...
mod tests {
    ...
    const CHILD_MARKER: &str = "DCODE_RUNTIME_AGENT_ENV_TEST_CHILD";
...
"#,
            ...
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("DCODE_CURRENT_ONLY", "from-current")
                .env("DCODE_OVERLAY", "from-current")
        ...
        assert_eq!(value("DCODE_CURRENT_ONLY").as_deref(), Some("from-current"));
        assert_eq!(value("DCODE_SHELL_ONLY").as_deref(), Some("from-shell"));
        assert_eq!(value("DCODE_OVERLAY").as_deref(), Some("from-shell"));
