...
fn bundled_root_from_parent() -> PathBuf {
    PathBuf::from(
        std::env::var_os("DCODE_BUNDLED_MANAGED_RESOURCES").expect("parent must pass the bundled resources root"),
    )
}
...
/// phase, so DCode is never told the installation is broken (spec AC1).
///
...
async fn transient_npm_version_failure_is_absorbed_without_failed_report() {
    ...
    if !crate::test_support::run_in_env_child(
        ...
        |command| {
            command.env("DCODE_BUNDLED_MANAGED_RESOURCES", &bundled_root);
        },
    ) {
        return;
    }
...
fn classify_error_detects_bundled_node_runtime_missing() {
    let err = NodeRuntimeError::managed_invalid(
        "bundled Node runtime missing under C:\\Program Files\\DCode\\resources\\bundled-dcode-core\\win32-x64\\managed-resources\\node\\node-v24.11.0-win-x64",
    );
...
fn managed_runtime_install_lock_path_uses_runtime_root() {
    let root = PathBuf::from("/tmp/dcode/runtime/node");
...
async fn bundled_runtime_validation_failure_does_not_fallback_to_remote_download() {
    ...
    let bundled_root = std::path::PathBuf::from(std::env::var_os("DCODE_BUNDLED_MANAGED_RESOURCES").unwrap());
...
fn windows_managed_cli_paths_use_node_modules_under_archive_root() {
    let root = PathBuf::from(r"C:\DCode\node-v24.11.0-win-x64");
...
fn unix_managed_cli_paths_use_lib_node_modules() {
    let root = PathBuf::from("/opt/dcode/node-v24.11.0-darwin-arm64");
