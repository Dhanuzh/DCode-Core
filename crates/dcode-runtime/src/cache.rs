...
///
/// Lifecycle: written once by `dcode-app`'s `main()` before
...
pub fn init(data_dir: impl AsRef<Path>) {
        ...
        && existing != path
    {
        tracing::warn!(
            ...
            "dcode_runtime::init called twice with different paths; keeping first"
        );
    }
}

/// Returns the root cache directory used for all dcode runtime artifacts.
///
/// Priority:
/// 1. Path supplied via [`init`] (`{data_dir}/runtime`) when the backend
///    started with `--data-dir`.
/// 2. Platform cache dir (via `dirs::cache_dir()`):
///    - macOS:   `~/Library/Caches/dcode/runtime`
///    - Linux:   `$XDG_CACHE_HOME/dcode/runtime` (fallback `~/.cache/dcode/runtime`)
///    - Windows: `%LOCALAPPDATA%\dcode\runtime`
///
...
pub fn runtime_root() -> Option<PathBuf> {
    ...
    dirs::cache_dir().map(|d| d.join("dcode").join("runtime"))
}
...
mod tests {
    ...
    fn runtime_root_ends_with_expected_suffix() {
        ...
        let tail: Vec<_> = root
            ...
            .collect();
        assert_eq!(tail, vec!["runtime".to_string(), "dcode".to_string()]);
    }
    ...
    fn node_runtime_root_appends_node_directory() {
        ...
        assert_eq!(
            tail,
            vec!["node".to_string(), "runtime".to_string(), "dcode".to_string()]
        );
    }
}
