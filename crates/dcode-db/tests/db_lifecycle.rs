...
async fn system_user_has_valid_timestamps() {
    let before = dcode_common::now_ms();
    let db = init_database_memory().await.unwrap();
    let after = dcode_common::now_ms();
...
async fn unauthorized_open_stage_corruption_upgrades_to_recoverable_stage() {
    ...
    // NOT the generic database.open stage (which DCode maps to restart-only).
...
fn copy_legacy_noop_when_no_legacy_db() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("dcode-backend.db");
...
fn copy_legacy_noop_when_target_exists() {
    let dir = tempfile::tempdir().unwrap();
    let target = dir.path().join("dcode-backend.db");
    let legacy = dir.path().join("dcode.db");
...
fn concurrent_init_database_does_not_panic_on_unique_conflict() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("dcode-backend.db");
    ...
    let lock = path.with_file_name("dcode-backend.db.migrate.lock");
