...
async fn insert_feedback_fixture(db: &dcode_db::Database) {
    let pool = db.pool();
    sqlx::query(
...
async fn insert_mcp_feedback_fixture(db: &dcode_db::Database) {
...
async fn insert_foreign_agent_metadata(db: &dcode_db::Database) {
    sqlx::query(
        "INSERT INTO users \
