...
async fn repo() -> (Arc<dyn IRemoteAgentRepository>, dcode_db::Database) {
...
async fn full_crud_lifecycle() {
    ...
    // Update status
    r.update_status(USER_ID, &created.id, "connected", Some(dcode_common::now_ms()))
        .await
        .unwrap();
