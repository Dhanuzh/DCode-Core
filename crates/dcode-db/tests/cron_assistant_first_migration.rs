...
async fn seed_legacy_assistant_identity(pool: &sqlx::SqlitePool) {
    ...
    for (id, backend, agent_type, name, source, sort_order) in [
        ("agent-aionrs", "", "aionrs", "DCode CLI", "internal", 100),
    ...
    for (definition_id, assistant_key, agent_backend, source_ref) in [
        ("def-aionrs", "dcode-assistant", "aionrs", "dcode-assistant"),
...
async fn migration_015_populates_aionrs_catalog_by_agent_type() {
    ...
    sqlx::query(
        "INSERT INTO agent_metadata (
            id, name, backend, command, agent_type, enabled, agent_source, sort_order, created_at, updated_at
         ) VALUES ('agent-aionrs', 'DCode CLI', NULL, '', 'aionrs', 1, 'internal', 100, 1, 1)",
    )
    .execute(&pool)
    .await
    .unwrap();
...
async fn migration_013_normalizes_legacy_cron_agent_identity() {
    ...
    insert_legacy_cron(
        ...
        "conv_aionrs",
        "aionrs",
        r#"{"backend":"provider-1","name":"Aion","assistant_id":"dcode-assistant","model_id":"gpt-5"}"#,
    )
    .await;
    insert_legacy_cron(
        &pool,
    ...
    .await
    .unwrap();
    assert_eq!(aionrs.get::<String, _>("assistant_id"), "dcode-assistant");
