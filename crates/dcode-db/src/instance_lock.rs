...
//! database file. The winning dcode-core holds it for its whole lifetime so a
//! second dcode-core yields structurally before touching the database or binding
...
pub fn instance_lock_path(db_path: &Path) -> PathBuf {
    ...
    let new_name = match p.file_name().and_then(|s| s.to_str()) {
        ...
        None => "dcode.instance.lock".to_string(),
    };
...
mod tests {
    ...
    fn instance_lock_path_appends_instance_lock_suffix() {
        ...
        let path = instance_lock_path(Path::new("/data/dcode/dcode-backend.db"));
        assert_eq!(path, PathBuf::from("/data/dcode/dcode-backend.db.instance.lock"));
    }

    #[test]
    fn guard_is_exclusive_and_releases_on_drop() {
        let dir = std::env::temp_dir().join(format!("dcode-instance-lock-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("dcode-backend.db");
