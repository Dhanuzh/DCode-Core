...
pub struct AgentMetadataRow {
    ...
    /// Native mode id that DCode's legacy `yolo` / `yoloNoSandbox`
    ...
    /// (`dcode-common/src/enums.rs`), a hardcoded per-backend table whose
    /// default arm is the literal `"yolo"` — see Team spawn
    /// (`dcode-team/src/service/spawn_support.rs`) and cron
    /// (`dcode-cron/src/service.rs`). So clearing this column silently hands
