...
impl IUserRepository for SqliteUserRepository {
    ...
    async fn create_user(&self, username: &str, password_hash: &str) -> Result<User, DbError> {
        let id = dcode_common::generate_prefixed_id("user");
        let now = dcode_common::now_ms();
    ...
    async fn adopt_system_default_data(&self, owner_id: &str) -> Result<u64, DbError> {
        ...
        // DCode conversations were repeatedly re-owned). A stamped marker
        ...
        // adoption-coverage classification test in dcode-db/tests.
        let mut moved: u64 = 0;
