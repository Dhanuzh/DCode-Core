...
impl IProviderRepository for SqliteProviderRepository {
    ...
    async fn create(&self, params: CreateProviderParams<'_>) -> Result<Provider, DbError> {
        let id = params
            .id
            .map(String::from)
            .unwrap_or_else(|| dcode_common::generate_prefixed_id("prov"));
        ...
        sqlx::query(
            "INSERT INTO providers \
                (id, user_id, platform, name, base_url, api_key_encrypted, models, enabled, \
                 ...
                 model_health, model_settings, bedrock_config, vertex_config, is_full_url, created_at, updated_at) \
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&id)
        .bind(params.user_id)
        ...
        Ok(Provider {
            ...
            vertex_config: params.vertex_config.map(String::from),
    ...
    async fn update(&self, user_id: &str, id: &str, params: UpdateProviderParams<'_>) -> Result<Provider, DbError> {
        ...
        sqlx::query(
            "UPDATE providers SET \
                ...
                model_settings = ?, bedrock_config = ?, vertex_config = ?, is_full_url = ?, updated_at = ? \
...
fn merge_update(existing: Provider, params: UpdateProviderParams<'_>) -> Provider {
    ...
    Provider {
        ...
        vertex_config: params
            .vertex_config
            .map_or(existing.vertex_config, |v| v.map(String::from)),
