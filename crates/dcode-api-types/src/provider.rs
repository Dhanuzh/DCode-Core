...
/// Google Cloud Vertex AI authentication method.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum VertexAuthMethod {
    ...
    /// application-default login`. Intended for developer machines; shipped
    /// builds should use a service account instead.
    Adc,
}

/// Google Cloud Vertex AI-specific configuration.
///
/// A single Vertex project serves both Gemini (`publishers/google`) and Claude
/// (`publishers/anthropic`), so one provider can expose every model the project
/// is actually entitled to rather than splitting them across platforms.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct VertexConfig {
    pub auth_method: VertexAuthMethod,
    pub project_id: String,
    /// Vertex location, e.g. `global` or `us-east5`. Model availability varies
    /// by location, so this is part of the identity of the deployment.
    pub location: String,
    /// Service-account key JSON. Required when `auth_method` is
    /// `serviceAccount`; ignored for `adc`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct FetchModelsAnonymousRequest {
    pub platform: String,
    /// Absent for platforms addressed by account/project identity rather than a
    /// user-supplied endpoint (Bedrock, Vertex), whose forms omit the field.
    #[serde(default)]
    pub base_url: String,
    /// Plain-text API key (supports multi-key). Empty for platforms whose
    /// credentials live in a dedicated config struct.
    #[serde(default)]
    pub api_key: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub bedrock_config: Option<BedrockConfig>,
...
mod tests {
    ...
    #[test]
    fn test_fetch_models_anonymous_request_missing_platform_is_error() {
        // `platform` selects the fetcher, so it has no meaningful default.
        let raw = json!({"base_url": "https://api.openai.com", "api_key": "sk"});
        assert!(serde_json::from_value::<FetchModelsAnonymousRequest>(raw).is_err());
    }

    #[test]
    fn test_fetch_models_anonymous_request_allows_missing_base_url_and_key() {
        // Bedrock and Vertex are addressed by account/project identity, so their
        // forms omit base_url and api_key entirely. Deserialization must accept
        // that shape; whether the platform actually needs those values is
        // enforced by validate_anonymous_request, which can report a useful
        ...
        let req: FetchModelsAnonymousRequest = serde_json::from_value(raw).unwrap();
        assert_eq!(req.platform, "vertex-ai");
        assert_eq!(req.base_url, "");
        assert_eq!(req.api_key, "");
        let cfg = req.vertex_config.expect("vertex_config should parse");
        assert_eq!(cfg.auth_method, VertexAuthMethod::Adc);
