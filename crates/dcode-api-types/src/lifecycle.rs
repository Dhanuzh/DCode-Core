...
mod tests {
    ...
    fn test_system_info_response_serialization() {
        let resp = SystemInfoResponse {
            cache_dir: "/home/user/.cache/dcode".into(),
            work_dir: "/home/user/.local/share/dcode".into(),
            log_dir: "/home/user/.local/state/dcode/logs".into(),
            platform: "linux".into(),
            arch: "x64".into(),
        };
        let json = serde_json::to_value(&resp).unwrap();
        assert_eq!(json["cache_dir"], "/home/user/.cache/dcode");
        assert_eq!(json["work_dir"], "/home/user/.local/share/dcode");
        assert_eq!(json["log_dir"], "/home/user/.local/state/dcode/logs");
    ...
    fn test_update_check_request_with_options() {
        let raw = json!({
            ...
            "repo": "iOfficeAI/DCode"
        });
        let req: UpdateCheckRequest = serde_json::from_value(raw).unwrap();
        assert!(req.include_prerelease);
        assert_eq!(req.repo.as_deref(), Some("iOfficeAI/DCode"));
    }
    ...
    fn test_update_check_result_with_update() {
        let result = UpdateCheckResult {
            ...
            latest: Some(UpdateReleaseInfo {
                ...
                html_url: "https://github.com/iOfficeAI/DCode/releases/tag/v2.0.0".into(),
                published_at: Some("2026-04-01T00:00:00Z".into()),
                prerelease: false,
                draft: false,
                assets: vec![GitHubReleaseAsset {
                    name: "dcode-2.0.0-darwin-arm64.dmg".into(),
                    url: "https://github.com/download/dcode.dmg".into(),
                    size: 85_000_000,
        ...
        assert_eq!(json["latest"]["assets"][0]["name"], "dcode-2.0.0-darwin-arm64.dmg");
