...
//!
//! These are promoted from `dcode-team::mcp::bridge` so that downstream
//! crates (`dcode-ai-agent` deserializing `AcpBuildExtra`, etc.) can reference
//! the same shape without depending on `dcode-team`.
...
pub const TEAM_MCP_SERVER_NAME: &str = "dcode-team";
...
mod tests {
    ...
    fn deserialization_tolerates_unknown_fields() {
        ...
        let json = r#"{"team_id":"t-1","port":1,"token":"t","slot_id":"s","binary_path":"/usr/bin/dcode-core","future_field":42}"#;
        let parsed: TeamMcpStdioConfig = serde_json::from_str(json).unwrap();
