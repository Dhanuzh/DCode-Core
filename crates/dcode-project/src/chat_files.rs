...
//! reproduces the legacy `[[DCODE_FILES]]` inlined-attachment content form so
...
/// re-inlined into [`content`](Self::content) via the `[[DCODE_FILES]]` marker.
#[derive(Debug)]
...
impl ProjectService {
    ...
    ///
    /// `upload_root` is the managed upload directory (`temp_dir()/dcode`);
    ...
    /// (send boundary) and the preview content endpoint (`dcode-file`, cross-crate — hence `pub`).
