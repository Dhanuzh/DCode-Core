...
use dcode_db::{Database, IProjectStore, SqliteProjectStore, init_database_memory};
use dcode_project::ProjectService;
use dcode_project::canonical::{canonicalize, to_file_uri};
use dcode_project::types::{AttachInput, FileOp, ReferenceInput};
...
async fn create_standard_missing_dir_is_folder_not_found() {
    let (svc, _db) = service().await;
    let missing = uri_of(std::path::Path::new("/nonexistent-dcode-xyz-8f3a2b1c"));
...
async fn a_case_variant_folder_resolves_to_one_repo_identity() {
    use dcode_project::canonical::IGNORE_PATH_CASING;
