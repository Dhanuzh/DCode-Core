...
/// MCP Agent adapter for Dcode itself.
///
/// Unlike CLI-based adapters, this adapter reads/writes directly to the
/// local database. It is always "installed" since DCode is the host
/// application.
///
/// # Behavior
///
...
impl McpAgentAdapter for DcodeAdapter {
    ...
    async fn install_server(&self, _name: &str, _transport: &McpServerTransport) -> Result<(), McpError> {
        ...
        // Dcode the server is already in the DB.
        Ok(())
    }
...
mod tests {
    ...
    fn source_is_dcode() {
        ...
        let adapter = DcodeAdapter::new(repo);
        assert_eq!(adapter.source(), McpSource::Dcode);
    }
    ...
    async fn trait_is_object_safe() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter: Arc<dyn McpAgentAdapter> = Arc::new(DcodeAdapter::new(repo));
        assert_eq!(adapter.source(), McpSource::Dcode);
        assert!(adapter.is_installed().await.unwrap());
    }
}
