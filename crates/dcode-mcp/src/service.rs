...
use dcode_db::{CreateMcpServerParams, IMcpServerRepository, UpdateMcpServerParams};
...
mod tests {
    use super::*;
    use dcode_api_types::{ImportMcpServerRequest, McpTransport};
    use dcode_common::{McpServerStatus, TimestampMs};
    use dcode_db::models::McpServerRow;
    use dcode_db::{CreateMcpServerParams, DbError, UpdateMcpServerParams};
    ...
    async fn persist_test_result_records_success_status_and_tools() {
        ...
        let result = McpConnectionTestResult {
            success: true,
            tools: Some(vec![dcode_api_types::McpToolResponse {
                name: "read_file".into(),
        ...
        assert_eq!(updated.last_test_status, dcode_common::McpServerStatus::Connected);
    ...
    async fn persist_test_result_records_error_and_clears_tools() {
        ...
        let updated = svc.get_server(TEST_USER_ID, &created.id).await.unwrap();
        assert_eq!(updated.last_test_status, dcode_common::McpServerStatus::Error);
