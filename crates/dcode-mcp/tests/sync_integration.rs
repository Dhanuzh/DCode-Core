...
use dcode_db::SqliteMcpServerRepository;
use dcode_mcp::{DetectedServer, McpAgentAdapter, McpError, McpServerTransport, McpSyncService};
...
async fn make_service(adapters: Vec<Arc<dyn McpAgentAdapter>>) -> McpSyncService {
    ...
    let repo: Arc<dyn dcode_db::IMcpServerRepository> = Arc::new(SqliteMcpServerRepository::new(db.pool().clone()));
