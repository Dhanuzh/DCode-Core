//! Integration tests for file-based MCP Agent adapters (Opencode, Aionrs, Dcode).
//!
...
//!
//! For Dcode, we use a mock repository since it reads from the DB.
...
use dcode_mcp::{DcodeAdapter, McpAgentAdapter, McpServerTransport};

// ===========================================================================
// Dcode adapter (DB-backed)
// ===========================================================================

mod dcode {
    use super::*;
    use dcode_db::models::McpServerRow;
    use dcode_db::{CreateMcpServerParams, DbError, IMcpServerRepository, UpdateMcpServerParams};
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn update_status(
            ...
            _id: &str,
            _s: &str,
            _lc: Option<dcode_common::TimestampMs>,
        ) -> Result<(), DbError> {
            unimplemented!()
        }
    ...
    async fn install_is_noop() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = DcodeAdapter::new(repo.clone());
