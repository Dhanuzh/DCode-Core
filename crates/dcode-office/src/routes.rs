...
use dcode_file::{FileError, path_safety::validate_path_with_extra_root};
...
async fn start_preview(
    ...
    let validated_path = match &req.file {
        Some(file) => {
            ...
            state
                .project
                .resolve_chat_file_ref(user_id, file, &upload_root, dcode_project::FileOp::Read)
                .await
...
fn refresh_resolve_error(err: dcode_project::ProjectError) -> ApiError {
    let code = err.code();
...
) -> Result<Json<ApiResponse<dcode_api_types::DocumentConversionResponse>>, ApiError> {
    let Json(req) = body.map_err(ApiError::from)?;
...
mod tests {
    ...
    async fn build_test_state() -> OfficeRouterState {
        ...
        impl dcode_realtime::EventBroadcaster for NoopBroadcaster {
            fn broadcast(&self, _msg: dcode_api_types::WebSocketMessage<serde_json::Value>) {}
        }

        let spawner = Arc::new(NoopSpawner);
        let bc: Arc<dyn dcode_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
        ...
        let project = Arc::new(dcode_project::ProjectService::new(store, std::env::temp_dir()));
