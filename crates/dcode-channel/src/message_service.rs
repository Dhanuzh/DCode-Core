...
use dcode_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
use dcode_api_types::{AssistantConversationRequest, CreateConversationRequest, SendMessageRequest};
use dcode_common::{AgentType, ConversationSource};
...
fn platform_to_source(platform: PluginType) -> ConversationSource {
    match platform {
        ...
        // Reserved variants default to Dcode
        PluginType::Slack | PluginType::Discord => ConversationSource::Dcode,
    }
}
...
mod tests {
    ...
    #[test]
    fn platform_to_source_reserved_defaults_to_dcode() {
        assert_eq!(platform_to_source(PluginType::Slack), ConversationSource::Dcode);
        assert_eq!(platform_to_source(PluginType::Discord), ConversationSource::Dcode);
    }
