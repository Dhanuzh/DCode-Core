...
impl ChannelSettingsService {
    ...
    async fn effective_assistant_backend(
        &self,
        user_id: &str,
        definition: &dcode_db::models::AssistantDefinitionRow,
        overlays: &[dcode_db::models::AssistantOverlayRow],
...
mod tests {
    ...
    use dcode_db::{IAssistantDefinitionRepository, IAssistantOverlayRepository};
    use std::sync::Mutex;
    ...
    async fn agent_config_aionrs_has_no_backend() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            ...
            r#"{"backend":"aionrs","name":"DCode CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
    ...
    async fn agent_config_reads_new_format_aionrs() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "assistant.lark.agent",
            r#"{"agent_type":"aionrs","name":"DCode CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
