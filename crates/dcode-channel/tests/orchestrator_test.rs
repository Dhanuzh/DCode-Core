...
async fn unauthorized_user_gets_pairing_response() {
    ...
    let repo: Arc<dyn dcode_db::IChannelRepository> = Arc::new(dcode_db::SqliteChannelRepository::new(pool.clone()));
    let bus = Arc::new(dcode_realtime::BroadcastEventBus::new(64));

    let pref_repo: Arc<dyn dcode_db::IClientPreferenceRepository> =
        Arc::new(dcode_db::SqliteClientPreferenceRepository::new(pool));
