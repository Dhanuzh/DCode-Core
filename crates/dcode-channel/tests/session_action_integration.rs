...
use dcode_common::{generate_id, now_ms};
use dcode_db::models::AssistantUserRow;
use dcode_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
use dcode_realtime::EventBroadcaster;

use dcode_channel::action::{ActionExecutor, MessageResult};
use dcode_channel::channel_settings::ChannelSettingsService;
use dcode_channel::pairing::PairingService;
use dcode_channel::session::SessionManager;
use dcode_channel::types::{
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn dcode_db::IClientPreferenceRepository> =
        Arc::new(dcode_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
// dcode-db sqlite_channel.rs::update_session_conversation_persists.
