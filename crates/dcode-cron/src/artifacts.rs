...
use dcode_api_types::{ConversationArtifactResponse, WebSocketMessage};
use dcode_common::generate_id;
use dcode_db::ConversationArtifactRow;
...
mod tests {
    ...
    fn builds_skill_suggest_response() {
        ...
        assert_eq!(response.kind, dcode_api_types::ConversationArtifactKind::SkillSuggest);
        assert_eq!(response.status, dcode_api_types::ConversationArtifactStatus::Pending);
    ...
    fn builds_cron_trigger_payload() {
        ...
        let response = artifact_response_from_row(&row).unwrap();
        assert_eq!(response.kind, dcode_api_types::ConversationArtifactKind::CronTrigger);
