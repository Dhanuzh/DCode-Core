...
use dcode_common::{AgentKillReason, AgentType, ConversationSource, ConversationStatus, TimestampMs};
use dcode_conversation::skill_resolver::SkillResolver;
use dcode_conversation::{ConversationError, ConversationService};
use dcode_db::{SqliteConversationRepository, init_database_memory};
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("dcode-conversation-crud-test-project");
...
async fn t4_4_extra_merge_preserves_existing_keys() {
    let (svc, _, task_mgr) = setup().await;
    let dir = std::env::temp_dir().join("dcode-conversation-crud-extra-merge");
...
async fn full_lifecycle_create_get_update_delete() {
    let (svc, broadcaster, task_mgr) = setup().await;
    let updated_workspace = std::env::temp_dir().join("dcode-conversation-crud-updated-workspace");
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use dcode_db::{SqliteAcpSessionRepository, init_database_memory};

    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(dcode_db::SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let agent_metadata_repo: Arc<dyn dcode_db::IAgentMetadataRepository> =
        Arc::new(dcode_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn dcode_db::IAcpSessionRepository> =
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = dcode_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
