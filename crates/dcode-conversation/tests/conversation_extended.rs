...
use dcode_ai_agent::{AgentError, IWorkerTaskManager};
use dcode_api_types::{
    CloneConversationRequest, CreateConversationRequest, ListMessagesQuery, SearchMessagesQuery, WebSocketMessage,
};
use dcode_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_prefixed_id, now_ms};
use dcode_conversation::skill_resolver::SkillResolver;
use dcode_conversation::{ConversationError, ConversationService};
use dcode_db::models::MessageRow;
use dcode_db::{IConversationRepository, SqliteConversationRepository, init_database_memory};
...
impl IWorkerTaskManager for NoopTaskManager {
    fn get_task(&self, _: &str) -> Option<dcode_ai_agent::AgentInstance> {
        None
    }
    async fn get_or_build_task(
        &self,
        _: &str,
        _: dcode_ai_agent::types::BuildTaskOptions,
...
async fn setup() -> (
    ...
    let agent_metadata_repo: Arc<dyn dcode_db::IAgentMetadataRepository> =
        Arc::new(dcode_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn dcode_db::IAcpSessionRepository> =
        Arc::new(dcode_db::SqliteAcpSessionRepository::new(db.pool().clone()));
...
fn ensure_test_workspace_path() -> String {
    ensure_named_workspace_path("dcode-conversation-extended-test-project")
}
...
async fn t10_1_same_workspace() {
    let (svc, _repo, _b) = setup().await;
    let shared_workspace = ensure_named_workspace_path("dcode-conversation-extended-shared-workspace");
    let other_workspace = ensure_named_workspace_path("dcode-conversation-extended-other-workspace");
...
async fn t10_2_no_associated() {
    let (svc, _repo, _b) = setup().await;
    let workspace = ensure_named_workspace_path("dcode-conversation-extended-unique-workspace");
...
async fn setup_fork() -> (
    ConversationService,
    Arc<SqliteConversationRepository>,
    Arc<dyn dcode_db::IAcpSessionRepository>,
) {
    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
...
fn fork_create_req(backend: &str) -> CreateConversationRequest {
    let workspace = ensure_named_workspace_path(&format!("dcode-fork-test-{backend}"));
    serde_json::from_value(json!({
        "type": "acp",
...
fn fork_req(message_id: &str) -> dcode_api_types::ForkConversationRequest {
...
async fn fork_head_creates_lineage_row_and_copies_messages() {
    ...
    assert_eq!(
        ...
        Some(dcode_api_types::ForkCapabilityView { at_turn: false }),
...
async fn fork_mid_history_resolves_codex_turn_anchor() {
    ...
    assert_eq!(
        fork.fork_capability,
        Some(dcode_api_types::ForkCapabilityView { at_turn: true }),
...
async fn create_strips_client_supplied_fork_spec() {
    ...
    let workspace = ensure_named_workspace_path("dcode-fork-test-strip");
    let req: CreateConversationRequest = serde_json::from_value(json!({
        "type": "acp",
        "extra": {
            "workspace": workspace,
...
async fn get_projects_prompt_capability_on_detail_path() {
    ...
    assert_eq!(
        ...
        Some(dcode_api_types::PromptCapabilityView {
            image: true,
            audio: false
        })
    );
...
fn aionrs_create_req() -> CreateConversationRequest {
    let workspace = ensure_named_workspace_path("dcode-fork-test-aionrs");
    serde_json::from_value(json!({
        "type": "aionrs",
