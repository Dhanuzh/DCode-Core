...
use dcode_ai_agent::session_context::{AgentSessionContext, AgentSessionKind};
...
use dcode_api_types::ChatFileRef;
use dcode_api_types::{
...
use dcode_db::models::{AssistantDefinitionRow, ConversationAssistantSnapshotRow, ConversationRow, MessageRow};
use dcode_db::{
...
use dcode_extension::AssistantRuleDispatcher;
use dcode_mcp::{AcpMcpCapabilities, parse_acp_mcp_capabilities};
use dcode_project::{ProjectService, ResolvedChatMessage, canonical};
use dcode_realtime::EventBroadcaster;
use dcode_runtime::{RuntimeCommandProbe, probe_node_runtime_supported, probe_runtime_command, resolve_command_path};
use chrono::Datelike;
...
fn assistant_snapshot_modes<'a>(
    ...
    definition: &'a dcode_db::AssistantDefinitionRow,
...
impl ConversationService {
    ...
    /// into the message content (`[[DCODE_FILES]]` form) at the send boundary.
    ...
    async fn resolve_message_attachments(
        ...
        let upload_root = std::env::temp_dir().join("dcode");
        project
    ...
    /// `delete()` removes the conversation row. Used by `dcode-app` to wire
    ...
    async fn static_supports_midturn_delivery(&self, conversation_id: &str) -> bool {
        ...
        serde_json::from_str::<serde_json::Value>(&row.extra)
            ...
            .is_some_and(dcode_ai_agent::backend_supports_midturn_delivery)
    }
...
impl ConversationService {
    ...
    async fn assistant_identity_from_snapshot(
    ...
    ) -> Result<dcode_api_types::ConversationAssistantIdentityResponse, ConversationError> {
        ...
        Ok(dcode_api_types::ConversationAssistantIdentityResponse {
    ...
    /// to `dcode`, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        let now = now_ms();
        let source = req.source.unwrap_or(ConversationSource::Dcode);
        ...
        let row = dcode_db::models::ConversationRow {
            id: id.clone(),
            user_id: user_id.to_owned(),
        ...
        if let Some(snapshot) = assistant_snapshot.as_ref() {
            response.assistant = Some(dcode_api_types::ConversationAssistantIdentityResponse {
                id: snapshot.assistant_id.clone(),
    ...
    async fn persist_assistant_preferences_from_snapshot(
        ...
        preference_repo
            .upsert_for_user(
                user_id,
                &dcode_db::UpsertAssistantPreferenceParams {
                    assistant_definition_id: &snapshot.assistant_definition_id,
    ...
    pub async fn fork(
        ...
        let latest = self
            ...
            .list_messages_page(
                user_id,
                id,
                &dcode_db::MessagePageParams {
                    limit: 1,
                    direction: dcode_db::MessagePageDirection::InitialLatest,
                },
            )
            .await?;
        ...
        if let Some(obj) = extra.as_object_mut() {
            obj.insert(
                "fork".to_owned(),
                serde_json::to_value(dcode_api_types::ForkSpec {
    ...
    /// when present, else the builtin DCode CLI row resolved through the
    ...
    async fn agent_capabilities_for_agent(
        ...
        let backend = dcode_db::runtime_backend_for_agent(&metadata_row);
        let persisted = metadata_row
            .agent_capabilities
            .as_deref()
            .and_then(|capabilities| serde_json::from_str::<serde_json::Value>(capabilities).ok());
        Ok(dcode_ai_agent::effective_agent_capabilities(
            &backend,
            persisted.as_ref(),
        ))
    }
...
pub(crate) async fn apply_message_receipt(
    ...
    let payload = dcode_api_types::MessageStatusChangedPayload {
        user_id: user_id.to_owned(),
        conversation_id: conversation_id.to_owned(),
...
impl ConversationService {
    ...
    async fn deliver_midturn_message(
        ...
        if persisted {
            let user_msg = dcode_db::models::MessageRow {
        ...
        let data = dcode_ai_agent::types::SendMessageData {
    ...
    pub async fn send_message(
        ...
        // `[[DCODE_FILES]]` content used for persistence, broadcast, and the turn.
        let resolved = self
...
impl ConversationService {
    ...
    fn runtime_token_for_build(
        ...
        // CLI (`dcode-core config` / `diagnose`) can authenticate in AionPro
    ...
    /// workspaces are constrained to DCode's generated path; custom
...
async fn resolve_acp_mcp_support_policy(
    ...
    let effective_backend = row
        .as_ref()
        .map(dcode_db::runtime_backend_for_agent)
        .or_else(|| backend.map(str::to_owned))
        .unwrap_or_default();
    let capabilities = dcode_ai_agent::effective_agent_capabilities(&effective_backend, persisted.as_ref())
        .as_ref()
...
fn classify_repo_mcp_status(row: &dcode_db::models::McpServerRow, support: McpSupportPolicy) -> ConversationMcpStatus {
...
mod tests {
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::Dcode).unwrap(), "dcode");
    ...
    fn response_with_type(agent_type: dcode_common::AgentType) -> ConversationResponse {
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_dcode_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
    ...
    async fn direct_cli_mcp_policy_uses_effective_descriptor_on_a_fresh_database() {
        ...
        let repo: Arc<dyn IAgentMetadataRepository> =
            Arc::new(dcode_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
