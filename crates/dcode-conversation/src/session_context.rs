...
use dcode_ai_agent::shared_kernel::{ConfigKey, ConfigValue, ModeId, ModelId, PersistedSessionState};
use dcode_ai_agent::types::BuildTaskOptions;
use dcode_api_types::{AcpBuildExtra, AionrsBuildExtra, TeamSessionBinding};
use dcode_common::{AgentType, WorkspacePathValidationError, validate_workspace_path_availability};
use dcode_db::models::ConversationRow;
use dcode_db::{IAcpSessionRepository, IAgentMetadataRepository};
use chrono::Datelike;
...
impl<'a> SessionContextBuilder<'a> {
    ...
    async fn resolve_acp_identity(
        ...
        extra: &serde_json::Value,
        session_row: Option<&dcode_db::models::AcpSessionRow>,
...
fn decode_persisted_session_state(state: dcode_db::PersistedSessionState) -> PersistedSessionState {
...
mod tests {
    ...
    fn workspace_validation_failure_logs_redacted_runtime_check() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        let raw_path = "/tmp/dcode-secret-workspace-token-12345";
    ...
    async fn legacy_agent_types_are_archived_before_runtime_context() {
        ...
        for (agent_type, extra) in [
            ...
            ("codex", serde_json::json!({ "workspace": "/tmp/dcode-codex-history" })),
            (
                "openclaw-gateway",
