...
use dcode_common::{AgentType, ConversationStatus, ErrorChain, now_ms};
...
use dcode_api_types::AgentErrorCode;
...
pub(crate) struct TurnStartInput {
    ...
    /// User message content, already resolved to the inlined `[[DCODE_FILES]]`
...
/// `dcode_common::enums`). What cron's forced full-auto — and a resumed full-access
...
async fn apply_required_runtime_mode(
    ...
    // it look-before-leap. This logic is sunk into dcode-ai-agent because the
