...
use dcode_api_types::{ConversationArtifactResponse, ConversationResponse, MessageResponse, MessageSearchItem};
use dcode_common::{
    AgentType, ConversationSource, ConversationStatus, MessagePosition, MessageStatus, MessageType, ProviderWithModel,
};
use dcode_db::MessageSearchRow;
use dcode_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
///
/// DCode stores the full provider object (`TProviderWithModel`) which includes
...
mod tests {
    use super::*;
    use dcode_common::{AgentType, ConversationSource, ConversationStatus};
    use serde_json::json;
    ...
    #[test]
    fn parse_provider_with_model_dcode_format() {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("dcode"),
            None,
            r#"{"workspace":"/srv/dcode-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/dcode-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("dcode"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/dcode-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        let row = MessageSearchRow {
            ...
            conversation_source: Some("dcode".into()),
        ...
        assert_eq!(item.conversation.source, Some(ConversationSource::Dcode));
