...
use dcode_ai_agent::protocol::events::{ErrorEventData, TipType, TipsEventData};
use dcode_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use dcode_api_types::{AgentErrorCode, WebSocketMessage};
use dcode_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
...
use dcode_db::IConversationRepository;
...
impl StreamRelay {
    ...
    async fn consume_inner(
        ...
        info!(
            target: "dcode_feedback_diagnostics",
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let mut outcome = if deleting {
                        ...
                        self.finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(dcode_ai_agent::protocol::events::FinishEventData::default()),
    ...
    fn forward_workflow_progress(&self, data: &dcode_ai_agent::protocol::events::WorkflowProgressData) {
...
mod tests {
    ...
    use dcode_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    use dcode_db::DbError;
    ...
    impl SkillResolver for RecordingSkillResolverForRelay {
        ...
        async fn resolve_skills(&self, _names: &[String]) -> Vec<dcode_extension::ResolvedAgentSkill> {
            Vec::new()
        }
        ...
        async fn link_workspace_skills(
            ...
            _rel_dirs: &[&str],
            _skills: &[dcode_extension::ResolvedAgentSkill],
        ) -> usize {
            0
        }
    }
    ...
    async fn run_text_then_finish_persists_message() {
        ...
        let bus = Arc::new(dcode_realtime::BroadcastEventBus::new(64));
    ...
    fn agent_title_test_row(name: &str, name_source: Option<&str>) -> dcode_db::models::ConversationRow {
    ...
    async fn needs_auth_tip_sets_summary_flag_on_finish() {
        use dcode_ai_agent::protocol::events::{TipType, TipsEventData};
    ...
    async fn workflow_progress_does_not_split_the_streaming_reply() {
        use dcode_ai_agent::protocol::events::tool_call::{ToolGroupEntry, ToolGroupStatus};
        use dcode_ai_agent::protocol::events::{
    ...
    async fn acp_dialect_signal_is_not_forwarded_or_persisted() {
        use dcode_ai_agent::protocol::events::{AcpDialectSignalData, AcpDialectSignalKind};
    ...
    fn stream_relay_emits_feedback_runtime_terminal_log() {
        ...
        assert!(captured.contains("dcode_feedback_diagnostics"), "{captured}");
    ...
    async fn run_warning_tip_with_finish_persists_warning_tip() {
        ...
        tx.send(AgentStreamEvent::Tips(
            dcode_ai_agent::protocol::events::TipsEventData {
                content: String::new(),
                tip_type: dcode_ai_agent::protocol::events::TipType::Warning,
                code: Some("ACP_EMPTY_TURN".into()),
                params: None,
                supersedes_key: None,
            },
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(dcode_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use dcode_ai_agent::protocol::events::tool_call::{
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(dcode_realtime::BroadcastEventBus::new(64));
        let adapter =
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, dcode_db::MessageRowUpdate)>>,
        /// Optional conversation row served by `get` (agent-title tests).
        conversation: Mutex<Option<dcode_db::models::ConversationRow>>,
        /// Conversation-level updates recorded by `update` (agent-title tests).
        conversation_updates: Mutex<Vec<(String, dcode_db::ConversationRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, dcode_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _user_id: &str, _id: &str) -> Result<Option<dcode_db::models::ConversationRow>, DbError> {
            Ok(self.conversation.lock().unwrap().clone())
        }
        async fn owner_user_id(&self, _id: &str) -> Result<Option<String>, DbError> {
            Ok(Some("user-1".into()))
        }
        async fn create(&self, _row: &dcode_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(
            &self,
            _user_id: &str,
            id: &str,
            updates: &dcode_db::ConversationRowUpdate,
        ) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &dcode_db::ConversationFilters,
        ) -> Result<dcode_common::PaginatedResult<dcode_db::models::ConversationRow>, DbError> {
            Ok(dcode_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<dcode_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<dcode_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            ...
            _user_id: &str,
            _conv_id: &str,
            _params: &dcode_db::MessagePageParams,
        ) -> Result<dcode_db::MessagePageResult, DbError> {
            Ok(dcode_db::MessagePageResult {
                items: vec![],
        ...
        async fn update_message(
            ...
            _conversation_id: &str,
            id: &str,
            updates: &dcode_db::MessageRowUpdate,
        ) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<dcode_common::PaginatedResult<dcode_db::MessageSearchRow>, DbError> {
            Ok(dcode_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
