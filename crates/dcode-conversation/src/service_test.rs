...
use dcode_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
use dcode_ai_agent::protocol::events::{AgentStreamEvent, ErrorEventData, FinishEventData, TextEventData};
use dcode_ai_agent::types::{
    DCODE_BASE_URL_ENV, DCODE_HELPER_BIN_ENV, DCODE_RUNTIME_TOKEN_ENV, BuildTaskOptions,
...
use dcode_db::{MessagePageCursor, MessagePageDirection, MessagePageParams, MessagePageResult};
use dcode_extension::{AssistantRuleDispatcher, ExtensionError};
...
impl IConversationRepository for MockRepo {
    async fn get(&self, user_id: &str, id: &str) -> Result<Option<ConversationRow>, dcode_db::DbError> {
    ...
    async fn create(&self, row: &ConversationRow) -> Result<(), dcode_db::DbError> {
        self.rows.lock().unwrap().push(row.clone());
        Ok(())
    }

    async fn update(&self, user_id: &str, id: &str, updates: &ConversationRowUpdate) -> Result<(), dcode_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let row = rows
            .iter_mut()
            .find(|r| r.user_id == user_id && r.id == id)
            .ok_or_else(|| dcode_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, user_id: &str, id: &str) -> Result<(), dcode_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let len_before = rows.len();
        rows.retain(|r| !(r.user_id == user_id && r.id == id));
        if rows.len() == len_before {
            return Err(dcode_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, dcode_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, dcode_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, dcode_db::DbError> {
        Ok(vec![])
    }
        ...
        let items = match &params.direction {
            ...
            MessagePageDirection::Anchor { message_id } => {
                let anchor = matched
                    ...
                    .cloned()
                    .ok_or_else(|| dcode_db::DbError::NotFound(format!("Message {message_id}")))?;
    ...
    async fn insert_message(&self, _user_id: &str, message: &MessageRow) -> Result<(), dcode_db::DbError> {
    ...
    async fn update_message(
        ...
        let message = messages
            ...
            .ok_or_else(|| dcode_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, _user_id: &str, conv_id: &str) -> Result<(), dcode_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, dcode_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_stale_runtime_messages(&self) -> Result<Vec<dcode_db::StaleRuntimeMessageRow>, dcode_db::DbError> {
        let messages = self.messages.lock().unwrap();
        let rows = self.rows.lock().unwrap();
        Ok(messages
            .iter()
            ...
            .filter_map(|message| {
                ...
                Some(dcode_db::StaleRuntimeMessageRow {
                    user_id,
    ...
    async fn list_artifacts(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<ConversationArtifactRow>, dcode_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, dcode_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        ...
        _user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, dcode_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
fn stub_agent_metadata_rows() -> Vec<AgentMetadataRow> {
    [
        ...
        ("632f31d2", None, "aionrs", "DCode CLI", 200),
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("dcode-conversation-service-test-project");
...
fn unique_test_workspace_path(label: &str) -> PathBuf {
    let workspace = std::env::temp_dir()
        .join(format!("dcode-conversation-service-test-{label}"))
...
async fn insert_conversation_with_type(repo: &Arc<MockRepo>, user_id: &str, agent_type: AgentType) -> ConversationRow {
    let id = format!(
        ...
        dcode_common::generate_short_id()
    );
...
async fn get_row(repo: &Arc<MockRepo>, user_id: &str, id: &str) -> ConversationRow {
    use dcode_db::IConversationRepository as _;
...
async fn create_returns_conversation_with_defaults() {
    ...
    assert_eq!(resp.source, Some(ConversationSource::Dcode));
    assert!(!resp.pinned);
    ...
    assert_eq!(events[0].data["source"], "dcode");
}

// ── Project-bind side branch tests ─────────────────────────────────

async fn make_injected_project_service(temp_root: &std::path::Path) -> std::sync::Arc<dcode_project::ProjectService> {
    ...
    .await
    .unwrap();
    let store: std::sync::Arc<dyn dcode_db::IProjectStore> =
        std::sync::Arc::new(dcode_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
    std::sync::Arc::new(dcode_project::ProjectService::new(
        store,
...
async fn create_auto_provisions_workspace_under_date_partition() {
    ...
    let workspace_root = temp.path().join("dcode-data");
...
async fn create_rejects_unavailable_workspace_with_trailing_whitespace_in_request() {
    let (svc, _broadcaster, _repo, _task_mgr) = make_service();
    let dir = std::env::temp_dir().join(format!("dcode-test-{}", dcode_common::generate_short_id()));
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, dcode_api_types::ConversationRuntimeStateKind::Idle);
...
async fn update_extra_merge() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "dcode-conversation-update-extra-merge-{}",
        dcode_common::generate_short_id()
    ));
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            dcode_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
fn assert_conversation_runtime_context(options: &BuildTaskOptions, user_id: &str, conversation_id: &str) {
    assert!(
        options
            .context
            .runtime_env
            .contains(&("DCODE_USER_ID".to_owned(), user_id.to_owned())),
        "runtime env should include DCODE_USER_ID"
    );
    assert!(
        options
            .context
            .runtime_env
            .contains(&("DCODE_CONVERSATION_ID".to_owned(), conversation_id.to_owned())),
        "runtime env should include DCODE_CONVERSATION_ID"
    );
    assert_eq!(
...
impl IAgentTask for MidturnMockAgent {
    ...
    fn last_activity_at(&self) -> dcode_common::TimestampMs {
        dcode_common::now_ms()
    }
    fn subscribe(&self) -> broadcast::Receiver<AgentStreamEvent> {
        self.event_tx.subscribe()
...
async fn send_message_injects_configured_runtime_helper_context() {
    ...
    let svc = svc.with_runtime_helper_context(
        "/Applications/DCode/aioncore".to_owned(),
    ...
    assert!(
        options[0].context.runtime_env.contains(&(
            DCODE_HELPER_BIN_ENV.to_owned(),
            "/Applications/DCode/aioncore".to_owned()
        )),
        "runtime env should include DCODE_HELPER_BIN"
    );
    assert!(
        options[0]
            .context
            .runtime_env
            .contains(&(DCODE_BASE_URL_ENV.to_owned(), "http://127.0.0.1:51234".to_owned())),
        "runtime env should include DCODE_BASE_URL"
    );
}
...
async fn send_message_missing_workspace_persists_message_and_failure_tip() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    broadcaster.take_events();
    let legacy_workspace = format!("/tmp/does-not-exist-{}", dcode_common::generate_short_id());
    repo.update(
        "user_1",
        &conv.id,
        &ConversationRowUpdate {
...
async fn send_message_records_agent_availability_feedback_on_send_failure() {
    ...
    create_req.source = Some(ConversationSource::Dcode);
...
async fn warmup_injects_runtime_token_for_mcp_team_conversation() {
    ...
    assert!(
        options[0]
            ...
            .runtime_env
            .iter()
            .any(|(key, value)| key == DCODE_RUNTIME_TOKEN_ENV && !value.is_empty()),
        "MCP Team conversations should receive DCODE_RUNTIME_TOKEN for CLI fallback"
    );
}
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = dcode_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
async fn create_resolves_assistant_snapshot_and_updates_preferences() {
    ...
    assert_eq!(
        resp.assistant,
        Some(dcode_api_types::ConversationAssistantIdentityResponse {
            id: "preset-1".into(),
            source: "builtin".into(),
            name: "Preset".into(),
            avatar: "🤖".into(),
...
async fn seed_aionrs_conversation_with_snapshot(
    ...
    let row = ConversationRow {
        id: format!("aionrs-seed-{}", dcode_common::generate_short_id()),
        user_id: "user_1".into(),
