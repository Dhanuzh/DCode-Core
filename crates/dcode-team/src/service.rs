...
use dcode_ai_agent::{ActiveLeaseRegistry, AgentError, AgentInstance, IWorkerTaskManager, IdleCleanupCoordinator};
...
use dcode_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_id, now_ms};
use dcode_db::models::TeamRow;
use dcode_db::{
...
use dcode_project::{ProjectService, canonical};
...
impl TeamSessionService {
    ...
    pub async fn get_run_state(&self, user_id: &str, team_id: &str) -> Result<TeamRunStateResponse, TeamError> {
        ...
        let active_run = session.team_run_manager().current_payload(&snapshot).filter(|run| {
            matches!(
                run.status,
                dcode_api_types::TeamRunStatus::Accepted
                    | dcode_api_types::TeamRunStatus::Running
                    | dcode_api_types::TeamRunStatus::Cancelling
            )
        });
    ...
    pub async fn resolve_team_tool_context(
        ...
        let allowed_tools = dcode_api_types::team_tool_descriptors_for_role(role_to_tool_role(agent.role))
            .into_iter()
    ...
    /// the content (`[[DCODE_FILES]]` form) at the team send boundary. Atomic;
...
mod tests {
    ...
    use dcode_api_types::{AddAgentRequest, ConfigOptionConfirmation, SetConfigOptionResponse};
    use dcode_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
    use dcode_db::{IConversationRepository, ITeamRepository};
    ...
    fn two_agent_team_request(name: &str) -> dcode_api_types::CreateTeamRequest {
        dcode_api_types::CreateTeamRequest {
            name: name.into(),
            agents: vec![
                dcode_api_types::TeamAgentInput {
                    name: "Lead".into(),
                    role: "lead".into(),
    ...
    fn team_with_aionrs_worker_request(name: &str) -> dcode_api_types::CreateTeamRequest {
        let mut request = two_agent_team_request(name);
        request.agents.push(dcode_api_types::TeamAgentInput {
            name: "Butler".into(),
            role: "teammate".into(),
