...
use dcode_ai_agent::IWorkerTaskManager;
use dcode_api_types::{AddAgentRequest, GetConfigOptionsResponse, TeamAgentInput, TeamToolTransport};
use dcode_common::{AgentKillReason, AgentType, ProviderWithModel, generate_id};
use dcode_db::models::{AgentMetadataRow, TeamRow};
use dcode_db::{IAgentMetadataRepository, IProviderRepository, ITeamRepository, UpdateTeamParams};
...
impl TeamAgentProvisioner {
    ...
    async fn resolve_team_tool_transport(
        ...
        user_id: &str,
        agent: &TeamAgent,
    ) -> Result<(TeamToolTransport, dcode_common::ResolvedBackendCapabilities), TeamError> {
...
mod tests {
    ...
    use dcode_ai_agent::{AgentError, AgentInstance};
    use dcode_db::models::{
        AgentMetadataRow, Provider, UpdateAgentAvailabilitySnapshotParams, UpdateAgentHandshakeParams,
        UpsertAgentMetadataParams,
    };
    use dcode_db::{CreateProviderParams, DbError, UpdateProviderParams};
    use std::sync::Mutex;
    ...
    impl TeamToolCapabilityPort for TestCapabilityPort {
        ...
        ) -> Result<dcode_common::ResolvedBackendCapabilities, TeamError> {
            let direct = matches!(backend, "claude" | "codex" | "antigravity");
            let internal = backend == "aionrs";
            Ok(dcode_common::ResolvedBackendCapabilities {
                mcp: dcode_common::McpTransportCapabilities {
                ...
                origin: if direct {
                    ...
                    dcode_common::CapabilityOrigin::InternalDescriptor
                } else {
                    dcode_common::CapabilityOrigin::Unknown
                },
            })
        }
    }
    ...
    fn test_mcp_config() -> TeamMcpStdioConfig {
        TeamMcpStdioConfig {
            ...
            token: "token".into(),
            slot_id: "slot-1".into(),
            binary_path: "/tmp/dcode-core".into(),
        }
    }
