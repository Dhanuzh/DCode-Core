use dcode_common::now_ms;
use dcode_db::models::{MailboxMessageRow, TeamRow, TeamTaskRow};
use dcode_db::{ActivityCursor, DbError, ITeamRepository, PageDirection, UpdateTaskParams, UpdateTeamParams};
use std::sync::Mutex;
...
pub(crate) mod workspace_harness {
    ...
    use dcode_common::{AgentKillReason, AgentType, PaginatedResult, now_ms};
    use dcode_db::models::{
    ...
    impl ITeamRepository for FullMockTeamRepo {
        ...
        async fn write_message(
            &self,
            _user_id: &str,
            _row: &dcode_db::models::MailboxMessageRow,
        ) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_task(
            ...
            _team_id: &str,
            _task_id: &str,
            _params: &dcode_db::UpdateTaskParams,
        ) -> Result<(), DbError> {
            Ok(())
        }
    ...
    impl IWorkerTaskManager for NoopTaskManager {
        fn get_task(&self, _conversation_id: &str) -> Option<dcode_ai_agent::AgentInstance> {
            None
        }
        ...
        async fn get_or_build_task(
            &self,
            _conversation_id: &str,
            _options: dcode_ai_agent::types::BuildTaskOptions,
        ...
        fn collect_idle(&self, _idle_threshold_ms: dcode_common::TimestampMs) -> Vec<String> {
            vec![]
        }
    }
    ...
    pub(crate) fn setup_with_factory_metadata_team_repo_conversation_repo_broadcaster_and_task_manager(
        ...
        let svc = TeamSessionService::new(
            ...
            Arc::new(std::path::PathBuf::from("/tmp/dcode-core-test")),
        );
    ...
    pub(crate) fn single_agent_team_request(name: &str) -> CreateTeamRequest {
        CreateTeamRequest {
            name: name.into(),
            agents: vec![dcode_api_types::TeamAgentInput {
                name: "Lead".into(),
                role: "lead".into(),
