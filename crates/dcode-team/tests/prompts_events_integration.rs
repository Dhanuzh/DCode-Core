...
use dcode_team::events::TeamEventEmitter;
use dcode_team::message_projection::{
    ProjectedTeamMessage, TeamMessageProjection, TeamProjectionMessageStore, TeamProjectionRequest,
    TeamProjectionSource,
};
use dcode_team::prompts::{AvailableAssistant, build_lead_prompt, build_teammate_prompt, build_wake_payload};
use dcode_team::types::{
    MailboxMessage, MailboxMessageType, TaskStatus, TeamAgent, TeamTask, TeammateRole, TeammateStatus,
};
use dcode_team::visibility::TeamVisibilityPolicy;
use dcode_team::{Mailbox, TaskBoard, TeammateManager};
...
fn lp2_lead_prompt_contains_tool_descriptions() {
    ...
    // DCode lead prompt references the `team_*` coordination tools that the
