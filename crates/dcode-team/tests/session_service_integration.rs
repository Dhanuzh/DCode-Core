...
use dcode_ai_agent::session_context::{
    AcpSessionBuildContext, AgentSessionContext, AgentSessionKind, ConversationContext, WorkspaceContext,
};
use dcode_ai_agent::task_manager::AgentFactory;
use dcode_ai_agent::types::BuildTaskOptions;
use dcode_ai_agent::{ActiveLeaseRegistry, AgentError, IWorkerTaskManager, WorkerTaskManagerImpl};
use dcode_api_types::{
...
impl FakeConversationPorts {
    fn new(repo: Arc<MockConversationRepo>) -> Self {
        let workspace_root =
            std::env::temp_dir().join(format!("dcode-team-fake-workspaces-{}", dcode_common::generate_id()));
        Self {
            repo,
            workspace_root,
...
impl TeamConversationProvisioningPort for FakeConversationPorts {
    ...
    ) -> Result<TeamConversationCreateResult, dcode_team::TeamError> {
        if request.assistant_id.is_some() && request.agent_type.is_some() {
            return Err(dcode_team::TeamError::InvalidRequest(
                "assistant-backed team conversations must not provide agent_type".into(),
            ));
        }
        let id = dcode_common::generate_id();
    ...
    async fn conversation_workspace(&self, conversation_id: &str) -> Result<Option<String>, dcode_team::TeamError> {
    ...
    async fn conversation_assistant_id(&self, conversation_id: &str) -> Result<Option<String>, dcode_team::TeamError> {
        Ok(self.repo.get_extra(conversation_id).and_then(|extra| {
            extra
                .get("assistant_id")
                .or_else(|| extra.get("preset_assistant_id"))
    ...
    async fn create_team_temp_workspace(
        ...
        _user_id: &str,
        team_id: &str,
    ) -> Result<String, dcode_team::TeamError> {
        if self
    ...
    async fn patch_runtime_config(
        ...
        let user_id = self
            ...
            .await?
            .ok_or_else(|| dcode_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
        self.repo
            .update(
                &user_id,
                conversation_id,
                &ConversationRowUpdate {
                    ...
                    status: None,
                    updated_at: Some(dcode_common::now_ms()),
                    project_id: None,
                    folder_id: None,
                    name_source: None,
                },
    ...
    async fn save_acp_runtime_mode(&self, conversation_id: &str, mode: &str) -> Result<(), dcode_team::TeamError> {
    ...
    async fn get_config_options(
        &self,
        conversation_id: &str,
    ) -> Result<GetConfigOptionsResponse, dcode_team::TeamError> {
        let extra = self
            .repo
            .get_extra(conversation_id)
            .ok_or_else(|| dcode_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
    ...
    async fn warmup_agent_process(
        ...
        let row = self.repo.get(user_id, conversation_id).await?.ok_or_else(|| {
            dcode_team::TeamError::InvalidRequest(format!("conversation not found: {conversation_id}"))
        })?;
        let extra: serde_json::Value = serde_json::from_str(&row.extra)?;
        let team = dcode_api_types::TeamSessionBinding::from_extra_value(&extra)?;
        ...
        task_manager
            ...
            .await
            .map_err(|error| {
                dcode_team::TeamError::InvalidRequest(format!("failed to warm up agent process: {error}"))
            })?;
        Ok(())
    }
...
impl TeamProjectionMessageStore for FakeConversationPorts {
    ...
    async fn find_projected_message(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, dcode_team::TeamError> {
    ...
    async fn insert_projected_message(&self, row: &MessageRow) -> Result<(), dcode_team::TeamError> {
        let user_id = self
            .repo
            .owner_user_id(&row.conversation_id)
            .await?
            .ok_or_else(|| dcode_team::TeamError::InvalidRequest("conversation not found".into()))?;
...
impl TeamConversationLookupPort for FakeConversationPorts {
    async fn lookup_team_binding_by_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<TeamConversationBindingLookup>, dcode_team::TeamError> {
        let Some(user_id) = self.repo.owner_user_id(conversation_id).await? else {
            return Ok(None);
        };
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<dcode_db::models::TeamRow>>,
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &dcode_db::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams_for_restore(&self) -> Result<Vec<dcode_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn list_teams_by_user(&self, user_id: &str) -> Result<Vec<dcode_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team(&self, user_id: &str, id: &str) -> Result<Option<dcode_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team_for_restore(&self, id: &str) -> Result<Option<dcode_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().iter().find(|t| t.id == id).cloned())
    }
    async fn update_team(&self, user_id: &str, id: &str, params: &dcode_db::UpdateTeamParams) -> Result<(), DbError> {
        ...
        team.updated_at = dcode_common::now_ms();
        Ok(())
    }
    ...
    async fn write_message(&self, user_id: &str, row: &dcode_db::models::MailboxMessageRow) -> Result<(), DbError> {
    ...
    async fn read_unread_and_mark(
        ...
        team_id: &str,
        to_agent_id: &str,
    ) -> Result<Vec<dcode_db::models::MailboxMessageRow>, DbError> {
    ...
    async fn list_messages_by_ids(&self, ids: &[String]) -> Result<Vec<dcode_db::models::MailboxMessageRow>, DbError> {
    ...
    async fn create_task(&self, user_id: &str, row: &dcode_db::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<dcode_db::models::TeamTaskRow>, DbError> {
    ...
    async fn update_task(
        ...
        team_id: &str,
        task_id: &str,
        params: &dcode_db::UpdateTaskParams,
    ) -> Result<(), DbError> {
        self.inner.update_task(user_id, team_id, task_id, params).await
    }
    async fn list_tasks(&self, user_id: &str, team_id: &str) -> Result<Vec<dcode_db::models::TeamTaskRow>, DbError> {
    ...
    async fn list_tasks_by_ids(
        ...
        team_id: &str,
        ids: &[String],
    ) -> Result<Vec<dcode_db::models::TeamTaskRow>, DbError> {
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<dcode_ai_agent::AgentInstance> {
    ...
    ) -> Result<dcode_ai_agent::AgentInstance, AgentError> {
    ...
    fn collect_idle(&self, idle_threshold_ms: dcode_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    use dcode_ai_agent::AgentError;
    use dcode_ai_agent::agent_task::{IAgentTask, IMockAgent};
    use dcode_ai_agent::protocol::events::AgentStreamEvent;
    use dcode_ai_agent::types::SendMessageData;
    use dcode_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs};
    ...
    impl IAgentTask for MockAgent {
        ...
        async fn send_message(&self, _data: SendMessageData) -> Result<(), dcode_ai_agent::AgentSendError> {
            Ok(())
        }
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(dcode_ai_agent::AgentInstance::Mock(Arc::new(
...
fn test_acp_build_options(conversation_id: String, workspace: String) -> BuildTaskOptions {
    BuildTaskOptions::new(AgentSessionContext {
        conversation: ConversationContext {
            conversation_id,
            user_id: "user1".into(),
            agent_type: dcode_common::AgentType::Acp,
            source: None,
        },
        workspace: WorkspaceContext {
            path: workspace.clone(),
...
impl IProviderRepository for EmptyProviderRepo {
    async fn list(&self, _user_id: &str) -> Result<Vec<dcode_db::models::Provider>, DbError> {
        Ok(vec![])
    }
    async fn find_by_id(&self, _user_id: &str, _id: &str) -> Result<Option<dcode_db::models::Provider>, DbError> {
        Ok(None)
    }
    async fn create(
        &self,
        _params: dcode_db::CreateProviderParams<'_>,
    ) -> Result<dcode_db::models::Provider, DbError> {
    ...
    async fn update(
        ...
        _user_id: &str,
        _id: &str,
        _params: dcode_db::UpdateProviderParams<'_>,
    ) -> Result<dcode_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
...
async fn recovery_creates_system_run_intents_without_restoring_old_memory_run() {
    ...
    team_repo
        .write_message(
            "user1",
            &dcode_db::models::MailboxMessageRow {
...
async fn renew_active_lease_allows_empty_team_without_unrelated_lease() {
    ...
    team_repo
        .create_team(&dcode_db::models::TeamRow {
            id: "team-empty".into(),
            user_id: "user1".into(),
            name: "Empty".into(),
            ...
            created_at: dcode_common::now_ms(),
            updated_at: dcode_common::now_ms(),
            project_id: None,
            folder_id: None,
        })
        .await
...
async fn create_team_with_workspace_writes_same_workspace_to_team_and_initial_agents() {
    ...
    let workspace_dir =
        std::env::temp_dir().join(format!("dcode-team-user-workspace-{}", dcode_common::generate_id()));
...
async fn create_team_side_branch_backfills_project_binding_when_injected() {
    ...
    .await
    .unwrap();
    let store: Arc<dyn dcode_db::IProjectStore> = Arc::new(dcode_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
    svc.with_project_service(Arc::new(dcode_project::ProjectService::new(
        store,
        std::env::temp_dir().join("dcode-team-bind-test-conversations"),
    )));

    let workspace_dir = std::env::temp_dir().join(format!("dcode-team-bind-{}", dcode_common::generate_id()));
    std::fs::create_dir_all(&workspace_dir).unwrap();
...
async fn tg3_get_team_rejects_cross_user_access() {
    ...
    assert!(matches!(result, Err(dcode_team::TeamError::TeamNotFound(_))));
}
...
async fn aa_add_agent_inherits_team_workspace() {
    ...
    let (svc, _, conv_repo) =
        setup_with_factory_and_metadata_and_conversation_repo(success_factory(), agent_metadata_repo);
    let workspace = std::env::temp_dir().join(format!("dcode-team-workspace-{}", dcode_common::generate_id()));
...
async fn add_agent_uses_team_temp_workspace_when_team_and_leader_workspaces_are_unusable() {
    ...
    conv_repo
        .patch_extra(
            ...
            serde_json::json!({ "workspace": "/tmp/dcode-team-missing-leader-workspace" }),
        )
        .unwrap();
...
async fn d31b2_ensure_session_broadcasts_failed_loading_team_for_missing_team() {
    ...
    assert!(matches!(err, dcode_team::TeamError::TeamNotFound(_)));
...
fn activity_team_row(id: &str, user_id: &str) -> dcode_db::models::TeamRow {
    dcode_db::models::TeamRow {
        id: id.into(),
...
fn activity_message_row(id: &str, team_id: &str, created_at: i64) -> dcode_db::models::MailboxMessageRow {
    dcode_db::models::MailboxMessageRow {
        id: id.into(),
...
fn activity_task_row(id: &str, team_id: &str, created_at: i64) -> dcode_db::models::TeamTaskRow {
    dcode_db::models::TeamTaskRow {
        id: id.into(),
        team_id: team_id.into(),
...
async fn activity_all_merges_and_orders_desc_with_cursor() {
    ...
    let page = svc
        .list_team_activity(
            ...
            "t1",
            None,
            dcode_db::PageDirection::Desc,
            dcode_team::ActivityKind::All,
            3,
        )
        .await
        .unwrap();
...
async fn activity_kind_task_only_paginates_tasks() {
    ...
    let page = svc
        .list_team_activity(
            ...
            None,
            dcode_db::PageDirection::Desc,
            dcode_team::ActivityKind::Task,
            10,
        )
        .await
        .unwrap();
    assert_eq!(page.items.len(), 1);
    assert!(matches!(page.items[0].kind, dcode_api_types::TeamActivityKind::Task));
}
