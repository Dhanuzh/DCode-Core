...
use dcode_ai_agent::protocol::events::{AgentStreamEvent, FinishEventData};
use dcode_ai_agent::shared_kernel::approval_key;
use dcode_ai_agent::types::{BuildTaskOptions, SendMessageData};
use dcode_api_types::AgentModeResponse;
use dcode_api_types::WebSocketMessage;
use dcode_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs, now_ms};
use dcode_db::ITeamRepository;
use dcode_realtime::EventBroadcaster;
use dcode_team::event_loop::AgentLoopContext;
use dcode_team::mcp::protocol::{read_frame, write_frame};
...
use dcode_team::service::TeamSessionService;
use dcode_team::{TeamAgent, TeamProjectionMessageStore, TeamSession, TeammateRole};
...
impl TeamProjectionMessageStore for NoopProjectionStore {
    ...
    async fn find_projected_message(
        ...
        _msg_id: &str,
        _msg_type: &str,
    ) -> Result<Option<dcode_db::models::MessageRow>, dcode_team::TeamError> {
        Ok(None)
    }

    async fn insert_projected_message(
        &self,
        _row: &dcode_db::models::MessageRow,
...
impl IAgentTask for RecordingAgent {
    ...
    async fn send_message(&self, data: SendMessageData) -> Result<(), dcode_ai_agent::AgentSendError> {
        ...
        match &self.fail_with {
            Some(msg) => Err(dcode_ai_agent::AgentSendError::from_agent_error(AgentError::internal(
                msg.clone(),
            ))),
            None => Ok(()),
        }
...
#[async_trait]
impl dcode_ai_agent::IWorkerTaskManager for StubTaskManager {
...
async fn setup_session_with_turn_recorder_inner(
    ...
    let task_manager_dyn: Arc<dyn dcode_ai_agent::IWorkerTaskManager> = task_manager.clone();
    ...
    let team = dcode_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
...
async fn setup_session_with_runtime_ports(
    ...
    let task_manager_dyn: Arc<dyn dcode_ai_agent::IWorkerTaskManager> = task_manager;
    let team = dcode_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
        workspace: "/tmp/e2e-team".into(),
...
async fn s3a_on_agent_finish_writes_idle_notification_to_lead() {
    ...
    session
        .scheduler()
        .set_status("worker-1", dcode_team::TeammateStatus::Working)
        .await
        .unwrap();
    ...
    assert_eq!(
        worker_status,
        dcode_team::TeammateStatus::Idle,
...
async fn s3b_lead_finish_does_not_write_idle_notification() {
    ...
    session
        .scheduler()
        .set_status("lead-1", dcode_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3c_finish_triggers_lead_wake_with_idle_notification() {
    ...
    session
        ...
        .write(
            ...
            "worker-1",
            "lead-1",
            dcode_team::MailboxMessageType::Message,
            "do the work",
            None,
        )
        .await
...
async fn one_notify_drains_five_intents_after_busy_turn() {
    ...
    for index in 1..=5 {
        ...
        assert_eq!(ack.enqueue_status, dcode_api_types::TeamMessageEnqueueStatus::Queued);
...
async fn s10_error_finish_sets_agent_status_to_error() {
    ...
    assert_eq!(
        status,
        dcode_team::TeammateStatus::Error,
