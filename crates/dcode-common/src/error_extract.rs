...
//! backends. It lives in `dcode-common` (below every consumer: `dcode-session`,
//! `dcode-conversation`, the agent-* crates all depend on common) so the
//! consuming backends gain no upward crate dependency. Ported verbatim from the
//! deleted `dcode-ai-agent::manager::acp::stderr_error_extractor` (git
