//! `dcode-process` — self-contained subprocess mechanism (feature 001).
//!
...
//! existing `CliAgentProcess` / process registry in `dcode-ai-agent`.
//!
//! "Bytes not semantics": it never parses agent output, holds no session
//! state, and never mutates `std::env`. It depends only on `dcode-common`
//! and `dcode-runtime`.
//!
//! ## Isolation contract (why two mechanisms coexist without conflict)
//! All shared resources are namespaced under `{data_dir}/runtime/dcode-process/`
