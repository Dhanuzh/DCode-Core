...
use dcode_process::{Containment, ContainmentKillOutcome, ProcessGroupContainment};
...
fn a_tool_child_that_left_the_group_is_still_reaped() {
    let marker = std::env::temp_dir().join(format!("dcode-reap-{}.pid", std::process::id()));
...
fn an_unrelated_process_is_left_running() {
    ...
    let marker = std::env::temp_dir().join(format!("dcode-bystander-{}.pid", std::process::id()));
    let marker_s = marker.to_string_lossy().to_string();
    let _ = std::fs::remove_file(&marker);
...
async fn managed_process_kill_reaps_a_child_that_left_the_group() {
    ...
    use dcode_process::ManagedProcess;

    let marker = std::env::temp_dir().join(format!("dcode-mpkill-{}.pid", std::process::id()));
    let marker_s = marker.to_string_lossy().to_string();
    let _ = std::fs::remove_file(&marker);
