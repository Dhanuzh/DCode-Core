...
DCode Core is the backend server for DCode, built with Rust (Axum + Tokio + SQLite).
It provides HTTP REST APIs and WebSocket real-time events for the DCode desktop client.
...
│  tion    │          │          │                 │
├──────────┴──────────┴──────────┴────────────────┤
│   dcode-auth          dcode-realtime           │
│  (JWT, CSRF, middleware) (WebSocket, events)     │
├─────────────────────────────────────────────────┤
│  dcode-db    dcode-api-types   dcode-runtime  │
│ (repositories) (API contracts)  (runtime/process)│
├─────────────────────────────────────────────────┤
│       dcode-common          dcode-assets       │
│  (error types, enums, crypto)  (embedded data)   │
└─────────────────────────────────────────────────┘
```

Dependencies flow strictly downward. Domain crates must not depend on dcode-app,
and dcode-common has zero internal dependencies.
...
|-------|----------------|
| `dcode-common` | Shared error types (ApiError), enums, ID generation, crypto utilities, timestamps, pagination |
| `dcode-api-types` | All HTTP/WebSocket request and response types — the single source of truth for API contracts |
| `dcode-db` | SQLite database layer, defines Repository traits and implementations |
| `dcode-assets` | Embedded static assets (agent metadata, prompts) |
| `dcode-runtime` | Managed Node, subprocess spawning, PATH enhancement |
| `dcode-process` | Supervised subprocess lifecycle, containment, and startup cleanup for direct CLI sessions |
...
| Crate | Responsibility |
|-------|----------------|
| `dcode-auth` | JWT authentication, password hashing, CSRF protection, cookie management, auth middleware |
| `dcode-realtime` | WebSocket connection management, event broadcasting (BroadcastEventBus), message routing |
...
| Crate | Responsibility |
|-------|----------------|
| `dcode-conversation` | Conversation management, messaging, confirmations, streaming responses |
| `dcode-session` | Unified session state, capabilities, commands, and events across direct CLI and ACP backends |
| `dcode-channel` | Multi-channel integration (WeChat, DingTalk, Lark), plugin system, pairing sessions |
| `dcode-team` | Team collaboration, task scheduling, mailbox system |
| `dcode-team-prompts` | Shared team role prompts, governance rules, and tool authorization metadata |
| `dcode-cron` | Scheduled job execution, cron expressions, event triggering |
| `dcode-file` | File operations, watching, snapshots, git operations, compression |
| `dcode-project` | Project/folder bindings, Project Explorer, filesystem monitoring, and resource containment |
| `dcode-office` | Office document handling (Excel, PPT, Word), preview, conversion |
| `dcode-system` | System settings, provider management, version checking, model fetching |
| `dcode-mcp` | MCP protocol integration, OAuth, multi-platform adapters |
| `dcode-ai-agent` | Agent lifecycle management, worker task queues, ACP/auxiliary skills |
| `dcode-extension` | Extension registry, hub management, skill discovery and installation |
| `dcode-shell` | Shell command execution, speech-to-text |
| `dcode-assistant` | Assistant configuration and management |

### Composition

| Crate | Responsibility |
|-------|----------------|
| `dcode-app` | Top-level binary entry point, assembles all crates into the Axum server |
...
Every domain crate follows a consistent internal organization. Using dcode-conversation as a reference:

### Standard Directory Structure

```
crates/dcode-conversation/src/
...
All response types are defined in `dcode-api-types` — the single source of truth for API contracts.
...
the `dcode-ai-agent` translation boundary. That boundary keeps WebSocket and
...
All database access goes through trait abstractions defined in `dcode-db`:
...
|------|----------|---------|---------|
| Row models | `dcode-db/src/models/` | Database row mapping | `ConversationRow` |
| Params objects | `dcode-db/src/repository/` | Database write parameters | `UpdateConversationParams` |
| Request/response types | `dcode-api-types` | API contracts and shared DTOs | `CreateConversationRequest`, `ConversationResponse` |

**The service layer may directly use types from `dcode-api-types`.** This crate contains
pure data structure definitions with no HTTP framework dependencies, essentially serving as a shared DTO layer.

⚠️ **Critical constraint: `dcode-api-types` must not depend on axum, tower, or any HTTP framework.
...
- **Repository (dcode-db):** Pure database operations, no business logic
...
- Migration files are located in `crates/dcode-db/migrations/`
...
DbError (database layer)
  ↓ From trait implementation (dcode-db/src/error.rs)
...
`dcode-app` defines `AppServices`, which holds all shared dependencies centrally:
...
- **Domain crates do not construct their own dependencies** — they only define what they need (RouterState), `dcode-app` handles assembly
...
- Validity: 24 hours
- Payload: `user_id`, `username`, `iat`, `exp`, `iss` ("dcode"), `aud` ("dcode-webui")
- Secret source priority: environment variable → database → random generation (64 bytes, getrandom)
- Token extraction priority: `Authorization: Bearer` header → `dcode-session` cookie
- Supports token blacklist (SHA-256 hash, DashMap storage)

### CSRF Protection

Uses the Double Submit Cookie pattern:
- Cookie name: `dcode-csrf-token` (not HttpOnly — JavaScript must read it)
...
|--------|----------|--------|----------|---------|
| `dcode-session` | ✅ | When HTTPS | Strict(HTTPS) / Lax(HTTP) | 30 days |
| `dcode-csrf-token` | ❌ | When HTTPS | Strict(HTTPS) / Lax(HTTP) | 30 days |
...
| E2E tests | `crates/dcode-app/tests/` | In-memory SQLite | Full HTTP request chain verification |
...
`dcode-app/tests/common/mod.rs` provides shared test utilities:
...
**Step 3: If database access is needed, add to dcode-db**
...
**Step 4: If API types are needed, add to dcode-api-types**

Define request/response types in `dcode-api-types` to keep API contracts centrally managed.

**Step 5: Wire into dcode-app**

1. Add dependency in `dcode-app/Cargo.toml`:
   ```toml
   dcode-my-feature.workspace = true
   ```
...
- [ ] Repository trait defined in dcode-db, implementation uses Sqlite prefix
- [ ] API types defined in dcode-api-types
...
`crates/dcode-runtime/src/node_runtime/`. Packaged builds activate Node
...
`fn main()` calls `dcode_runtime::enhance_process_path()` **before** the
...
`dcode_runtime::Builder::agent(program)` (for long-running agent CLIs
whose stdio the caller owns) or `dcode_runtime::Builder::clean_cli(program)`
