...
1. **Captured real data** — actual sampled wire traffic under
   `~/dcode/protocols/samples/` (e.g. `codex-cli/<ver>/`, `claude-cli/<ver>/`, `codex-acp/`,
...
3. **An official adapter's code** — the codex `app-server` machine-generated JSON schema under
   `~/dcode/protocols/samples/codex-cli/<ver>/schema-full/` (ground truth from the codex binary
...
- All request/response types defined in `dcode-api-types`
- `dcode-api-types` must NOT depend on axum/tower or any HTTP framework
- Use `dcode_common::ApiError` only at API/HTTP boundaries such as routes and middleware. Service/domain code must prefer crate-owned errors (`ConversationError`, `TeamError`, etc.) and map them to `ApiError` in route modules.
...
- Repository traits in `dcode-db`, prefixed with `I`
- Concrete implementations prefixed with `Sqlite`
- Row models in `dcode-db/src/models/`
...
- All assembly happens in `dcode-app`'s `build_*_state()` functions
...
New subprocess spawn sites must go through `dcode_runtime`'s spawn Builder — never raw `tokio::process::Command`. See [ARCHITECTURE.md § Runtime Infrastructure](./ARCHITECTURE.md#runtime-infrastructure) for the correct constructor and details.
...
1. Request/response types → `dcode-api-types/src/{domain}.rs`
2. Handler function → `crates/dcode-{domain}/src/routes.rs`
3. Business logic → `crates/dcode-{domain}/src/service.rs`
4. Register route in `domain_routes()` function
5. Add test → `crates/dcode-{domain}/tests/` or `crates/dcode-app/tests/`

### Add Migration

1. Next number → `ls crates/dcode-db/migrations/`
2. Create `NNN_descriptive_name.sql` with `IF NOT EXISTS`

### Add WebSocket Event

1. Event type → `dcode-api-types`
...
> ⚠️ **When to run what:**
> - During development: only test the crate you're working on → `cargo test -p dcode-<crate>`
...
> - `cargo clippy -p dcode-<crate>` and `cargo test -p dcode-<crate>` typically complete in under 1 minute.

### During Development (fast feedback loop)

```bash
cargo test -p dcode-<crate>                          # Test the crate you changed
cargo clippy -p dcode-<crate> -- -D warnings         # Lint the crate you changed
```
...
cargo clippy -p dcode-<crate1> -p dcode-<crate2> -- -D warnings              # Lint affected crates
cargo test -p dcode-<crate1> -p dcode-<crate2>                               # Test affected crates
```
